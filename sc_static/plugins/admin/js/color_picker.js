var ColorPicker = function($parent, callback, colorC){
	...
	var $colorInputSubmit = $(Fce('input')).attr('type', 'button').attr('value', 'Ok').attr('class', 'sc_ic_ok').bind('click', function(){callback(color);});

	var $colorInputSubmitWrapper = $('<span class="sc_ic_ok"></span>').append($colorInputSubmit);
	$colorInputSubmitWrapper = $('<span class="sc_button"></span>').append($colorInputSubmitWrapper);
