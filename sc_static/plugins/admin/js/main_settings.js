var mainSettings = function( $responderUrl )
{
    ...
    $("#verify_site_email_button").click(
                function() { self.floatbox = new SC_FloatBox({$title: SC.getLanguageText('admin', 'verify_site_email'), $contents: $('#site-email-verify'), height: '170px', width: '350px'}); }
            );
    ...
    this.sendMailButton.click(
        ...
        this.sendVerificationMail = function()
        {
            if ( self.sentRequest === false )
            {
                try
                {
                    window.scForms['configSaveForm'].getElement('siteEmail').removeErrors()
                    window.scForms['configSaveForm'].getElement('siteEmail').validate();
                }
                catch(e)
                {
                    self.floatbox.close();
