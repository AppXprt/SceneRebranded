...
questionValuesField.prototype = {
    ...
    construct : function ( params )
    {
        ...
        self.valuesArea.on( 'click', '.remove', function() {
            if( confirm(SC.getLanguageText('admin', 'questions_edit_delete_value_confirm_message')) )
            {
    ...
    setValue : function ( values )
    {
        ...
        if ( list )
        {
            ...
            self.renderValues();
            SC.trigger('question.value.add', {values:addValues, node:self.tr});
    ...
    deleteValue : function ( element )
    {
        ...
        div.remove();
        SC.trigger('question.value.delete', {value:item, node:self.tr});
        self.updateDataField();
    },
...
infiniteQuestionValuesField.prototype.setValue = function(values){
    ...
    if ( list )
    {
        ...
        $.each(list, function( key, item )
        {
            ...
            {
                SC.error(SC.getLanguageText('admin', 'questions_values_should_not_be_empty'));
            }
        });
...
questionAccountTypesModel.prototype = {
    ...
    loadDataFromView : function ()
    {
        ...
        $.each(self.accountTypes, function( key, item ) {
            self.data[item] = self.nodes[item].hasClass('sc_checkbox_cell_marked');
        } );
    },
...
,
    ...
    change : function( element )
    {
        ...
        var element = $(element);
        //element.toggleClass('sc_checkbox_cell_marked');

        if( element.hasClass('sc_checkbox_cell_marked') )
        {
            element.removeClass('sc_checkbox_cell_marked');
            element.addClass('sc_checkbox_cell');
        }
        else
        {
            element.addClass('sc_checkbox_cell_marked');
            element.removeClass('sc_checkbox_cell');
        }
        ...
        self.data[accountType] = self.nodes[accountType].hasClass('sc_checkbox_cell_marked');
...
questionPagesModel.prototype = {
    ...
    {
        var self = this;
        self.setRequired( self.required_node.hasClass('sc_checkbox_cell_marked') );
        self.setJoin( self.join_node.hasClass('sc_checkbox_cell_marked') );
        self.setEdit( self.edit_node.hasClass('sc_checkbox_cell_marked') );
        self.setView( self.view_node.hasClass('sc_checkbox_cell_marked') );
        self.setSearch( self.search_node.hasClass('sc_checkbox_cell_marked') );
    },
    ...
    change : function( element )
    {
        ...
        switch( true )
        {
            case  ( element.hasClass('required') ) :
                self.setRequired( element.hasClass('sc_checkbox_cell_marked') );
                changes = 'required';
            break;

            case ( element.hasClass('on_join') ) :
                self.setJoin( element.hasClass('sc_checkbox_cell_marked') );
                changes = 'onJoin';
            break;

            case ( element.hasClass('on_edit') ) :
                self.setEdit( element.hasClass('sc_checkbox_cell_marked') );
                changes = 'onEdit';
            break;

            case ( element.hasClass('on_search') ) :
                self.setSearch( element.hasClass('sc_checkbox_cell_marked') );
                changes = 'onSearch';
            break;

            case ( element.hasClass('on_view') ) :
                self.setView( element.hasClass('sc_checkbox_cell_marked') );
                changes = 'onView';
            break;
        }
...
var indexQuestions = function( $params )
{
    ...
    var $questionDiv = $('.sc_admin_profile_questions_list_div');
    var $questionTable = $('.sc_admin_profile_questions_list');
    ...
    SC.bind( "admin.add_account_type", function( params ) {
            ...
            if ( params && params.result.add == true && params.accountTypeName )
            {
                ...
                SC.info(SC.getLanguageText('admin', 'questions_account_type_was_added'));
                
                window.location.reload();
            }
            else
            {
                SC.error(SC.getLanguageText('admin', 'questions_account_type_added_error'));
            }
        });

    SC.bind( "admin.update_account_type", function( params ) {
            ...
            if ( params && params.result.update == true && params.accountTypeName )
            {
                ...
                SC.info(SC.getLanguageText('admin', 'questions_account_type_was_updated'));
            }
        });
    ...
    $('a.question_edit_button').click( function()  {
        ...
        if ( questionId )
        {
            SC.ajaxFloatBox('ADMIN_CMP_EditQuestion', [questionId], {
                width: '700px',
                title: SC.getLanguageText('admin', 'questions_edit_profile_question_title')
            } );
        }
    } );
    ...
    $('a.add_account_type').click( function()  {
        ...
        SC.ajaxFloatBox('ADMIN_CMP_AddAccountType', [], {
            width: '500px',
            title: SC.getLanguageText('admin', 'questions_add_account_type_title')
        } );
    } );
    ...
    $('a.question_edit_account_type_button').click( function()  {
        ...
        {
            SC.ajaxFloatBox('ADMIN_CMP_EditAccountType', [input.val()], {
                width: '500px',
                title: SC.getLanguageText('admin', 'questions_edit_account_type_title')
            } );
        }
    } );
   ...
   $('a.question_delete_account_type_button').click(
        function(event) {
            ...
            if( confirm(SC.getLanguageText('admin', 'questions_delete_account_type_confirmation')) )
            {
                self.sendRequest = true;
                $.ajax( {
                    url: self.responderUrl,
    ...
    $('input.add_new_question_button').click( function()  {
        //window.location = $questionAddUrl
        SC.ajaxFloatBox('ADMIN_CMP_AddQuestion', [], {
            width: '700px',
            title: SC.getLanguageText('admin', 'questions_add_profile_question_title')
        } );
    } );
    
    $('input.add_new_section_button').on( 'click', function()  {
        SC.ajaxFloatBox('ADMIN_CMP_AddQuestionSection', [], {
            width: '600px',
            title: SC.getLanguageText('admin', 'questions_profile_question_sections_title')
        } );
    } );
    ...
    $('.question_delete_button').click(
        function()
        {
            ...
            if( confirm(SC.getLanguageText('admin', 'questions_delete_question_confirmation_' + $questionId)) )
            {
                self.sendRequest = true;
                $.ajax( {
                    url: self.responderUrl,
                    ...
                    success: function( result ) {
                        ...
                        if ( result['result'] == 'success' )
                        {
                            ...
                            $question_tr.removeClass('sc_alt1');
                            $question_tr.removeClass('sc_alt2');

                            $questionTable.find('.question_tr:not(.no_question):odd').addClass('sc_alt2');
                            $questionTable.find('.question_tr:not(.no_question):even').addClass('sc_alt1');
        ...
        function() {
            ...
            .done(
                function ( result ) {
                    ...
                    var message = SC.getLanguageText('admin', 'questions_delete_section_confirmation');
    ...
    $(".edit_sectionNameLang").click( function() {
         ...
         var $element = $tr.find(".section_value .sc_section_label");
         var $name = $(this).parents(".sc_admin_profile_questions_list:eq(0)").attr("sectionName");
     ...
     $questionDiv.sortable({
       ...
       items: '.sc_admin_profile_questions_list',
       cancel: 'no_section',
       cursor: 'move',
       tolerance: 'pointer',
       handle: '.question_section_tr',
       placeholder: 'section_placeholder sc_table_2 sc_smallmargin sc_admin_content',
       ...
       update: function(event, ui)
       {
            ...
            $questionDiv.find('.sc_admin_profile_questions_list:not(.no_section)').each(function(ord, o){
        ...
        {
            //$(ui.placeholder).append('<table class="sc_table_2 sc_smallmargin"><tr><td colspan="9"><div style="width:869px;"></div></td></tr></table>');
        ...
        helper: function(event, ui)
        {
            ...
            return $('<div class="sc_dnd_helper" style="width: 180px;height: 30px; text-align:center; vertical-align:middle;"></div>');
        }
     ...
     $questionTable.not(".about_my_match").sortable(
     {
       ...
       snapToleranse: 50,
       forcePlaceholderSize: true,
       connectWith: '.sc_admin_profile_questions_list:not(.no_section):not(.about_my_match)',

        update: function(event, ui) {

             var newSection = ui.item.parents(".sc_admin_profile_questions_list:eq(0)");
        ...
        helper: function(event, ui)
        {
            this.oldSection = ui.parents(".sc_admin_profile_questions_list:eq(0)");
            var itemWidth = ui.outerWidth();
            if (itemWidth > 160)
            {
                var k = 160 / ui.outerWidth();
