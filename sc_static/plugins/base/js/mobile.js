var SCMobile = function(){
    var self = this;
    var langs = {};
    var events = {};        
    var $overlay = $('#scm_overlay'), leftSbClass = 'scm_sidebar_left_active', rightSbClass = 'scm_sidebar_right_active', $main = $('#main');
    var $leftHeaderBtn = $('#scm_header_left_btn'), $rightHeaderBtn = $('#scm_header_right_btn'), $heading = $('#scm_heading'), $title = $('title');
    var state = "content";

    $('.scm_content_header_count').on("click.nav", function(){
        if ( state === "content" ) {
    ...
    this.showLeftSidebar = function(){
        ...
        state = 'left';
        self.trigger('mobile.shsc_sidebar', {type: state});
    };
    ...
    this.showRightSidebar = function(options){
        ...
        if (!silent){
            state = 'right';
            self.trigger('mobile.shsc_sidebar', {type: "right"});
        }
    };
...
//        var states = [];
//        var lBtnClass = 'scm_nav_menu';
//        var rBtnClass = 'scm_nav_profile';
//
...
//    );
    ...
    this.info = function( message ){
     ...
     var $messageCont = $('<div class="scm_msg_wrap"></div>');
     var messageTime = 3000;
     $messageCont.appendTo($('body'));

    this.message = function( message, type, paramTime ){
        var $messageNode = $( '<div class="scm_msg_block scm_msg_'+type+' clearfix" style="display:none;"><a href="javascript://" onclick="$(this).closest(\'.scm_msg_block\').slideUp(200, function(){$(this).remove();})" class="scm_close_btn"></a><span class="scm_msg_txt">'+message+'</span></div>').appendTo($messageCont);
    ...
    {
        SCM.ajaxFloatBox("BASE_MCMP_Flag", [entityType, entityId], {
            width: 315,
            title: SCM.getLanguageText('base', 'flag_as')
        });
    };

    this.authorizationLimitedFloatbox = function( message )
    {
        SCM.ajaxFloatBox("BASE_MCMP_AuthorizationLimited", [message],
            {title: SCM.getLanguageText('base', 'authorization_limited_permissions')}
        );
    };
        ...
        $.each( items, function(key, value){
                $('#'+value['id']).bind('click', {value:value},
                    function(e){
                        ...
                        linkIdSelector.removeClass('scm_box_menu_item_active');
                        $(this).addClass('scm_box_menu_item_active');
    ...
    this.loadComponent = function( cmpClass, p1, p2 )
    {
        ...
        if ( options.place )
        {
            $preloader = $('<div class="scm_preloader ' + options.addClass + '"></div>');
        ...
        var ajaxOptions = {
            ...
            success: function(markup)
            {
                ...
                {
                    SCM.addCss(markup.styleDeclarations);
                }

                if (markup.beforeIncludes)
                {
                    SCM.addScript(markup.beforeIncludes, options.scope, options.context);
                }

                if (markup.scriptFiles)
                {

                    SCM.addScriptFiles(markup.scriptFiles, function()
                    {
                        if (markup.onloadScript)
                        {
                            SCM.addScript(markup.onloadScript, options.scope, options.context);
                            options.onLoad();
                        }
                    });
                }
...
var SCM_Console = function( params )
{
    ...
    var $tabCounter = $(".scm_sidebar_count_txt", "#console-tab-notifications");
    var $preloader = $("#console_preloader");
    var $body = $("#console_body");

    /* on right sidebar shown */
    SCM.bind("mobile.shsc_sidebar", function( data ){

        var beforeShowSidebarData = {openDefaultTab: true};
        SC.trigger("mobile.before_shsc_sidebar", beforeShowSidebarData);
        ...
        if ( data.type == "right" ) {
            ...
            if (beforeShowSidebarData.openDefaultTab){
                ...
                SCM.trigger("mobile.open_sidebar_tab", {key: self.lastSelectedTab});
...
//                });
    ...
    SCM.bind("mobile.hide_sidebar", function( data ){
        if ( data.type == "right" )
        {
            self.consoleState = 'closed';
            self.counter = 0;
    ...
    SCM.bind("mobile.console_item_removed", function( data ){
        self.hideCounter();
    });

    /* open right sidebar and specified tab */
    SCM.bind("mobile.open_sidebar_tab", function( data ){
        SCM.showRightSidebar({silent: true});
        var pageData = self.getPageData(data.key);
        self.activateTab(data.key);
        self.loadPage(data.key, pageData.cmpClass);
    });

    SCM.bind('mobile.console_shsc_counter', function( data ){
        self.showCounter(data.counter, data.tab, data.options);
    });

    /* console tab clicked */
    $("a.scm_sidebar_console_item_url").click(function() {
        ...
        SCM.loadComponent(cmpClass, { },
            {
                onReady: function(html){
                    ...
                    SCM.trigger('mobile.console_page_loaded', {key : key});
                }
            }
        );
    ...
    this.activateTab = function( key ) {
        $(".scm_sidebar_console_item").removeClass('scm_sidebar_console_item_active');
        $(".scm_sidebar_console_" + key).addClass("scm_sidebar_console_item_active");
    };
    ...
    this.showCounter = function(counter, tab, options){
        ...
        if ( $(".scm_content_header_count").is(":visible") ) {
            $counter.html(self.counter);
        }
        else {
            // show counter
            $(".scm_nav_profile").fadeOut("fast", function(){
                $counter.html(self.counter);
                $(".scm_content_header_count").css({display: "inline-block"}).animate({right: "5"}, {duration: 300});
            });
        }

        if ( tab ) {
            var $tab = $("#console-tab-notifications");
            if ( $(".scm_sidebar_count", $tab).is(":visible") ) {
                $tabCounter.html(counter);
            }
            else {
                // show counter
                $tabCounter.html(counter);
                $(".scm_sidebar_count", $tab).fadeIn();
            }
        }
        else
        {
    ...
    this.hideCounter = function(){
        $(".scm_content_header_count").hide();
        $counter.html("");
        $(".scm_nav_profile").show();
        $(".scm_sidebar_count", "#console-tab-notifications").hide();
        $tabCounter.html("");
    };
    ...
    this.updateMarkup = function( data ) {
        if ( data.new_items )
        {
            ...
            $.each(data.new_items, function( section, markup ){
                if ( markup != '' )
                {
                    SCM.trigger('mobile.console_load_new_items', {page: 'notifications', section: section, markup: markup});
                }
            });
        }
    };
    ...
    $("body")
        .on("click", "li.scm_sidebar_msg_disabled", function(){
            SCM.error(
                SCM.getLanguageText(
                    'base',
    ...
    this.init = function() {
        SCM.getPing().setRspUrl(params.rspUrl);
        var cmd = SCM.getPing().addCommand("mobileConsole", {
            before: function() {
...
var SCM_InvitationsConsole = function( params )
{
    var self = this;
    self.params = params;
    ...
    this.consoleAcceptRequest = function( $node )
    {
        ...
        var $row = $node.closest(".scm_sidebar_msg_item");
        $.ajax({
            ...
            success : function(data) {
                if ( data ) {
                    $row.remove();
                    SCM.trigger('mobile.console_item_removed', { section : "invitations" });
                    if ( data.result == true ) {
                        SCM.info(data.msg);
                    }
                }
    ...
    {
        $node.addClass("scm_sidebar_load_more_preloader");

        var exclude =
            $("li.scm_sidebar_msg_item", "#invitations-list")
                .map(function(){
        ...
        SCM.loadComponent(
            ...
            {limit: self.params.limit, exclude: exclude},
            {
                onReady: function(html){
                    ...
                    $node.removeClass("scm_sidebar_load_more_preloader");
                }
            }
        );
    };
    ...
    {
        $("#invitations-load-more").closest(".scm_sidebar_msg_list").hide();
    };

    $("body")
        .on("click", "a.scm_invite_accept", function(){
            self.consoleAcceptRequest($(this));
        })
        .on("click", "a.scm_invite_ignore", function(){
    ...
    SCM.bind("mobile.console_hide_invitations_load_more", function(){
        self.hideLoadMoreButton();
    });

    SCM.bind("mobile.console_load_new_items", function(data){
    ...
    SCM.bind("mobile.hide_sidebar", function(data){
        if ( data.type == "right" )
        {
            SCM.unbind("mobile.console_hide_invitations_load_more");
            SCM.unbind("mobile.console_load_new_items");
            $("body")
                .off("click", "a.scm_invite_accept")
                .off("click", "a.scm_invite_ignore")
...
ScForm.prototype = {
    ...
    submitForm: function(){
        ...
        var buttons = $('input[type=button], input[type=submit], button', '#' + this.id).addClass('sc_inprogress');

        if( this.ajax ){
            SCM.inProgressNode(buttons);
            var postString = '';
            ...
            $.ajax({
                ...
                error: function( XMLHttpRequest, textStatus, errorThrown ){
                    SCM.error(textStatus);
                    throw textStatus;
                },
                complete: function(){
                    SCM.activateNode(buttons);
                }
            });
...
/* PING */
SCM_PingCommand = function( commandName, commandObject, stack )
{
...
SCM_PingCommand.PROTO = function()
{
...
SCM_PingCommand.prototype = new SCM_PingCommand.PROTO();
...
SCM_Ping = function()
{
    ...
    this.addCommand = function( commandName, commandObject )
    {
        ...
        _commands[commandName] = new SCM_PingCommand(commandName, commandObject, _stack);
...
SCM_Ping.getInstance = function()
{
    if ( !SCM_Ping.pingInstance )
    {
        SCM_Ping.pingInstance = new SCM_Ping();
    }

    return SCM_Ping.pingInstance;
};

SCM.getPing = function()
{
    return SCM_Ping.getInstance();
};


/* FloatBox */

SCM.getActiveFloatBox = function() {
    return window.SCActiveFloatBox || null;
};

SCM.FloatBox = (function() {
    var _overlay = $(".scm_overlay");
...
SCM.ajaxFloatBox = function(cmpClass, params, options)
{
    params = params || [];
...
window.SC = window.SCM;

/* Comments */
var scmobileComments = function( contextId, formName, genId ){
...
scmobileComments.prototype = {
    repaintCommentsList: function( data ){
        scForms[this.formName].getElement('commentText').resetValue();
        if(data.error){
            SC.error(data.error);
            return;
        }
    ...
    initForm: function( textareaId, submitId ){
        ...
        //end
        scForms[this.formName].bind('success',
            function(data){                
                resetTa();
                self.repaintCommentsList(data);
                SC.trigger('base.comment_add', {entityType: data.entityType, entityId: data.entityId}, this);
                $textA.focus();
            }
        );

       scForms[this.formName].bind('submit',
            function(data){
        ...
        scForms[this.formName].reset = false;
    }

};

SCM.postRequest = function(url, params)
{
    var $form = $("<form></form>")
        .attr({
            action: url,
...
var scmobileCommentsList = function( params ){
	this.$context = $('#' + params.contextId);
	$.extend(this, params, owCommentListCmps.staticData);
};

scmobileCommentsList.prototype = {
	init: function(){
		var self = this;
//
...
//        );
          ...
          $('.cmnt_load_more_cont', this.$context).click(
            function(){
                ...
                self.reload();
                $(this).removeClass('scm_load_more');
            }
          );
    ...
    updateMarkup: function( data ){
        ...
        SC.trigger('base.comments_list_update', {entityType: data.entityType, entityId: data.entityId, id:this.genId});
    },
...
SCM.Users = null;
SCM_UsersApi = function( _settings )
{
    var _usersApi = this;
