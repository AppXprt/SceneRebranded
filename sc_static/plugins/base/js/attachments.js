var SCFileAttachment = function(params) {
    $.extend(this, params);
    this.$context = $('#' + this.uid);
    this.$previewCont = $('.sc_file_attachment_preview', this.$context);
    ...
    var refreshClasses = function() {
        ...
        $.each(items,
                function(index, data) {
                    data['html'].removeClass('sc_file_attachment_block1').removeClass('sc_file_attachment_block2').addClass('sc_file_attachment_block' + itemIndex);
    ...
    this.addItem = function(data, loader, customDeleteUrl, customConfirmation) {
        data['html'] = $('<div><div class="sc_file_attachment_info">' +
                '<div class="sc_file_attachment_name">' + data.name + ' <span class="sc_file_attachment_size" style="display: inline-block;">(' + data.size + 'KB)</span></div>' +
                '<div class="sc_file_attachment_preload" style="display:' + (loader ? 'block' : 'none') + ';"></div>' +
                '<a href="javascript://" class="sc_file_attachment_close"></a>' +
                '</div></div>');
        ...
        SC.trigger('base.attachment_rendered', {'data' : data}, this);
 
        $('.sc_file_attachment_close', data['html']).bind('click', function() {
            var confirmed = true;
    ...
    this.initInput = function() {
        var $input = $('<input class="mlt_file_input" type="file"' + (this.multiple ? ' multiple=""' : '') + ' name="sc_file_attachment[]" />');
        ...
        $input.change(
                function(e) {
                    ...
                    SC.trigger('base.add_attachment_to_queue', {'pluginKey': self.pluginKey, 'uid': self.uid, 'items': items});
...
//        $form.appendTo($('body'));
        ...
        if (data.items) {
            $.each(data.items, function(index, item) {
                ...
                if (item.result) {
                    ...
                    if (self.showPreview) {
                        $('.sc_file_attachment_preload', items[index]['html']).hide();
                    }
                } else {
                    self.deleteItem(index);
                    if (self.showPreview) {
                        SC.error(item.message);
                    }
                }
            });

            SC.trigger('base.update_attachment', {'pluginKey': self.pluginKey, 'uid': self.uid, 'items': data.items});
        }
    ...
    this.deleteItem = function(id, customDeleteUrl) {
        SC.trigger('base.attachment_deleted', {'id' : id}, this);
...
SC.bind('base.file_attachment', function(data) {
...
var SCPhotoAttachment = function(params) {
    $.extend(this, params);
    var self = this, $previewCont = $('#' + this.previewId),
            $buttonCont = $('#' + this.buttonId),
            $form,
            $iframe = null,
            $item = $('.sc_photo_attachment_pic', $previewCont),
            canceled = false;
    ...
    this.initInput = function() {
        ...
        SC.trigger('base.attachment_shsc_button_cont', self.eventParams);
        ...
        $('div', $item).unbind('click').click(function() {
            ...
            $previewCont.hide();
            self.initInput();
            SC.trigger('base.attachment_deleted', self.eventParams);
        });
        ...
        $input.change(
                function(e) {
                    $buttonCont.hide();
                    SC.trigger('base.attachment_hide_button_cont', self.eventParams);
                    $previewCont.show();
                    ...
                    $iframe = $('<div style="display:none" id="hd_' + self.uid + '"><div>').appendTo($('body'))
                            ...
                            .append($form);
                    $form.submit();
                    SC.trigger('base.add_photo_attachment_submit', self.eventParams);
                }
        );
    };
        ...
        if (data.result) {
            ...
            previewImg.onload = function() {
                $item.removeClass('loading').css({backgroundImage: 'url(' + data.url + ')'}).click(function() {
                    SC.showImageInFloatBox(data.url)
                });
                $('div', $item).unbind('click').click(function(e) {
                    e.stopPropagation();
                    self.initInput();
                    SC.trigger('base.attachment_deleted', eventParams);
                });
                SC.trigger('base.attachment_added', eventParams);
            };
    ...
    SC.bind('base.photo_attachment_reset', function(data) {
...
SC.bind('base.photo_attachment_uid_update', function(data) {
