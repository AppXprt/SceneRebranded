var ScUtils = function(){
    var langs = {};
    var messageTime = 3000;
    var $messageCont = $('<div class="sc_message_cont"></div>');
    var events = {};
    ...
    this.message = function( message, type, paramTime ){
        var $messageNode = $('<div class="sc_message_node '+type+'" style="display:none;"><div><div><a class="close_button" href="javascript://" onclick="$(this).closest(\'.sc_message_node\').slideUp(200, function(){$(this).remove();})"></a>'+message+'</div></div></div>').appendTo($messageCont);
    ...
    {
        SC.registry['flag-panel-fb'] = SC.ajaxFloatBox("BASE_CMP_Flag", [entityType, entityId], {
            width: 315,
            title: SC.getLanguageText('base', 'flag_as')
        });
    };
    ...
    {
    	SC.Users.showUsers(userIds, title);
    },

    this.getActiveFloatBox = function getFloatBox()
    {
    	if ( typeof window.SCActiveFloatBox === 'undefined' )
    	{
    		return false;
    	}

    	return window.SCActiveFloatBox;
    },
     ...
     * Examples:
     * 1:
     * SC.loadComponent(cmpClass, '.place_to_node');
     *
     * 2:
     * SC.loadComponent(cmpClass, function(){
     *     //onReady event
     *     //Add the html somewhere
     * });
     *
     * 3:
     * SC.loadComponent(cmpClass, [ p1, p2, ... ], '.place_to_node');
     *
     * 4:
     * SC.loadComponent(cmpClass, [ p1, p2, ... ], function(html){
     *     //onReady event
     *     //Add the html somewhere
     * });
     *
     * 5:
     * SC.loadComponent(cmpClass, [ p1, p2, ... ],
     * {
     *     onLoad: function(){},
     *     onError: function(){},
     *     onComplete: function(){},
    ...
    this.loadComponent = function( cmpClass, p1, p2 )
    {
        ...
        if ( options.place )
        {
            $preloader = $('<div class="sc_ajaxloader_preloader sc_preloader_content ' + options.addClass + '"></div>');
        ...
        var ajaxOptions = {
            ...
            success: function(markup)
            {
                ...
                SC.bindAutoClicks($contentHtml);
                SC.bindTips($contentHtml);
                ...
                {
                    SC.addCss(markup.styleDeclarations);
                }

                if (markup.beforeIncludes)
                {
                    SC.addScript(markup.beforeIncludes, options.scope, options.context);
                }

                if (markup.scriptFiles)
                {

                    SC.addScriptFiles(markup.scriptFiles, function()
                    {
                        if (markup.onloadScript)
                        {
                            SC.addScript(markup.onloadScript, options.scope, options.context);
                            options.onLoad();
                        }
                    });
                }
	...
	{
        var fb = SC.ajaxFloatBox("BASE_CMP_LanguageValueEdit", [prefix, key, true], {width: 520, title: this.getLanguageText('admin', 'edit_language')});

        SC.bind("admin.language_key_edit_success", function( e ) {
            fb.close();
            SC.unbind("admin.language_key_edit_success");
            success(e);
        });
	};
    ...
    this.bindAutoClicks = function(context){
        ...
        $.each(autoClicks, function(i,o){
            ...
            .bind('focus.auto_click', {context:context},
                function(e){
                    $('.sc_submit_auto_click', e.data.context).show();
            ...
            .bind('blur.auto_click', {context:context},
                function(e){
                    if( $(this).hasClass('invitation') ){
                        $('.sc_submit_auto_click', e.data.context).hide();
                    }
                }
            );*/
        });
    ...
    this.initWidgetMenu = function( items ){
        ...
        $.each( items, function(key, value){
                if( $toolbarCont === null ){
                    $contex = $('#'+value['contId']).closest('.sc_box, .sc_box_empty');
                    $toolbarCont = $('.sc_box_toolbar_cont', $contex);
                }
                        ...
                        if( e.data.value.toolbarId != undefined ){
                            if( e.data.value.toolbarId ){
                                if( $toolbarCont.length === 0 ){
                                    $toolbarCont = $('<div class="sc_box_toolbar_cont"></div>');
    ...
    this.shscTip = function( $el, params ){
        ...
        var shscTipN = function(){
            ...
            var tod = setTimeout( function(){
                $el.data('scTip').show( 1,
                    function(){
                        if( params.hideEvent ){
                            $el.bind(params.hideEvent, function(){SC.hideTip($el)});
                        }
                        $el.data('scTipStatus', true);
                        if( $el.data('scTipHide') == true ){
                            SC.hideTip($el);
                        }
                    }
                );
        ...
        if( params.show != null ){
            ...
            showContent = '<span class="sc_tip_title">'+$el.attr('title')+'</span>';
        }

        var $rootEl = $('<div class="sc_tip sc_tip_'+params.side+'"></div>').css({display:'none'}).append($('<div class="sc_tip_arrow"><span></span></div><div class="sc_tip_box">'+ showContent +'</div>'));
    ...
    this.hideTip = function( $el ){
        ...
        if( $el.data('scTip') && $el.data('scTipStatus') == true ){
            $el.data('scTip').hide();
            $el.data('scTipStatus', false);
            $el.data('scTipHide', false);
        }
    };

    this.bindTips = function( $context ){
      //$('*[title]', $context).each( function(i, o){$(o).hover(function(){SC.shscTip($(this), {timeout:200})}, function(){SC.hideTip($(this))})});
      $('*[title]', $context).each( function(i, o){
          $(o).on('mouseenter', function(){ SC.shscTip($(this), {timeout:200}); });
          $(o).on('mouseleave', function(){ SC.hideTip($(this)); });
      });
    };
    ...
    {
        var floatBox = new SC_FloatBox({layout: 'empty'});
...
jQuery.fn.extend({
	inprogress: function() {
		this.each(function()
		{
			var $this = jQuery(this).addClass('sc_inprogress');
			this.disabled = true;
	...
	activate: function() {
		this.each(function()
		{
			var $this = jQuery(this).removeClass('sc_inprogress');
			this.disabled = false;
 ...
 */
function SC_FloatBox(options)
{
    var fl_box = this;
    var fb_class;
    this.parentBox = SC.getActiveFloatBox();
    ...
    if (typeof document.body.style.maxHeight === 'undefined') { //if IE 6
            ...
            { //iframe to hide select elements in ie6
                ...
                fb_class = SC_FloatBox.detectMacXFF() ? 'floatbox_overlayMacFFBGHack' : 'floatbox_overlayBG';
    ...
    window.SCActiveFloatBox = this;
}

SC_FloatBox.version = 3;
SC_FloatBox.detectMacXFF = function()
{
    var userAgent = navigator.userAgent.toLowerCase();
    return (userAgent.indexOf('mac') != -1 && userAgent.indexOf('firefox') != -1);
}

SC_FloatBox.prototype = {
    ...
    showPreloader: function( preloaderContent )
    {
        ...
        this.$canvas.addClass('sc_floatbox_loading');
    },

    hidePreloader: function()
    {
        this.$canvas.removeClass('sc_floatbox_loading');
    },
    ...
    close: function( options )
    {
        ...
        window.SCActiveFloatBox = this.parentBox;
...
var ScFormElement = function( id, name ){
    this.id = id;
    this.name = name;
...
ScForm.prototype = {
    ...
    submitForm: function(target){
        ...
        $(target).addClass('sc_inprogress');

        if( this.ajax ){
            SC.inProgressNode(buttons);
            var postString = '';
...
var ScTextField = function( id, name, invitationString ){
    var formElement = new ScFormElement(id, name);
    if( invitationString ){
...
var ScTextArea = function( id, name, invitationString ){
    var formElement = new ScFormElement(id, name);
    if( invitationString ){
        addInvitationBeh(formElement, invitationString);
    }
...
var ScWysiwyg = function( id, name, invitationString ){
    var formElement = new ScFormElement(id, name);
...
var ScRadioField = function( id, name ){
    var formElement = new ScFormElement(id, name);
...
var ScCheckboxGroup = function( id, name ){
    var formElement = new ScFormElement(id, name);
...
var ScCheckboxField = function( id, name ){
    var formElement = new ScFormElement(id, name);
...
var ScAvatarField = function( id, name, params ){
    var formElement = new ScFormElement(id, name);
    var $preview = $(formElement.input).closest(".sc_avatar_field").find(".sc_avatar_field_preview");
    ...
    $(formElement.input).change(function(e){
        document.avatarFloatBox = SC.ajaxFloatBox(
            ...
            { width : 749, title: SC.getLanguageText('base', 'avatar_change') }
        );
    });
...
/* Comments */
var ScComments = function( params ){
    $.extend(this, params);
    this.$cmpContext = $('#' + this.contextId);
    var self = this;
    var sleeping = false;
    this.eventParams = {entityType:self.entityType, entityId:self.entityId, customId:self.customId};
    this.submitHandler = function(){SC.error('FORM IS NOT INITIALIZED!');};
    this.attachmentInProgressHandler = function(){SC.error(params.labels.attachmentLoading);};
    ...
    this.$formWrapper = $('.sc_comments_form_wrap', this.$cmpContext);
    this.$formWrapperPre = $('.sc_comments_form_wrap_pre', this.$cmpContext);
    this.$hiddenBtnCont = $('.comments_hidden_btn', this.$cmpContext);
    this.$commentsInputCont = $('.sc_comments_item_info', this.$formWrapper);
    this.initTextarea();
    this.attachmentInfo = false;
    this.oembedInfo = false;

    SC.bind('base.comments_list_init',
        function(data){
            if( sleeping ) return;
            if( checkAddress(data) ){
    ...
    SC.bind('base.add_photo_attachment_submit',
        function(data){
            if( sleeping ) return;            
            if( data.uid == self.attchUid ){
                ...
                SC.trigger('base.comment_button_show', self.eventParams);
            }
        }
    );

    SC.bind('base.attachment_added',
        function(data){
            if( sleeping ) return;            
            if( data.uid == self.attchUid ){                
                ...
                SC.trigger('base.comment_attachment_added', self.eventParams);
            }
        }
    );

    SC.bind('base.attachment_deleted',
        function(data){
            if( sleeping ) return;
            if( data.uid == self.attchUid ){
                self.attachmentInfo = false;
                self.$hiddenBtnCont.hide();
                SC.trigger('base.comment_attachment_deleted', self.eventParams);
            }
        }
    );

    SC.bind('base.move_comments_form',
        function(data){
            if( sleeping ) return;
...
ScComments.prototype = {
    repaintCommentsList: function(data){
        if(data.error){
            SC.error(data.error);
            return;
        }
        //SC.trigger('base.comments_list_update', {entityType: data.entityType, entityId: data.entityId, id:this.uid});
        $('.comments_list_cont', this.$cmpContext).empty().append($(data.commentList));        
        SC.addScript(data.onloadScript);
    },
    ...
    sendMessage: function( message ){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                SC.trigger('base.photo_attachment_uid_update', {uid:self.attchUid, newUid:data.newAttachUid});
                self.eventParams.commentCount = data.commentCount;
                SC.trigger('base.comment_added', self.eventParams);
                self.attchUid = data.newAttachUid;
                
                self.$formWrapper.removeClass('sc_preloader');
    ...
    initTextarea: function(){
        ...
        this.realSubmitHandler = function(){
            ...
            if( this.mediaAllowed ){
                SCLinkObserver.getObserver(self.textAreaId).resetObserver();
            }
            self.$attchCont.empty();
            SC.trigger('base.photo_attachment_reset', {pluginKey:self.pluginKey, uid:self.attchUid});
            SC.trigger('base.comment_add', self.eventParams);
            
            self.$formWrapper.addClass('sc_preloader');
        ...
        this.$textarea
                    ...
                    if( e.which === 13 && !e.shiftKey ){
                        ...
                        var textBody = $(this).val();
                         ...
                         if ( $.trim(textBody) == '' && !self.attachmentInfo && !self.oembedInfo ){
                             SC.error(self.labels.emptyCommentMsg);
                            return false;
                         }
            ...
            .one('focus', function(){$(this).removeClass('invitation').val('').autosize({callback:function(data){SC.trigger('base.comment_textarea_resize', self.eventParams);}});});

       this.$hiddenBtnCont.unbind('click').click(function(){self.submitHandler();});

       if( this.mediaAllowed ){
           SCLinkObserver.observeInput(this.textAreaId, function( link ){
            if( !self.attachmentInfo ){
                 self.$attchCont.html('<div class="sc_preloader" style="height: 30px;"></div>');
                 this.requestResult( function( r ){
                     self.$attchCont.html(r);
                     self.$hiddenBtnCont.show();

                     SC.trigger('base.comment_attach_media', {})
                 });
...
var ScCommentsList = function( params ){
	this.$context = $('#' + params.contextId);
	$.extend(this, params, owCommentListCmps.staticData);
        this.$loader = $('.sc_comment_list_loader', this.$context);
}

ScCommentsList.prototype = {
	init: function(){
	var self = this;
        $('.sc_comments_item', this.$context).hover(function(){$('.cnx_action', this).show();$('.sc_comments_date_hover', this).hide();}, function(){$('.cnx_action', this).hide();$('.sc_comments_date_hover', this).show();});
        this.$loader.one('click',
            function(){
                self.$loader.addClass('sc_preloader');
                $('a', self.$loader).hide();
...
//        SC.bind('base.comments_list_update',
//            function(data){
...
//        );
        ...
        SC.trigger('base.comments_list_init', {entityType: this.entityType, entityId: this.entityId}, this);

        SC.bind('base.comment_add', function(data){ if( data.entityType == self.entityType && data.entityId == self.entityId ) self.initialCount++ });
            ...
            function(i,o){
                $('#'+i).click(
                    function(){
                        ...
                        {
                           $(this).closest('div.sc_comments_item').slideUp(300, function(){$(this).remove();});
                            ...
                            $.ajax({
                                ...
                                success : function(data){
                                    ...
                                    SC.trigger('base.comment_delete', eventParams, this);
                                }
                            });
                        }
                    }
		...
		$.ajax({
            ...
            success : function(data){
               if(data.error){
                ...
                self.$loader.removeClass('sc_preloader');
                $('a', self.$loader).hide();
                self.$context.replaceWith(data.commentList);
                SC.addScript(data.onloadScript);
            },
            error : function( XMLHttpRequest, textStatus, errorThrown ){
                SC.error('Ajax Error: '+textStatus+'!');
                throw textStatus;
            }
        });
	}
    ...
    updateRate: function( rate ){
        ...
        $.ajax({
            ...
            success : function(data){
                ...
                SC.trigger('base.rate_update', {entityType: self.entityType, entityId: self.entityId, rate: rate, ownerId: self.ownerId}, this);
            },
            error : function( XMLHttpRequest, textStatus, errorThrown ){
...
SCLinkObserver =
{
    ...
    {
        this.observers[inputId] = new SCLinkObserver.handler(inputId, callBack);
    },
...
SCLinkObserver.handler = function( inputId, callBack )
{
...
SCLinkObserver.handler.prototype =
{
        ...
        $.ajax({
            ...
            success: function( r )
            {
                if ( r.content )
                {
                    ...
                    if ( r.content.js )
                    {
                        SC.addScript(r.content.js);
                    }
                }

                if ( r.attachment && SC_AttachmentItemColletction[r.attachment] )
                {
                    SC_AttachmentItemColletction[r.attachment].onChange = function(oembed){
...
SC_AttachmentItemColletction = {};

SC_Attachment = function(uniqId, data)
{
    var self = this;
    ...
    //SC.resizeImg(this.$('.EQ_AttachmentImageC'),{width:'150'});

    this.$('.SC_AttachmentSelectPicture').click(function()
    {
        self.showImageSelector();
    });

    this.$('.SC_AttachmentDelete').click(function()
    {
        $(self.node).remove();
...
SC_AttachmentProto = function()
{
    ...
    this.showImageSelector = function()
    {
        ...
        $contents = this.$('.SC_AttachmentPicturesFbContent')

        fb = new SC_FloatBox({
            $title: this.$('.SC_AttachmentPicturesFbTitle'),
            $contents: $contents,
            width: 520
        });

        $contents.find('.SC_AttachmentPictureItem').unbind().click(function()
        {
            var img = $('img', this);
    ...
    this.changeImage = function( url )
    {
        ...
        original = this.$('.SC_AttachmentImage');
...
SC_Attachment.prototype = new SC_AttachmentProto();
...
SC_PingCommand = function( commandName, commandObject, stack )
{
...
SC_PingCommand.prototype = new SC_PingCommand.PROTO();
...
SC_Ping = function()
{
    ...
    this.addCommand = function( commandName, commandObject )
    {
        ...
        _commands[commandName] = new SC_PingCommand(commandName, commandObject, _stack);
...
SC_Ping.getInstance = function()
{
    if ( !SC_Ping.pingInstance )
    {
        SC_Ping.pingInstance = new SC_Ping();
    }

    return SC_Ping.pingInstance;
}
...
SC.getPing = function()
{
    return SC_Ping.getInstance();
};

/* Add command example
SC.getPing().addCommand('ajaxim', {
    params: {
        p1: 1,
        p2: 3
    },
...
SC.addScroll = function( node, settings )
{
    settings = settings || {};
    settings = $.extend({}, SC.Scroll.settings, settings);
    ...
    if ( !$node.hasClass('sc_scrollable') )
    {
        $node.addClass('sc_scrollable');
    }
...
SC.removeScroll = function( node )
{
    ...
    $node.removeClass('sc_scrollable');
};

SC.updateScroll = function( node, settings )
{
    var jsp = $(node).data('jsp');
...
SC.addScrolls = function( context )
{
    context = context || null;

    $('.sc_scrollable', context).each(function()
    {
        SC.addScroll(this);
    });
};

SC.removeScrolls = function( context )
{
    context = context || null;

    $('.sc_scrollable', context).each(function()
    {
        SC.removeScroll(this);
    });
};

SC.updateScrolls = function( context )
{
    context = context || null;

    $('.sc_scrollable', context).each(function()
    {
        SC.updateScroll(this);
    });
};
...
SC.Users = null;
SC_UsersApi = function( _settings )
{
    var _usersApi = this;
    ...
    var _query = function(command, params, callback) {
        callback = callback || function( r ) {
            if ( !r ) return;
            if (r.info) SC.info(r.info);
            if (r.error) SC.error(r.error);
        };
    ...
    this.deleteUser = function(userId, callBack, showMessage)
    {
        ...
        floatBox = SC.ajaxFloatBox('BASE_CMP_DeleteUser', [{userId: userId, showMessage : showMessage && redirectUrl}],
        {
            title: SC.getLanguageText('base', 'delete_user_confirmation_label'),
            iconClass: 'sc_ic_add',
            scope: {
    ...
    this.showUsers = function(userIds, title)
    {
    	title = title || SC.getLanguageText('base', 'ajax_floatbox_users_title');

    	SC.ajaxFloatBox('BASE_CMP_FloatboxUserList', [userIds], {iconClass: "sc_ic_user", title: title, width: 470});
    },
    ...
    this.blockUserWithConfirmation = function( userId, confirmationCallback, callback ) {
        var floatBox = SC.ajaxFloatBox("BASE_CMP_BlockUser", [userId], {
            scope: {
...
SC.postRequest = function(url, params)
{
    var $form = $("<form></form>")
        .attr({
            action: url,
...
SC.ResponsiveMenu = (function() {
    ...
    // Private methods
    function changed() {
        this.node[this.moreItems.length ? "addClass" : "removeClass"]("sc_main_menu_more_active");
    }
