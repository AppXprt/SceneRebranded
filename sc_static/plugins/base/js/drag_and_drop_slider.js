(function($){
	...
	$.fn.schemeSwitcher = function(options) {
		var $node = $(this);
		var $handle = $node.find('.sc_dnd_slider_handle');
		var $markers = $node.find(".sc_dnd_slider_marker");
		var $activeMarker = $node.find('.sc_dnd_slider_item .current');
		...
		$handle.draggable({
		    ...
		    start: function() {
				$(this).addClass('sc_dnd_slider_in_move');
			},
			stop: function() {
				$(this).removeClass('sc_dnd_slider_in_move');
				updateDelegate.apply($activeMarker.get(0));
			},
			helper: function(){
		        return $(this).clone(false).addClass('sc_dnd_slider_helper');
		    }
		});
		
		$markers.droppable({
		    tolerance: 'touch',
		    accept: '.sc_dnd_slider_handle',
			over: changeDelegate
		});
		...
		for (var i = 0; i < $markers.length; i++) {
			...
			var $pointer = $('<div class="sc_dnd_slider_marker_point"></div>');
			$node.append($pointer);
