SC_DataModel = function( data )
{
    this.data = data || {};
    this.observers = [];
};

SC_DataModel.PROTO = function()
{
...
SC_DataModel.prototype = new SC_DataModel.PROTO();


SC_Console = function( params, modelsData )
{
    ...
    {
        _models[key] = _models[key] || new SC_DataModel();
    ...
    var command = SC.getPing().addCommand('consoleUpdate',
    {
        before: function()
        {
            var c = this;
...
SC_ConsoleItem =
{
    ...
    showContent: function()
    {
        ...
        SC.Console.hideOtherContents(this);
        ...
        {
            self.animate = false;
            SC.addScrolls(self.$contentNode);
...
SC_ConsoleDropdownHover = function( uniqId, contentUniqId )
{
...
SC_ConsoleDropdownHover.prototype = SC_ConsoleItem;


SC_ConsoleDropdownClick = function( uniqId, contentUniqId )
{
    this.init( uniqId, contentUniqId );

    this.initDropdown();
}
$.extend(SC_ConsoleDropdownClick.prototype, SC_ConsoleItem,
{
    initDropdown: function()
    {
        ...
        this.addObserver({
           beforeShow: function()
           {
               self.$node.addClass('sc_console_dropdown_pressed');
           },

           beforeHide: function()
           {
               self.$node.removeClass('sc_console_dropdown_pressed');
           }
        });
...
SC_ConsoleDropdownList = function( uniqId, contentUniqId )
{
    this.init( uniqId, contentUniqId );
    this.initDropdown();

    var $counter = $('.SC_ConsoleItemCounter', this.$node),
        $number = $counter.find('.SC_ConsoleItemCounterNumber'),
        $place = $counter.find('.SC_ConsoleItemCounterPlace');
    ...
    var numberSetActive = function( active )
    {
        ...
        $place[ active ? 'addClass' : 'removeClass' ]('sc_count_active');
    }
...
SC_ConsoleDropdownList.prototype = SC_ConsoleDropdownClick.prototype;
...
SC_ConsoleList =
{
    construct: function( params )
    {
        ...
        this.rsp = params.rsp;
        this.key = params.key;
        this.data = SC.Console.getData(this.key);
        this.item = SC.Console.getItem(this.key);

        this.$container = function() {
            return $('.SC_ConsoleListContainer', self.$contentNode);
        };

        this.$list = function() {
            return $('.SC_ConsoleList', self.$container());
        };

        this.$preloader = function() {
            return $('.SC_ConsoleListPreloader', self.$container());
        };

        this.$noContent = function() {
            return $('.SC_ConsoleListNoContent', self.$container());
        };
        ...
        {
            SC.removeScroll(this.$container().get(0));
        };
    },
    ...
    _loadList: function()
    {
        ...
        var request = JSON.stringify({
            console: SC.Console.getData('console').get(),
            data: this.data.get(),
            target: target,
    ...
    addItems: function( items, updateScrooll )
    {
        ...
        $.each(items, function(i, item)
        {
            ...
            SC.trigger('base.onAddConsoleItem', [], $item);
        });
    ...
    updateScroll: function( toTop )
    {
        ...
        if ( this.opened )
        {
            ...
            var jsp = SC.addScroll(this.$container().get(0));
    ...
    _ajaxSuccess: function( resp )
    {
        ...
        if ( resp.markup )
        {
            ...
            {
                SC.addCss(resp.markup.styleDeclarations);
            }

            if (resp.markup.beforeIncludes)
            {
                SC.addScript(resp.markup.beforeIncludes);
            }

            if (resp.markup.scriptFiles)
            {
                SC.addScriptFiles(resp.markup.scriptFiles, function()
                {
                    if (resp.markup.onloadScript)
                    {
                        SC.addScript(resp.markup.onloadScript);
                    }
                });
            }
            else
            {
                if (resp.markup.onloadScript)
                {
                    SC.addScript(markup.onloadScript);
                }
            }
        }
...
SC_Invitation = function( itemKey, params )
{
    ...
    this.removeItem = function( invitationKey )
    {
        ...
        if ( item.hasClass('sc_console_new_message') )
        {
    ...
    model = SC.Console.getData(itemKey);
    list = SC.Console.getItem(itemKey);
    counter = new SC_DataModel();
    ...
    {
        list.getItems().removeClass('sc_console_new_message');
        counter.set('new', 0);
