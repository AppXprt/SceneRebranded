(function( window, $, params ) {'use strict';
    ...
    _elements = {
        preloader: $('<div>', {class: 'sc_floatbox_preloader sc_photo_preload'})
    },
    _methods = {
        isLoading: function()
        {
        ...
        cachePhotoComponents: function( photos )
        {
            ...
            keys.forEach(function( item )
            {
                if ( !_methods.isCached(item) )
                {
                    ...
                    SC.trigger('photo.afterPhotoCached', [cmp.photo.id, cmp], _methods);
                }
            });
        ...
        showFloatBox: function()
        {
            ...
            if ( _vars.isClassic )
            {
                options.addClass = 'sc_photoview_overlay_pint';
                options.$title = '';
            }
            else
            {
                options.addClass = 'sc_photoview_overlay floatbox_overlayBG';
                options.layout = 'empty';
            }

            _elements.photoFB = new SC_FloatBox(options);
            ...
            _elements.photoFB.bind('close', function()
            {
                ...
                SC.trigger('photo.onFloatboxClose', [], _methods);
            });
        },
        ...
        {
            SC.trigger('photo.onBeforeShow', [photoId, listType], _methods);
            
            if ( photoId !== _vars.photoId )
            {
                SC.trigger('photo.onBeforePhotoChange', [_vars.photoId, photoId], _methods);
            }
            ...
            if ( data && data.main && data.main.length === 2 && data.mainUrl )
            {
                ...
                else
                {
                    ...
                    _elements.content.find('.sc_photoview_info_wrap').height(floatboxCss.height);
                }
                
                _elements.photoFB.fitWindow(floatboxCss);
                _elements.content.find('.sc_photoview_stage_wrap').css(stageCss);
                _elements.content.find('img.sc_photo_view').css(imgCss).attr('src', data.mainUrl);
            }
        ...
        fetchCmp: function( dataToSend, params )
        {
            ...
            _vars.fetchRequest = $.ajax(
            {
                ...
                {
                    SC.trigger('photo.onBeforeLoadPhoto', [dataToSend.photoId], _methods);
                },
                success : function( responce )
                {
                    ...
                    SC.trigger('photo.onRequestComplete', [dataToSend, params, responce], _methods);
        ...
        {
            _methods.createFloatboxContent();
            SC.trigger('photo.onBeforeLoadFromCache', [photoId], _methods);
            ...
            {
                SC.trigger('photo.beforeResize', [photoId], _methods);
                ...
                if ( !_methods.fullscreen.isFullscreen() )
                {
                    if ( _vars.layout !== 'page' )
                    {
                        ...
                        if ( _vars.isClassic )
                        {
                            _elements.content.find('.sc_photoview_stage_wrap').css({float: 'none', height: _vars.minSize.height});
                            options.width = offset.width;
                        }
                        else
                        {
                            _elements.content.find('.sc_photoview_stage_wrap').css({width: _vars.minSize.width, height: _vars.minSize.height});
                ...
                SC.bindAutoClicks(_elements.content);
                SC.bindTips(_elements.content);

                SC.trigger('photo.afterResize', [photoId], _methods);
                SC.trigger('photo.photo_show', {photoId: photoId}, _methods);
        ...
        fitSize: function( photoId, callback, resize )
        {
            ...
            var complete = function()
            {
                ...
                {
                    var img = _elements.content.find('img.sc_photo_view').css({width: 'auto', height: 'auto'})[0];
        ...
        setContent: function( photoId )
        {
            ...
            $('.sc_user_list_data .sc_timestamp', content).html(cmp.photo.addDatetime);

            if ( cmp.contextAction.length )
            {
                $(cmp.contextAction).addClass('sc_photo_context_action').insertAfter($('.sc_photoview_arrsc_right', content)).show();
            }
            ...
            $("#photo-delete").off().on('click', function( event )
            {
                ...
                if ( confirm(SC.getLanguageText('admin', 'confirm_delete')) )
                {
                    $.ajax(
                    {
                        url: _vars.ajaxResponder,
            ...
            {
                _elements.content.find('.sc_photoview_fullscreen').attr({target: '_blank', href: cmp.photo.url});
            }

            $('.sc_photoview_title input', content).val(cmp.photo.title);
            $('.sc_photoview_url input', content).val(cmp.photo.url).attr('id', 'photo-target-' + photoId);
            $('.sc_photoview_url a', content).addClass('floatbox-clipboard-button');
            var randId = Math.random().toString(36).substring(7);
            $('.sc_photoview_url a', content).attr('id', randId);
            $('.sc_photoview_url a', content).unbind('click');
            document.getElementById(randId).setAttribute('data-clipboard-target', '#photo-target-' + photoId);
            $('.sc_photoview_date span', content).html(cmp.photo.addDatetime);
            $('.sc_photoview_size span', content).html(cmp.photo.dimensions);
            $('.sc_photoview_filesize span', content).html(cmp.photo.filesize);
            $('.sc_photoview_id', content).val(photoId);
        },
        setComment: function( photoId )
        {
            var cmp = _methods.getPhotoCmp(photoId);

            $('.sc_feed_comments', _elements.content).append(cmp.comment);

            _vars.commentEvents.forEach(function( item )
            {
                SC.bind(item, function( data )
                {
        ...
        updateComment: function( data, eventName )
        {
            ...
            var height, cmp = _methods.getPhotoCmp(_vars.photoId),
                scrollCont = _elements.content.find('.sc_photo_scroll_cont'),
                padding = parseInt(_elements.content.find('.sc_photoview_info').css('padding-top')),
                appendTo = _elements.content.find('.sc_feed_comments_input_sticky');
            ...
            {
                SC.addScroll(scrollCont, {autoReinitialise: true}).scrollToBottom();
                ...
                if ( _methods.fullscreen.isFullscreen() )
                {
                    height = _elements.content.find('.sc_photoview_info_wrap').height();
                }
                else
                {
                    height = _elements.content.find('.sc_photoview_info_wrap').height() - appendTo.height() - padding;
                }
                ...
                if ( scrollContHeight >= height )
                {
                    if ( !cmp.isMoved )
                    {
                        SC.trigger('base.move_comments_form', 
                        {
                        ...
                        cmp.isMoved = true;
                        _elements.content.find('.sc_photoview_info_wrap').addClass('sticked');
                    }

                    scrollCont.height(_elements.content.find('.sc_photoview_info_wrap').height() - appendTo.height() - padding);
                    jsp.reinitialise();
                    jsp.scrollToBottom();
                ...
                else
                {
                    ...
                    {
                        _elements.content.find('.sc_photo_scroll_cont').height(height);
                    }
                    else
                    {
                        if ( cmp.isMoved )
                        ...
                        SC.removeScroll(scrollCont);
                        _elements.content.find('.sc_photo_scroll_cont').css('height', 'auto');
                    }
                    
                    if ( eventName == 'base.comment_textarea_resize' )
                    {
                        $('textarea', _elements.content.find('.sc_photo_scroll_cont')).photoFocus();
                    }
                }
            }
            ...
            if ( jsp && ['base.comment_added', 'base.comment_textarea_resize'].indexOf(eventName) !== -1 )
            {
                ...
                _elements.content.find('.sc_feed_comments_input_sticky textarea').photoFocus();
            }
        ...
        loadPhoto: function( direction, listType )
        {
            ...
            SC.trigger('photo.onPhotoSwitch', [photoId, _vars.direction], _methods);
        ...
        addCmpMarkup: function( photoId )
        {
            ...
            if ( cmp.scriptFiles )
            {
                SC.addScriptFiles(cmp.scriptFiles, function()
                {
                    if ( cmp.onloadScript )
                    {
                        SC.addScript(cmp.onloadScript);
                    }
                });
            }
            else
        ...
        beforeLoadCmp: function( photoId )
        {
            if ( _methods.isCached(photoId) )
            {
                ...
                if ( cmp.isMoved )
                {
                    ...
                    {
                        SC.removeScroll(_elements.content.find('.sc_photo_scroll_cont'));
                    }
                    ...
                    if ( cmp.isMoved )
                    {
                        ...
                        cmp.isMoved = false;
                        _elements.content.find('.sc_photoview_info_wrap').removeClass('sticked');
                    }
                }
                
                SC.trigger('base.comments_destroy',
                {
                    entityType: 'photo_comments',
                    customId: cmp.customId,
                    entityId: photoId
                ...
                $('.sc_feed_comments', _elements.content).empty();
                _elements.content.find('.sc_photoview_info_wrap').height('');
                $('.sc_rates_wrap', _elements.content).hide();
                ...
                $('.sc_photo_context_action', _elements.content).remove();
            }
        ...
        fullscreen: {
            ...
            onchange: function( event )
            {
                ...
                if ( _methods.fullscreen.isFullscreen() )
                {
                    ...
                    content.height('');
                    content.find('.sc_photoview_stage_wrap').addClass('sc_photoview_stage_wrap_fullscreen');
                    content.find('.sc_photoview_info_wrap').css({height: screen.height});
                    content.find('.sc_photoview_play_btn').removeClass('stop');
                    
                    if ( _vars.layout === 'page' || _vars.isClassic )
                    {
                        $(document.body).addClass('sc_photo_page_fullscreen');
                        content.find('.sc_photoview_stage_wrap').css({float: 'left', width: screen.width, height: screen.height});
                        content.find('.sc_photoview_info').removeClass('sc_photoview_info_onpage');
                    }
                    else
                    {
                        _elements.photoFB.fitWindow({top: '0', width: screen.width, height: screen.height});
                        content.find('.sc_photoview_stage_wrap').css({width: screen.width, height: screen.height});
                    }
                    ...
                    if ( cmp.photo.hasFullsize && cmp.photo.dimension )
                    {
                        try
                        {
                            ...
                            if ( dimension.fullscreen && dimension.fullscreen.length === 2 )
                            {
                                ...
                                _elements.content.find('img.sc_photo_view').css(css).attr('src', cmp.photo.url);
                ...
                else
                {
                    ...
                    content.find('.sc_photoview_info_btn').removeClass('close').addClass('open');
                    content.find('.sc_photoview_stage_wrap').removeClass('sc_photoview_stage_wrap_fullscreen');
                    content.find('.sc_photoview_info_wrap').css({height: ''});
                    content.find('.sc_photoview_play_btn').addClass('stop');
                    
                    if ( _vars.layout === 'page' || _vars.isClassic )
                    {
                        _elements.content.find('.sc_photoview_stage_wrap').css({float: 'none'});
                        _elements.content.find('.sc_photoview_info').addClass('sc_photoview_info_onpage');
                        $(document.body).removeClass('sc_photo_page_fullscreen');
                    }
                    
                    if ( _vars.isClassic )
                    {
                        _elements.content.find('.sc_photoview_info').addClass('sc_photoview_pint_mode');
                    }
                    ...
                    setTimeout(function()
                    {
                        ...
                        content.find('.sc_photoview_stage_wrap').css(css);
                        _methods.updateComment({entityType: 'photo_comments', entityId: _vars.photoId});
                ...
                content.find('.sc_photoview_info_wrap').height('');
                ...
                SC.trigger('photo.onSizeChange', [_vars.photoId, _methods.fullscreen.isFullscreen()], _methods);
            }
        },
        utils:
        {
        ...
        arrayUtils:
        {
            unionToList: function()
            {
                ...
                var event = {listType: []};
                SC.trigger('photo.collectListType', event, _methods);
        ...
        createFloatboxContent: function()
        {
            ...
            _elements.content.find('.sc_photoview_slide_time').slider({min: 3, max:10, range: 'min',
                slide: function( event, ui )
                {
                    var tip = $(this).data('owTip');
                    
                    tip.find('.sc_tip_title').html(SC.getLanguageText('photo', 'slideshsc_interval') + ui.value);
                    _vars.interval = ui.value * 1000;
                    _methods.updateSlideshow();
                }
            });
            SC.trigger('photo.onAfterContentRender', [_elements.content], _methods);
        ...
        resizeWindow: function(event)
        {
            ...
            _vars.timerId = setTimeout(function()
            {
                ...
                {
                    SC.trigger('photo.beforeResize', [_vars.photoId], _methods);
                    ...
                    if ( _vars.layout !== 'page' )
                    {
                        ...
                        if ( _vars.isClassic )
                        {
                            _elements.content.find('.sc_photoview_stage_wrap').css({float: 'none', height: _vars.minSize.height, width: ''});
                            options.width = offset.width;
                        }
                        else
                        {
                    ...
                    {
                        _elements.content.find('.sc_photoview_stage_wrap').css({height: offset.height, width: offset.width});
                    }
                    
                    _methods.updateComment({entityType :'photo_comments', entityId: _vars.photoId});
                    
                    SC.trigger('photo.afterResize', [_vars.photoId], _methods);
                }, true);
            }, 100);
        },
        runSlideshow: function()
        {
            ...
            _vars.slideshowTimeId = setInterval(function()
            {
                ...
                else
                {
                    ...
                    var img = _elements.content.find('img.sc_photo_view');
        ...
        init: function()
        {
            ...
            if ( _methods.fullscreen.enabled() )
            {
                $('.sc_photoview_fullscreen', content).on('click', function()
                {
                    _methods.fullscreen.isFullscreen() ? _methods.fullscreen.exit() : _methods.fullscreen.request();
                });
                
                $('.sc_photoview_info_btn', content).on('click', function( event )
                {
                    if ( !_methods.fullscreen.isFullscreen() )
                    {
                    ...
                    if ( $(this).hasClass('open') )
                    {
                        ...
                        var wrap = _elements.content.find('.sc_photoview_stage_wrap');
                        ...
                        {
                            wrap.css('float', 'left');
                            _elements.content.find('.sc_photoview_info').removeClass('sc_photoview_pint_mode');
                        }
                        else
                        {
                        ...
                        SC.trigger('photo.afterInfoShow', [_vars.photoId], _methods);
                    }
                    else
                    {
                        $(this).removeClass('close').addClass('open');
                        _elements.content.find('.sc_photoview_stage_wrap').width(screen.width);
                        ...
                        SC.trigger('photo.afterInfoHide', [_vars.photoId], _methods);
                    }
                ...
                $('.sc_photoview_slide_settings', content).on({
                    mouseenter: function()
                    {
                        if ( timerId )
                        {
                        ...
                        $('.sc_photoview_slide_settings_controls', _elements.content).stop(true).css({display: 'inline-block', opacity: 1});
                    }
                });
                $('.sc_photoview_bottom_menu_wrap', content).on({
                    mouseleave: function()
                    {
                        if ( timerId )
                        {
                        ...
                        {
                            $('.sc_photoview_slide_settings_controls', _elements.content).fadeOut(1000);
                        }, 5000);
                    }
                });
                
                $('.sc_photoview_slide_settings_effect', content).on('click', function()
                {
                    $('.sc_photoview_slide_settings_effect', _elements.content).removeClass('active');
                    _vars.effect = $(this).addClass('active').attr('effect');
                    _methods.updateSlideshow();
                });
                
                $('.sc_photoview_play_btn', _elements.content).on('click', function( event )
                {
                    if ( !_methods.fullscreen.isFullscreen() )
                    {
                        event.stopImmediatePropagation();
            ...
            SC.bind('photo.onAfterEditPhoto', function( photoId )
            {
                _methods.deletePhotoCmp(photoId);
            });
            SC.bind('photo.onChangeListType', function( listType )
            {
            ...
            $('.sc_photoview_stage_wrap,img.sc_photo_img', content).on('click', function( event )
            {
            ...
            $('.sc_photoview_arrsc_left', content).on('click', _methods.loadPrevPhoto);
            $('.sc_photoview_arrsc_right', content).on('click', _methods.loadNextPhoto);
