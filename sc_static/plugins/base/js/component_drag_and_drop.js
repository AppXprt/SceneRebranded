...
componentDragAndDrop.prototype = {
	...
	prepare: function()
		...
		this.defaultSettings = {
	        ...
	        beforeStop: function(e, ui) {
	        	if (ui.item.parent().hasClass('sc_dnd_locked_section'))
	        	{
	...
	initializePanel: function() {
        ...
        var panelSettings = this.extendSettings({
            ...
            over: function(e, ui) {
                ui.placeholder.removeClass('sc_dnd_placeholder');
    ...
    initializeSections: function() {
        ...
        var sectionSettings = this.extendSettings({
            cancel: '.sc_dnd_freezed',
            items: '.component:not(.sc_dnd_freezed)',
            receive: function(e, ui){
                if (ui.placeholder.hasClass('sc_dnd_placeholder')) {
            ...
            over: function(e, ui){
		        ...
		        ui.placeholder.addClass('sc_dnd_placeholder');
		    },
    ...
    initializeActions: function() {
        ...
        this.$panel.find('.dd_delete').unbind().click(function() {
           if ( !confirm( SC.getLanguageText('base', 'widgets_delete_component_confirm') ) ) {
               return false;
           }
    ...
    initializeScheme: function() {
        ...
        $sliderNode.schemeSwitcher({
	    	...
	    	update: function(event){
	    		var schemeId = $(event.marker).attr('sc_scheme');
    ...
    getHelper: function(sortable, e, ui) {
        ...
        return $('<div class="sc_dnd_helper" style="width: 160px; height: 30px"></div>');
    },
    ...
    handlStart: function(sortable, e, ui) {
        ...
        var within = $(ui.item).attr('sc_avaliable_sections');
    ...
    handleStop: function(sortable, e, ui)
    {
        ...
        event.to = {
            section: toSection,
            sectionName: toSection.attr('sc_place_section')
        };
        event.from = {
            section: fromSection,
            sectionName: fromSection.attr('sc_place_section')
        };
        event.ui = ui;
    ...
    handleSectionChange: function(sortable, e, ui)
    {
        ...
        if ( $ph.next().is(cmpNode) || $ph.prev().is(cmpNode)	) {
                $ph.removeClass('sc_dnd_placeholder');
                $ph.addClass('hidden-placeholder');
        }
        else {
                $ph.removeClass('hidden-placeholder');
                $ph.addClass('sc_dnd_placeholder');
        }
    },
    ...
    handleRemoveFromSection: function($item, completeFnc) {
        ...
        $item.removeClass('sc_dnd_freezed');
    ...
    disableSections: function($sections) {
    	$.each($sections, function(i, item) {
    		$(item).addClass('sc_dnd_locked_section');
    	});
    },

    enableSections: function($sections) {
    	$.each($sections, function(i, item){
    		$(item).removeClass('sc_dnd_locked_section');
    	});
    },
    ...
    clone: function(sortable, $cloningItem) {
        ...
        var section = $(sortable).attr('sc_place_section');
    ...
    update: function(sortable) {
        ...
        var section = $selfNode.attr('sc_place_section');
        var stack = [];
    ...
    showSettings: function($component)
    {
        ...
        var $content = $('.settings_content', '#fb_settings').addClass('sc_preloader_content');

        var $controls = $('.settings_controls', '#fb_settings');
        $controls.find('input.dd_save').unbind();

        this.settingBox = new SC_FloatBox({
            $title: $title,
            $contents: $content,
            $controls: $controls,
            width: 500
        ...
        this.trigger('loadSettings', [cmpId, function(settingMarkup){
                $content.empty();
                $content.removeClass('sc_preloader_content');
                ...
                if (settingMarkup.styleDeclarations) {
                    SC.addCss(settingMarkup.styleDeclarations)
                }

                if (settingMarkup.scriptFiles) {
                    SC.addScriptFiles(settingMarkup.scriptFiles, function(){
                        if (settingMarkup.onloadScript) {
                            SC.addScript(settingMarkup.onloadScript);
                        }
                    });
                }
                else
    ...
    saveSettings: function(cmpId, form, submitControl) {
        ...
        SC.inProgressNode(submitControl);
    ...
    applyComponentSettings: function(cmpId, settings) {
        ...
        if (settings.freeze > 0) {
            $component.addClass('sc_dnd_freezed');
    ...
    arrangeFreezed: function(section) {
    	...
    	/*var components = $section.find('.component').sort(function(a, b){
    		var x = $(a).is('.sc_dnd_freezed') ? 0 : 1;
    		var y = $(b).is('.sc_dnd_freezed') ? 0 : 1;
    	...
    	var components = [];
    	$section.find('.sc_dnd_freezed').each(iteration);
    	$section.find('.component:not(.sc_dnd_freezed)').each(iteration);
