(function( window, $ ) {'use strict';
    ...
    _methods = {
        ...
        updateSlot: function( slotId, fileUrl, id, filename )
        {
            ...
            scForms['ajax-upload'].addElement(new ScFormElement(rotateId, rotateId));
            var descriptionElement = new ScFormElement(descId, descId);
            descriptionElement.setValue(filename);
            scForms['ajax-upload'].addElement(descriptionElement);

            slot.find('.sc_photo_preview_x').on('click', function()
            {
                _methods.destroySlot(slotId, id);
            });
            slot.find('.sc_photo_preview_rotate').on('click', function()
            {
                var photo = slot.find('.sc_photo_preview_image'), _rotate;
            ...
            img.onload = function()
            {
                slot.find('.sc_photo_preview_image')
                    .hide(0, function()
                    {
                        this.style.backgroundImage = 'url(' + img.src + ')';
                        $(this).removeClass('sc_photo_preview_loading').fadeIn(300);
                        
                        SC.trigger('photo.onRenderUploadSlot', [_elements.descEditors[slotId]], slot);
                    });
            };
            img.src = fileUrl;
        },
        initHashtagEditor: function( slotId )
        {
            ...
            editor.setValue(SC.getLanguageText('photo', 'describe_photo'));
            editor.on('blur', function( editor )
            {
                var value = editor.getValue().trim(), lineCount;
                
                if ( value.length === 0 || value === SC.getLanguageText('photo', 'describe_photo') )
                {
                ...
                slot.find('.sc_photo_preview_image').removeClass('sc_photo_preview_image_active');
                
                if ( _elements.slotArea.find('.sc_photo_preview_image_active').length === 0 )
                {
                    _elements.slotArea.removeClass('sc_photo_preview_image_filtered');
                }
            });
            editor.on('focus', function( editor )
            {
                ...
                else
                {
                    ...
                    if ( value === SC.getLanguageText('photo', 'describe_photo') )
                    {
                        editor.setValue('');
                    }
                }
                ...
                _elements.slotArea.addClass('sc_photo_preview_image_filtered');
                slot.find('.sc_photo_preview_image').addClass('sc_photo_preview_image_active');
            });
        ...
        uploadFile: function( file )
        {
            ...
            if ( _vars.isHTML5 )
            {
                ...
                else
                {
                    ...
                    {
                        SC.error(SC.getLanguageText('photo', 'size_limit', {name: file.name, size: (_vars.maxFileSize / 1048576)}));
                    }
                    else
                    {
                        SC.error(SC.getLanguageText('photo', 'type_error', {name: file.name}));
                    }
            ...
            else
            {
                if ( file.search(/\.(?:jpe?g|png|gif)$/i) !== -1 )
                {
                    ...
                    _elements.dropArea.off('click').on('click', function(){alert(SC.getLanguageText('photo', 'please_wait'))});
                ...
                {
                    SC.error(SC.getLanguageText('photo', 'type_error', {name: file}));
        ...
        requestSuccess: function( jsonStr, slotId )
        {
            ...
            {
                _methods.destroySlot(slotId);
                SC.error(SC.getLanguageText('photo', 'not_all_photos_uploaded'));
            }
        },
        showAlbumList: function()
        {
            _elements.albumList.show();
            $('.upload_photo_spinner', _elements.albumForm).removeClass('sc_dropdown_arrsc_down').addClass('sc_dropdown_arrsc_up');
        },
        hideAlbumList: function()
        {
            _elements.albumList.hide();
            $('.upload_photo_spinner', _elements.albumForm).removeClass('sc_dropdown_arrsc_up').addClass('sc_dropdown_arrsc_down');
        },
        setIsRuning: function()
        {
            _vars.isRuning = true;
            SC.inProgressNode($(':submit', scForms['ajax-upload'].form));
        },
        afterUploadTask: function()
        {
            ...
            else
            {
                ...
                SC.activateNode($(':submit', scForms['ajax-upload'].form));
            }
        }
    };
    
    var _a = $('<a>', {class: 'sc_hidden sc_content a'}).appendTo(document.body);
    SC.addCss('.cm-hashtag{cursor:pointer;color:' + _a.css('color') + '}');
    _a.remove();
    ...
    window.ajaxFileUploader = Object.defineProperties({}, {
        init: { value: function()
        {
            ...
            if ( !_vars.isHTML5 )
            {
                _elements.dropAreaLabel.html(SC.getLanguageText('photo', 'dnd_not_support'));
            }
                ...
                (function()
                {
                    ...
                    if ( _vars.isHTML5 )
                    {
                        eventMap.drop = function( event )
                        {
                            ...
                            _elements.dropAreaLabel.html(SC.getLanguageText('photo', 'dnd_support'));
                        ...
                        eventMap.dragenter = function()
                        {
                            ...
                            _elements.dropAreaLabel.html(SC.getLanguageText('photo', 'drop_here'));
                        };
            ...
            _elements.albumList = $('.sc_dropdown_list', _elements.albumForm);
            ...
            {
                _methods.hideAlbumList();
                scForms['ajax-upload'].removeErrors();
            })
            .eq(0).on('click', function()
            {
                $('.new-album', _elements.albumForm).show();
                _elements.albumInput.val(SC.getLanguageText('photo', 'create_album'));
                $('input[name="album-name"]', _elements.albumForm).val(SC.getLanguageText('photo', 'album_name'));
                $('textarea', _elements.albumForm).val(SC.getLanguageText('photo', 'album_desc'));
            })
            ...
            SC.bind('photo.onCloseUploaderFloatBox', function()
            {
