(function( $ ) {'use strict';
        ...
        _methods = {
            ...
            removePhotoItems: function( idList )
            {
                ...
                if ( result )
                {
                    ...
                    SC.trigger('photo.onRemovePhotoItems', iDs);
                }
            ...
            buildPhotoList: function( data )
            {
                if ( data.photoList.length === 0 )
                {
                    ...
                    {
                        _elements.content.append($('<div>', {style: 'text-align:center; padding-top: 24px;'}).html(SC.getLanguageText('admin', 'no_items')));
                    }
            ...
            buildPhotoItem: function( photo )
            {
                ...
                SC.trigger('photo.onRenderPhotoItem', [$.extend({}, photo)], photoItem);

                _methods.buildPhotoItem.buildByMode(photoItem, photo);
                SC.trigger('photo.photoItemRendered', photoItem);
            },
            ...
            reorder: function()
            {
                ...
                $('.sc_photo_item_wrap', _elements.content).each(function()
                {
            ...
            {
                this.find('.sc_photo_item_info_title span').html(entity.title);
                this.find('.sc_photo_item_info_dimensions span').html(entity.dimensions);
                this.find('.sc_photo_item_info_fileszie span').html(entity.filesize);
                this.find('.sc_photo_item_info_uploaddate span').html(entity.addDatetime);
                var link = $('<a></a>', {
                    href: "javascript://",
                    class: 'clipboard-button',
                    'data-clipboard-text': entity.url,
                    'text': SC.getLanguageText('admin', 'copy_url')

                });
                this.find('.sc_photo_item_info_url').append(link);
            },
    ...
    _methods.buildPhotoItem.buildByMode = (function()
    {
        if ( _vars.classicMode )
        {
            ...
            {
                photoItem.find('.sc_photo_item')[0].style.backgroundImage = 'url(' + photo.url + ')';
                photoItem.find('img.sc_hidden').attr('src', photo.url);
    ...
    window.browsePhoto = Object.defineProperties({},
        {
            init: {
                value: function()
                {
                    ...
                    SC.bind('photo.onSetRate', _methods.updateRate);
            ...
            updateSlot: {
                ...
                {
                    SC.trigger('photo.onUpdateSlot', [slotId, data]);
...
(function( root, $ )
{
    ...
    var _methods = {
        ...
        deleteImages: function( imagesIds )
        {
            ...
            {
                SC.error(SC.getLanguageText('admin', 'no_photo_selected'));
                return false;
            }
            if ( !confirm(SC.getLanguageText('admin', 'confirm_delete_images')) )
            {
                return false;
            }
        ...
        call: function( event )
        {
            var closest = $(this).closest('.sc_photo_item_wrap');
        ...
        init: function()
        {
            ...
            var event = {buttons:[]};
            SC.trigger('photo.collectMenuItems', [event]);
            ...
            var list = $('<ul>', {class: 'sc_context_action_list'});
            ...
            _params.contextAction = $('<div>', {class: 'sc_photo_context_action'}).on('click', function( event )
            {
                event.stopImmediatePropagation();
            });
            _params.contextActionPrototype = $(document.getElementById('context-action-prototype')).removeAttr('id');
            _params.contextActionPrototype.find('.sc_tooltip_body').append(list);

            SC.bind('photo.onRenderPhotoItem', function( album )
            {
                var self = $(this);
                ...
                if ( _params.listType == 'albums' && album.name.trim() == SC.getLanguageText('photo', 'newsfeed_album').trim() )
                {
                ...
                {
                    self.find('.sc_photo_item').prepend(contextAction);
                }
                else
                {
                    self.find('.sc_photo_pint_album').append(contextAction);
                }
            });
        }
    };
