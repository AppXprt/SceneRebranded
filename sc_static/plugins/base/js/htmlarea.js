    ...
    jHtmlArea.fn = jHtmlArea.prototype = {
        ...
        init: function(elem, options) {
            ...
            if (elem.nodeName.toLowerCase() === "textarea") {
                ...
                var container = this.container = $("<div/>").addClass("jhtmlarea").width('95%').addClass('sc_smallmargin').insertAfter(textarea);
                ...
                SC.trigger('base.initjHtmlArea', [], this);
            }
        },
        dispose: function() {
        ...
        insertVideo: function(params){
            ...
            if( !params || !params.code ){
                SC.error(window.htmlAreaData.labels.messages.videoEmptyField);
                return;
            }
            $html = $('<div><span class="sc_ws_video"></span></div>');
        ...
        addHtml: function(params){
            ...
            $html = $('<div><span class="sc_ws_html"></span></div>');
            $('span', $html).append(params.code);
            this.pasteHTML($html.html());
            this.tempFB.close();
        },
        ...
        insertLink: function(params){
            ...
            if( !params || !params.url || !params.label ){
                SC.error(window.htmlAreaData.labels.messages.linkEmptyFields);
                return;
            }

            this.pasteHTML('<span class="sc_ws_link"><a rel="nofollow" href="'+params.url+'"'+ ( params.newWindow ? ' target="_blank"' : '') +'>'+params.label+'</a></span>');
            this.tempFB.close();
        },
            ...
            $.each(this.toolbarArray,
                function( key, $item ){
                    if( $item.toolbarHandler.call(self) ){
                        $item.addClass('sc_ws_active');
                    }
                    else{
                        $item.removeClass('sc_ws_active');
                    }
                }
                );
        },
