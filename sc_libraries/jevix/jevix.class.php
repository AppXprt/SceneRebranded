...
//            }
            ...
            {
                if ( in_array('*', $this->commonTagParamRules) || !empty($tagRules[self::TR_PARAM_ALLSCED]['*']) || isset($tagRules[self::TR_PARAM_ALLSCED][$param]) || in_array($param, $this->commonTagParamRules) )
                {
                    continue;
                }
            }
    ...
    protected function text( &$text )
    {
        ...
        while ( ($this->curCh != '<') && $this->curChClass )
        {
            ...
            elseif ( $this->isAutoLinkMode && ($this->curChClass & self::LAT) && $this->openedTag != 'a' && $this->url($url, $href) )
            {
                ...
                $text.= $this->makeTag('a', array('href' => $href, 'class' => 'sc_autolink', 'target' => '_blank', 'rel' => 'nofollow'), $linkLabel, false);
                $newWord = true;
            }
