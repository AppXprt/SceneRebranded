 ...
 * @package sc_core
 * @method static SC_Response getInstance()
 * @since 1.0
 */
class SC_Response
{
    ...
    /**
     * @param SC_Document $document
     */
    public function setDocument( SC_Document $document )
    {
    ...
    {
        $event = new SC_Event(SC_EventManager::ON_BEFORE_DOCUMENT_RENDER);
        ...
        if ( $this->document !== null )
        {
            ...
            SC::getEventManager()->trigger($event);
            $data = $event->getData();
            $this->markup = str_replace(SC_Document::APPEND_PLACEHOLDER, PHP_EOL . implode(PHP_EOL, $data), $renderedMarkup);
        }

        $event = new SC_Event(SC_EventManager::ON_AFTER_DOCUMENT_RENDER);
        SC::getEventManager()->trigger($event);
