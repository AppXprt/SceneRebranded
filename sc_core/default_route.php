...
//    }
    ...
    public function generateUri( $controller, $action = null, array $params = array() )
    {
        ...
        $className = str_replace(SC::getAutoloader()->getPackagePointer($controller) . '_', '', $controller);

        $plugin = SC::getPluginManager()->getPlugin(SC::getAutoloader()->getPluginKey($controller));
    ...
    {//TODO check if method is in try/catch
        ...
        for ( $i = 0; $i < $arraySize; $i++ )
        {
            if ( $i === 0 )
            {
                try
                {
                    $classPrefix = strtoupper(SC::getPluginManager()->getPluginKey($uriArray[$i])) . '_' . $controllerNamePrefixAdd . $this->controllerNamePrefix;
                }
            ...
            if ( $i === 1 )
            {
                ...
                if ( !file_exists(SC::getAutoloader()->getClassPath($ctrClass)) )
                {
                    throw new Redirect404Exception('Invalid uri was provided for routing!');
                }
