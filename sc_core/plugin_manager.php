 ...
 * @package ow.sc_core
 * @method static SC_PluginManager getInstance()
 * @since 1.0
 */
class SC_PluginManager
{
    use SC_Singleton;
    ...
    public function getPlugin( $key )
    {
        ...
        {
            $this->cachedObjects[$plugin->getKey()] = new SC_Plugin($plugin);
        }
     ...
     */
    public function initPlugin( SC_Plugin $pluginObject )
    {
        $this->addPackagePointers($pluginObject->getDto());

        $initDirPath = $pluginObject->getRootDir();

        if ( SC::getApplication()->getContext() == SC::CONTEXT_MOBILE )
        {
            $initDirPath = $pluginObject->getMobileDir();
        }
        if ( SC::getApplication()->getContext() == SC::CONTEXT_CLI )
        {
            $initDirPath = $pluginObject->getCliDir();
        }
        else if ( SC::getApplication()->getContext() == SC::CONTEXT_API )
        {
