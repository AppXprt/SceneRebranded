 ...
 * @package sc_core
 * @method static SC_RequestHandler getInstance()
 * @since 1.0
 */
class SC_RequestHandler
{
    ...
    {
        if ( empty($attributes[SC_Route::DISPATCH_ATTRS_CTRL]) )
        {
            throw new Redirect404Exception();
        }

        $this->handlerAttributes = array(
            self::ATTRS_KEY_CTRL => trim($attributes[SC_Route::DISPATCH_ATTRS_CTRL]),
            self::ATTRS_KEY_ACTION => ( empty($attributes[SC_Route::DISPATCH_ATTRS_ACTION]) ? null : trim($attributes[SC_Route::DISPATCH_ATTRS_ACTION]) ),
            self::ATTRS_KEY_VARLIST => ( empty($attributes[SC_Route::DISPATCH_ATTRS_VARLIST]) ? array() : $attributes[SC_Route::DISPATCH_ATTRS_VARLIST])
        );
    }
    ...
    public function dispatch()
    {
        ...
        {
            SC::getRequest()->setUriParams($this->handlerAttributes[self::ATTRS_KEY_VARLIST]);
        }
        ...
        /* @var $controller SC_ActionController */
        try
        {
            $controller = SC::getClassInstance($this->handlerAttributes[self::ATTRS_KEY_CTRL]);
    ...
    {
        return $controller != null & $controller instanceof SC_ActionController;
    }

    /**
     * @param ReflectionMethod $action
     * @param SC_ActionController $controller
     */
    protected function processControllerAction( $action, $controller )
    {
        ...
        SC::getEventManager()->trigger(new SC_Event("core.performance_test",
        ...
        SC::getDocument()->setBody($controller->render());
    }

    /**
     * Returns template path for provided controller and action.
     *
     * @param SC_ActionController $ctrl
     * @return string
     */
    protected function getControllerActionDefaultTemplate( SC_ActionController $ctrl )
    {
        $plugin = SC::getPluginManager()->getPlugin(SC::getAutoloader()->getPluginKey($this->handlerAttributes[self::ATTRS_KEY_CTRL]));

        $templateFilename = SC::getAutoloader()->classToFilename($this->handlerAttributes[self::ATTRS_KEY_CTRL], false) . '_'
            . SC::getAutoloader()->classToFilename(ucfirst($this->handlerAttributes[self::ATTRS_KEY_ACTION]), false) . '.html';

        return ( $ctrl instanceof SC_MobileActionController ? $plugin->getMobileCtrlViewDir() : $plugin->getCtrlViewDir() ) . $templateFilename;
    }
        ...
        if ( $catchRequest )
        {
            if ( isset($this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_REDIRECT]) && (bool) $this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_REDIRECT] )
            {
                ...
                $redirectUrl = ($route !== null) ?
                    SC::getRouter()->urlForRoute($route, $params) :
                    SC::getRouter()->urlFor($this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_CTRL],
                        $this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_ACTION], $params);

                $redirectUrl = SC::getRequest()->buildUrlQueryString($redirectUrl,
                    array('back_uri' => SC::getRequest()->getRequestUri()));
                ...
                if ( isset($this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_JS]) && (bool) $this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_JS] )
                {
                    ...
                    if ( array_key_exists('base.members_only', $this->catchAllRequestsAttributes) )
                    {
                        ...
                        {
                            $backUri = isset($_GET['back_uri']) ? $_GET['back_uri'] : SC::getRequest()->getRequestUri();
                            SC::getDocument()->addOnloadScript("window.location = '" . SC::getRequest()->buildUrlQueryString($redirectUrl,
                                    array('back_uri' => $backUri)) . "'");
                            return null;
                        }
                        else
                        {
                            $ru = SC::getRequest()->buildUrlQueryString(SC::getRouter()->urlForRoute('static_sign_in'),
                                array('back_uri' => SC::getRequest()->getRequestUri()));
                            SC::getApplication()->redirect($ru);
                        }
                    }

                    SC::getDocument()->addOnloadScript("window.location = '" . $redirectUrl . "'");
                    return null;
                }
