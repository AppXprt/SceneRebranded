...
abstract class FormElement
{
    ...
    public function addFilter( SC_IFilter $filter )
    {
        $this->filters[] = $filter;
    }
    ...
    {
        if ( !$validator instanceof SC_Validator )
        {
    ...
    {
        $jsString = "var formElement = new ScFormElement(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ");";
...
abstract class InvitationFormElement extends FormElement
{
    ...
    public function __construct( $name )
    {
        ...
        $this->setInvitation(SC::getLanguage()->text('base', 'form_element_common_invitation_text'));
    }
...
class TextField extends InvitationFormElement
{
    ...
    public function getElementJs()
    {
        $jsString = "var formElement = new ScTextField(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ");";
...
class DateField extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        SC::getDocument()->addScript(SC::getPluginManager()->getPlugin("base")->getStaticJsUrl() . 'date_field.js');

        SC::getDocument()->addOnloadScript(" if( window.date_field == undefined ) { window.date_field = {}; } window.date_field['" . $this->getId() . "'] = new DateField( '" . ( $this->getName() ) . "' ); ");
        ...
        $config = SC::getConfig()->getValue('base', 'date_field_format');
        ...
        if ( $config === 'dmy' )
        {
            $result = '<div class="' . $this->getAttribute('name') . '">
                            ...
                            <div class="sc_inline owm_inline">' . UTIL_HtmlTag::generateTag('select', $dayAttributes, true, $dayOptionsString) . '</div>
                            <div class="sc_inline owm_inline">' . UTIL_HtmlTag::generateTag('select', $monthAttributes, true, $mounthOptionsString) . '</div>
                            <div class="sc_inline owm_inline">' . UTIL_HtmlTag::generateTag('select', $yearAttributes, true, $yearOptionsString) . '</div>
                        </div>';
        }
        else
        {
...
class Textarea extends InvitationFormElement
{
    ...
    public function getElementJs()
    {
        $jsString = "var formElement = new ScTextArea(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ");";
...
class Submit extends FormElement
{
    ...
    public function __construct( $name, $decorator = 'button' )
    {
        ...
        $this->setValue(SC::getLanguage()->text('base', 'form_element_submit_default_value'));
    ...
    public function renderInput( $params = null )
    {
        ...
        {
            $finalMarkup = SC::getThemeManager()->processDecorator($this->decorator, $params);
        }
        else
        {
...
class RadioField extends FormElement
{
    ...
    public function getElementJs()
    {
        $js = "var formElement = new ScRadioField(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ");";
    ...
    public function renderInput( $params = null )
    {
        ...
        $renderedString = '<ul class="sc_radio_group clearfix">';
...
//TODO rename getter or setter
    ...
    public function getElementJs()
    {
        $js = "var formElement = new ScCheckboxGroup(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ");";
    ...
    public function renderInput( $params = null )
    {
        ...
        $renderedString = '<ul class="sc_checkbox_group clearfix">';
...
class Selectbox extends InvitationFormElement
{
    ...
    public function __construct( $name )
    {
        ...
        $this->setInvitation(SC::getLanguage()->text('base', 'form_element_select_field_invitation_label'));
...
class CheckboxField extends FormElement
{
    ...
    public function getElementJs()
    {
        $jsString = "var formElement = new ScCheckboxField(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ");";
...
class Multiselect extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        return
            '<table class="sc_multiselect" id="' . $this->getId() . '">
                <tr>
                    <td>' . $chicesSelectMarkup . '</td>
                    <td><input type="button" class="select" value="" /><br /><br /><br /><input type="button" class="deselect sc_ic_left_arrow" value="" /></td>
    ...
    public function getElementJs()
    {
        ...
        SC::getDocument()->addScript(SC::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'multiselect_field.js');

        $jsString = "var formElement1 = new ScFormElement('" . $this->getId() . "', '" . $this->getName() . "');
...
class SuggestField extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        {
            $this->addAttribute('value', $this->value);
            $this->addAttribute('class', 'sc_inputready');
        }

        $this->addAttribute('type', 'text');

        $scriptUrl = SC::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'jquery-suggest.js';
        SC::getDocument()->addScript($scriptUrl);

        $js = '
            var $input = $("#' . $this->getId() . '");
            var initialLabel = $input.val();
            $input.parent().find(".sc_suggest_invitation").click(function(){ $input.focus(); });
            $input.suggest(' . $this->responderUrl . ', {autoSuggest: true, minchars: ' . $this->minChars . ', onFocus: function(first){
                    $(this).removeClass("sc_inputready");
        ...
        return '<div class="sc_suggest_field">' . UTIL_HtmlTag::generateTag('input', $this->attributes) . '<div class="sc_suggest_invitation"></div></div>';
    }
}
...
class MultiFileField extends FormElement
{
    ...
    public function getElementJs()
    {
        $js = "var formElement = new ScFormElement('" . $this->getId() . "', '" . $this->getName() . "');";
...
class TagsField extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        $tagTpl = '<div class="tags-field-tag-cont" style="margin: 2px 2px 0pt 0pt; padding: 2px 5px; background: rgb(204, 204, 204) none repeat scroll 0% 0%; float: left;"><span class="tags-field-tag">${tag}</span> <a class="sc_lbutton sc_red tags-field-del-tag">x</a></div>';
        ...
        $tpl = '<span id=${id}>' . '${hidden}' . '<input class="sc_text" type="text">' . '${tags}' . '</span>';
        ...
        SC::getDocument()->addScript($plugin->getStaticJsUrl() . 'tags_field.js');
...
{
    const CAPTCHA_PREFIX = 'sc_captcha_';
    ...
    public function renderInput( $params = null )
    {
        ...
        $captchaUrl = SC_URL_HOME . 'captcha.php';
        $captchaResponderUrl = SC::getRouter()->urlFor('BASE_CTRL_Captcha', 'ajaxResponder');
        $captchaClass = $this->getName() . '_' . $this->getId();
        $uniqueId = md5(time());

        $jsDir = SC::getPluginManager()->getPlugin("base")->getStaticJsUrl();
        SC::getDocument()->addScript($jsDir . "captcha.js");

        $string = ' window.' . $this->jsObjectName . ' = new SC_Captcha( ' . json_encode(array('captchaUrl' => $captchaUrl,
                'captchaClass' => $captchaClass,
                'captchaId' => $this->getId(),
                'responderUrl' => $captchaResponderUrl
            )) . ');';

        SC::getDocument()->addOnloadScript($string);

        return '<div class="' . $captchaClass . '">
                    <div class="sc_automargin clearfix" style="width: 230px;">
                            <div class="sc_left"><img src="' . $captchaUrl . '" id="siimage"></div>
                            <div class="sc_right" style="padding-top: 21px;"><span class="ic_refresh sc_automargin" id="siimage_refresh" style="cursor:pointer;"></span></div>
                    </div>
...
class AgeRange extends FormElement implements DateRangeInterface
{
    ...
    public function renderInput( $params = null )
    {
        ...
        SC::getDocument()->addScript(SC::getPluginManager()->getPlugin("base")->getStaticJsUrl() . 'age_range_field.js');
        SC::getDocument()->addOnloadScript(" window." . $this->getName() . " = new AgeRangeField( " . json_encode($this->getName()) . ", " . json_encode($this->minAge) . ", " . ( $this->maxAge ) . " ); ");
        ...
        $result = '<div id="' . $this->getAttribute('id') . '"class="' . $this->getAttribute('name') . '">
                        <div class="sc_range_from sc_inline ">' . $language->text('base', 'form_element_from') . '</div>
                        <div class="sc_inline">' . UTIL_HtmlTag::generateTag('select', $fromAgeAttrs, true, $fromAgeOptionsString) . '</div>
                        <div class="sc_range_to sc_inline">' . $language->text('base', 'form_element_to') . '</div>
                        <div class="sc_inline">' . UTIL_HtmlTag::generateTag('select', $toAgeAttrs, true, $toAgeOptionsString) . '</div>
                        <div class="sc_range_label sc_inline">' . $language->text('base', 'form_element_age_range') . '</div>
                    </div>';
...
class MatchAgeRange extends AgeRange
{
    ...
    public function getElementJs()
    {
        $js = "var formElement = new ScRange(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ");";
...
//    }
    ...
    public function renderInput( $params = null )
    {
        ...
        SC::getDocument()->addScript(SC::getPluginManager()->getPlugin("base")->getStaticJsUrl() . 'range_field.js');
        SC::getDocument()->addOnloadScript(" window." . $this->getName() . " = new RangeField( '" . ( $this->getName() ) . "', " . ( $this->minValue ) . ", " . ( $this->maxValue ) . " ); ");
...
class DateRange extends FormElement implements DateRangeInterface
{
    ...
    public function renderInput( $params = null )
    {
        ...
        $result = '<div id="' . $this->getAttribute('id') . '" class="' . $this->getAttribute('name') . '">
                       ' . $language->text('base', 'form_element_from') . '  <div class="sc_inline">' . ( $this->minDate->renderInput() ) . '</div>
                       ' . $language->text('base', 'form_element_to') . '
                       <div class="sc_inline">' . ( $this->maxDate->renderInput() ) . '</div>
                    </div>';
...
class BillingGatewaySelectionField extends FormElement
{
    ...
    public function renderInput( $params = null )
    {
        ...
        $event = new SC_Event(BOL_BillingService::EVENT_ON_COLLECT_GATEWAYS_LIST, array(), $gateways);
        ...
        if ( $gateways )
        {
            ...
            $cont_id = $id . '-cont';
            $renderedString .= '<ul class="sc_billing_gateways clearfix" id="' . $cont_id . '">';
            ...
            foreach ( $paymentOptions as $option )
            {
                ...
                if ( $gatewaysNumber == 1 )
                {
                    $renderedString .= '<li style="display: inline-block; padding-right: 20px;">' . SC::getLanguage()->text('base', 'billing_pay_with') . '</li>';
            ...
            $js = 'var $url_field = $("#url-' . $id . '");
                ...
                $("ul#' . $cont_id . ' input").change(function(){
                    ...
                    SC.trigger("core.gateway_changed", [{gatewayKey: this.value}]);
                });';
        ...
        {
            $renderedString = SC::getLanguage()->text('base', 'billing_no_gateways');
        }
    ...
    protected function getAdapterData( $gateways )
    {
        ...
        {
            /* @var $adapter SC_BillingAdapter */
            if ( $adapter = SC::getClassInstance($gateway->adapterClassName) )
            {
...
class YearRange extends FormElement implements DateRangeInterface
{
    ...
    public function renderInput( $params = null )
    {
        ...
        SC::getDocument()->addScript(SC::getPluginManager()->getPlugin("base")->getStaticJsUrl() . 'age_range_field.js');
        SC::getDocument()->addOnloadScript(" window." . $this->getName() . " = new AgeRangeField( " . json_encode($this->getName()) . ", " . json_encode($this->minYear) . ", " . json_encode($this->maxYear) . " ); ");
        ...
        $result = '<div id="' . $this->getAttribute('id') . '"class="' . $this->getAttribute('name') . '">
                        <div class="sc_range_from sc_inline">' . $language->text('base', 'form_element_from') . '</div>
                        <div class="sc_inline">' . UTIL_HtmlTag::generateTag('select', $fromYearAttrs, true, $fromAgeOptionsString) . '</div>
                        <div class="sc_range_to sc_inline">' . $language->text('base', 'form_element_to') . '</div>
                        <div class="sc_inline">' . UTIL_HtmlTag::generateTag('select', $toYearAttrs, true, $toAgeOptionsString) . '</div>
                        <div class="sc_range_label sc_inline">' . $language->text('base', 'form_element_year_range') . '</div>
                    </div>';
...
class MobileWysiwygTextarea extends Textarea
{
    ...
    public function __construct( $name, $pluginKey = 'blog', array $buttons = array() )
    {
        ...
        $stringValidator->setErrorMessage(SC::getLanguage()->text('base', 'text_is_too_long', array('max_symbols_count' => 50000)));
    ...
    public function renderInput( $params = null )
    {
        ...
        if ( SC::getRegistry()->get('baseWsInit') === null )
        {
            if ( in_array(BOL_TextFormatService::WS_BTN_IMAGE, $this->buttons) )
            {
                SC::getDocument()->addScript(SC::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'jquery.html5_upload.js');
            }

            SC::getDocument()->addScript(SC::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'suitup.jquery.js');
            SC::getDocument()->addStyleSheet(SC::getPluginManager()->getPlugin('base')->getStaticCssUrl() . 'suitup.css');

            // register js langs
            SC::getLanguage()->addKeyForJs('base', 'ws_button_label_link');
            SC::getLanguage()->addKeyForJs('base', 'ws_button_label_video');
            SC::getLanguage()->addKeyForJs('base', 'ws_error_video');

            SC::getRegistry()->set('baseWsInit', true);
        }
        ...
        $js->addScript('$("#" + {$uniqId}).suitUp({$buttons}, {$imageUploadUrl}, {$embedUrl}).show();', array(
            ...
            'uniqId' => $this->getId(),
            'imageUploadUrl' => SC::getRouter()->urlFor('BASE_CTRL_MediaPanel', 'ajaxUpload', array(
                'pluginKey' => $this->pluginKey
            )),
            'embedUrl' => SC::getRouter()->urlFor('BASE_MCTRL_Oembed', 'getAjaxEmbedCode')
        ));
...
class WysiwygTextarea extends InvitationFormElement
{
    ...
    public function __construct( $name, array $buttons = null, $init = true )
    {
        ...
        if ( SC::getRequest()->isMobileUserAgent() )
        {
    ...
    public function getElementJs()
    {
        ...
        $jsString = "var formElement = new ScWysiwyg(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ", " . json_encode($invitation) . ");            
        ";
    ...
    public function renderInput( $params = null )
    {
        ...
        if ( SC::getRegistry()->get('baseWsInit') === null )
        {
            ...
            $array = array(
                'editorCss' => SC::getPluginManager()->getPlugin('base')->getStaticCssUrl() . 'htmlarea_editor.css',
                'themeImagesUrl' => SC::getThemeManager()->getCurrentTheme()->getStaticImagesUrl(),
                'imagesUrl' => SC::getRouter()->urlFor('BASE_CTRL_MediaPanel', 'index', array('pluginKey' => 'blog', 'id' => '__id__')),
                'labels' => array(
                    'buttons' => array(
                    ...
                    'common' => array(
                        ...
                        'linkNewWindowLabel' => $language->text('base', 'ws_link_new_windsc_label'),
                    ),
                    'messages' => array(
                ...
                'buttonCode' => SC::getThemeManager()->processDecorator('button', array('label' => '#label#', 'class' => 'sc_ic_add mn_submit')),
            ...
            SC::getDocument()->addScriptDeclarationBeforeIncludes($script);
            SC::getDocument()->addScript(SC::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'htmlarea.js');
            SC::getDocument()->addStyleSheet(SC::getPluginManager()->getPlugin('base')->getStaticCssUrl() . 'htmlarea.css');
            SC::getRegistry()->set('baseWsInit', true);
        }
        ...
        if ( $this->init )
        {
            SC::getDocument()->addOnloadScript("$('#{$this->getId()}').htmlarea( " . json_encode($params) . " );");
        }
...
class TagsInputField extends FormElement
{
    ...
    public function __construct( $name )
    {
        parent::__construct($name);
        $this->value = array();
        $this->invLabel = SC::getLanguage()->text('base', 'tags_input_field_invitation');
    ...
    public function renderInput( $params = null )
    {
        ...
        SC::getDocument()->addScript(SC::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'jquery.tagsinput.js');
...
var formElement = new ScFormElement('" . $this->getId() . "', '" . $this->getName() . "');";
...
class CsrfHiddenField extends HiddenField
{
    ...
    {
        $jsString = "
            var formElement = new ScFormElement(" . json_encode($this->getId()) . ", " . json_encode($this->getName()) . ");
            formElement.resetValue = function(){};
        ";
