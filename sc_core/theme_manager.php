 ...
 * @package sc_core
 * @method static SC_ThemeManager getInstance()
 * @since 1.0
 */
class SC_ThemeManager
{
    ...
    {
        $selectedTheme = SC::getConfig()->getValue('base', 'selectedTheme');

        if ( empty($this->themeObjects[$selectedTheme]) )
        {
            $this->themeObjects[$selectedTheme] = $this->themeService->getThemeObjectByKey(SC::getConfig()->getValue('base', 'selectedTheme'));
        }
     ...
     * Sets current theme.
     *
     * @param SC_Theme $theme
     */
    public function setCurrentTheme( SC_Theme $theme )
    {
        if ( $theme === null )
        {
            return;
    ...
    public function addDecorator( $decoratorName, $pluginKey )
    {
        ...
        $this->decorators[trim($decoratorName)] = SC::getPluginManager()->getPlugin($pluginKey)->getDecoratorDir() . $decoratorName . '.html';
    }
...
//        }
    ...
    public function processBlockDecorator( $name, array $params, $content = '' )
    {
        ...
        switch ( $name )
        {
            case 'box':
                if ( !empty($params['langLabel']) )
                {
                    ...
                    $params['label'] = SC::getLanguage()->text($tmpArr[0], $tmpArr[1]);
                }
                ...
                $params['addClass'] = $params['type'] . (empty($params['addClass']) ? '' : ' ' . $params['addClass']) . ( $params['capEnabled'] ? '' : ' sc_no_cap' );
        ...
        $params = $event->getProperties();
        $viewRenderer->assignVar('data', $params);
        $markup = $viewRenderer->renderTemplate(SC::getThemeManager()->getDecorator($name));
