 ...
 * @package sc_core
 * @since 1.0
 * @method static SC_Application getInstance()
 */
class SC_Application
{
    ...
    {
        SC::getConfig()->saveConfig('base', 'maintenance', (bool) $mode);
    }
    ...
    public function init()
    {
        ...
        $router->setDefaultRoute(new SC_DefaultRoute());
        ...
        SC_DeveloperTools::getInstance()->init();

        SC::getThemeManager()->initDefaultTheme($this->isMobile());

        // setting current theme
        $activeThemeName = SC::getEventManager()->call('base.get_active_theme_name');
        $activeThemeName = $activeThemeName ? $activeThemeName : SC::getConfig()->getValue('base', 'selectedTheme');
        ...
        $staticPageDispatchAttrs = SC::getRequestHandler()->getStaticPageAttributes();

        /* @var $value BOL_Document */
        foreach ( $staticDocs as $value )
        {
            SC::getRouter()->addRoute(new SC_Route($value->getKey(), $value->getUri(),
                $staticPageDispatchAttrs['controller'], $staticPageDispatchAttrs['action'],
                array('documentKey' => array(SC_Route::PARAM_OPTION_HIDDEN_VAR => $value->getKey()))));
            ...
            {
                SC::getRequestHandler()->addCatchAllRequestsExclude('base.members_only',
                    $staticPageDispatchAttrs['controller'], $staticPageDispatchAttrs['action'],
        ...
        $availableFor = SC::getUser()->isAuthenticated() ? BOL_NavigationService::VISIBLE_FOR_MEMBER : BOL_NavigationService::VISIBLE_FOR_GUEST;
        ...
        if ( $item !== null )
        {
            ...
            {
                $route = SC::getRouter()->getRoute($item->getRoutePath());
                $ddispatchAttrs = $route->getDispatchAttrs();
            }
            else
            {
                $ddispatchAttrs = SC::getRequestHandler()->getStaticPageAttributes();
            }

            $router->addRoute(new SC_Route('base_default_index', '/', $ddispatchAttrs['controller'],
                $ddispatchAttrs['action'],
                array('documentKey' => array(SC_Route::PARAM_OPTION_HIDDEN_VAR => $item->getDocumentKey()))));
            $this->indexMenuItem = $item;
            SC::getEventManager()->bind(SC_EventManager::ON_AFTER_REQUEST_HANDLE, array($this, 'activateMenuItem'));
        }
        else
        {
            $router->addRoute(new SC_Route('base_default_index', '/', 'BASE_CTRL_ComponentPanel', 'index'));
        }

        if ( !SC::getRequest()->isAjax() )
        {
            SC::getResponse()->setDocument($this->newDocument());
            SC::getDocument()->setMasterPage($this->getMasterPage());
            SC::getResponse()->setHeader(SC_Response::HD_CNT_TYPE,
                SC::getDocument()->getMime() . '; charset=' . SC::getDocument()->getCharset());
        }
        else
        {
            SC::getResponse()->setDocument(new SC_AjaxDocument());
        }
        ...
        $currentThemeImagesDir = SC::getThemeManager()->getCurrentTheme()->getStaticImagesUrl();
        $currentThemeStaticUrl = SC::getThemeManager()->getCurrentTheme()->getStaticUrl();
        ...
        $viewRenderer->assignVar('siteName', SC::getConfig()->getValue('base', 'site_name'));
        $viewRenderer->assignVar('siteTagline', SC::getConfig()->getValue('base', 'site_tagline'));
        $viewRenderer->assignVar('siteUrl', SC_URL_HOME);
        $viewRenderer->assignVar('isAuthenticated', SC::getUser()->isAuthenticated());
        $viewRenderer->assignVar('bottomPoweredByLink',
            '<a href="https://developers.scene.com/" target="_blank" title="Powered by Oxwall Community Software"><img src="' . $currentThemeImagesDir . 'powered-by-oxwall.png" alt="Scene Community Software" /></a>');

        $spotParams = array(
            "platform-version" => SC::getConfig()->getValue("base", "soft_version"),
            "platform-build" => SC::getConfig()->getValue("base", "soft_build"),
            "theme" => SC::getConfig()->getValue("base", "selectedTheme")
        );

        $viewRenderer->assignVar('adminDashboardIframeUrl',
            SC::getRequest()->buildUrlQueryString("//static.scene.org/spotlight/", $spotParams));

        if ( function_exists('sc_service_actions') )
        {
            call_user_func('sc_service_actions');
        }
    ...
    public function handleRequest()
    {
        ...
        {
            $attributes = array(
                SC_RequestHandler::CATCH_ALL_REQUEST_KEY_CTRL => 'BASE_CTRL_User',
        ...
        {
            $attributes = array(
                SC_RequestHandler::CATCH_ALL_REQUEST_KEY_CTRL => 'BASE_CTRL_BaseDocument',
    ...
    public function onBeforeDocumentRender()
    {
        ...
        $document->addStyleSheet(SC::getPluginManager()->getPlugin('base')->getStaticCssUrl() . 'sc.css' . '?' . SC::getConfig()->getValue('base',
                'cachedEntitiesPostfix'), 'all', -100);
        $document->addStyleSheet($themeManager->getCssFileUrl() . '?' . SC::getConfig()->getValue('base',
        ...
        {
            $document->addStyleSheet(SC::getPluginManager()->getPlugin('admin')->getStaticCssUrl() . 'admin.css' . '?' . SC::getConfig()->getValue('base',
    ...
    public function activateMenuItem()
    {
        if ( !SC::getDocument()->getMasterPage() instanceof ADMIN_CLASS_MasterPage )
        {
            ...
            {
                SC::getNavigation()->activateMenuItem(SC_Navigation::MAIN, $this->indexMenuItem->getPrefix(),
    ...
    protected function newDocument()
    {
        ...
        $document->addScript(SC::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'jquery.min.js',
            'text/javascript', (-100));
        $document->addScript(SC::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'jquery-migrate.min.js',
            'text/javascript', (-100));

        //$document->addScript(SC::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'json2.js', 'text/javascript', (-99));
        $document->addScript(SC::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'sc.js?' . SC::getConfig()->getValue('base',
    ...
    protected function urlHostRedirect()
    {
        ...
        $urlArray = parse_url(SC_URL_HOME);
        ...
        {
            $this->redirect(SC_URL_HOME . SC::getRequest()->getRequestUri());
        }
    }
    /**
     * @var array 
    ...
    {
        $this->httpsHandlerAttrsList[] = array(SC_RequestHandler::ATTRS_KEY_CTRL => $controller, SC_RequestHandler::ATTRS_KEY_ACTION => $action);
    }
    ...
    protected function httpVsHttpsRedirect()
    {
        ...
        {
            if ( $item[SC_RequestHandler::ATTRS_KEY_CTRL] == $attrs[SC_RequestHandler::ATTRS_KEY_CTRL] && ( empty($item[SC_RequestHandler::ATTRS_KEY_ACTION]) || $item[SC_RequestHandler::ATTRS_KEY_ACTION] == $attrs[SC_RequestHandler::ATTRS_KEY_ACTION] ) )
            {
                $specAttrs = true;
                if ( !$isSsl )
                {
                    $this->redirect(str_replace("http://", "https://", SC_URL_HOME) . SC::getRequest()->getRequestUri());
                }
            }
        }
    ...
    {
        if ( !SC::getRequest()->isSsl() || substr(SC::getRouter()->getBaseUrl(), 0, 5) == "https" )
        {
            return;
        }
        ...
        {
            $markup = SC::getResponse()->getMarkup();
            $matches = array();
            ...
            SC::getResponse()->setMarkup($markup);
        }
        SC::getEventManager()->bind(SC_EventManager::ON_AFTER_DOCUMENT_RENDER, "base_post_handle_https_static_content");
    }

    protected function userAutoLogin()
    {
        if ( SC::getSession()->isKeySet('no_autologin') )
        {
            SC::getSession()->delete('no_autologin');
            return;
        }

        if ( !empty($_COOKIE['sc_login']) && !SC::getUser()->isAuthenticated() )
        {
            $id = BOL_UserService::getInstance()->findUserIdByCookie(trim($_COOKIE['sc_login']));

            if ( !empty($id) )
            {
                SC_User::getInstance()->login($id);
                $loginCookie = BOL_UserService::getInstance()->findLoginCookieByUserId($id);
                setcookie('sc_login', $loginCookie->getCookie(), (time() + 86400 * 7), '/', null, false, true);
            }
        }
    }
    ...
    protected function addCatchAllRequestsException( $eventName, $key )
    {
        ...
        foreach ( $exceptions as $item )
        {
            ...
            {
                SC::getRequestHandler()->addCatchAllRequestsExclude($key, trim($item['controller']),
                    trim($item['action']));
            }
        }
    }
    ...
    {
        return BOL_NavigationService::getInstance()->findFirstLocal($availableFor, SC_Navigation::MAIN);
    }

    protected function getSiteRootRoute()
    {
        return new SC_Route('base_default_index', '/', 'BASE_CTRL_ComponentPanel', 'index');
    }

    protected function getMasterPage()
    {
        return new SC_MasterPage();
    }
    ...
    protected function detectLanguage()
    {
        ...
        if( $languageId > 0 )
        {
            SC::getSession()->set(BOL_LanguageService::LANG_ID_VAR_NAME, $languageId);
        }

        $session_language_id = SC::getSession()->get(BOL_LanguageService::LANG_ID_VAR_NAME);
