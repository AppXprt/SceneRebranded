 ...
 * @package sc_core
 * @method static SC_Dispatcher getInstance()
 * @since 1.0
 */
class SC_Dispatcher
{
    ...
    public function dispatch()
    {
        ...
        {
            SC::getRequest()->setUriParams($this->dispatchAttributes[self::ATTRS_KEY_VARLIST]);
        }

        $plugin = SC::getPluginManager()->getPlugin(SC::getAutoloader()->getPluginKey($this->dispatchAttributes[self::ATTRS_KEY_CTRL]));
        ...
        try
        {
            $controller = SC::getClassInstance($this->dispatchAttributes[self::ATTRS_KEY_CTRL]);
        }
        ...
        if ( $controller === null || !$controller instanceof SC_ActionController )
        {
    ...
    private function getControllerActionDefaultTemplate()
    {
        ...
        $templateFilename = SC::getAutoloader()->classToFilename($this->dispatchAttributes[self::ATTRS_KEY_CTRL], false) . '_'
            . SC::getAutoloader()->classToFilename(ucfirst($this->dispatchAttributes[self::ATTRS_KEY_ACTION]), false) . '.html';
        ...
        if ( $catchRequest )
        {
            if ( isset($this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_REDIRECT]) && (bool) $this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_REDIRECT] )
            {
                ...
                $redirectUrl = ($route !== null) ?
                    ...
                    SC::getRouter()->urlFor($this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_CTRL], $this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_ACTION], $params);

                $redirectUrl = SC::getRequest()->buildUrlQueryString($redirectUrl, array('back_uri' => SC::getRequest()->getRequestUri()));
                ...
                if ( isset($this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_JS]) && (bool) $this->catchAllRequestsAttributes[$lastKey][self::CATCH_ALL_REQUEST_KEY_JS] )
                {
                    ...
                    if ( array_key_exists('base.members_only', $this->catchAllRequestsAttributes) )
                    {
                        if ( $this->dispatchAttributes[self::CATCH_ALL_REQUEST_KEY_CTRL] === 'BASE_CTRL_User' && $this->dispatchAttributes[self::CATCH_ALL_REQUEST_KEY_ACTION] === 'standardSignIn' )
                        {
                            ...
                            SC::getDocument()->addOnloadScript("window.location = '" . SC::getRequest()->buildUrlQueryString($redirectUrl, array('back_uri' => $backUri)) . "'");
                            return null;
                        }
                        else
                        {
                            $ru = SC::getRequest()->buildUrlQueryString(SC::getRouter()->urlForRoute('static_sign_in'), array('back_uri' => SC::getRequest()->getRequestUri()));
