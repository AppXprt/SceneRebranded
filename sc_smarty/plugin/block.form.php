...
function smarty_block_form( $params, $content )
{
    ...
    $vr = SC_ViewRenderer::getInstance();
    
    $assignedForms = $vr->getAssignedVar('_scForms_');
    ...
    if ( $content === null )
    {
        $vr->assignVar('_scActiveForm_', $assignedForms[$params['name']]);
        return;
    }

    /* @var $form SC_Form */
    $form = $vr->getAssignedVar('_scActiveForm_');

    if ( isset($params['decorator']) )
    {
        $viewRenderer = SC_ViewRenderer::getInstance();
        $viewRenderer->assignVar('formInfo', $form->getElementsInfo());
        $content = $viewRenderer->renderTemplate(SC::getThemeManager()->getDecorator($params['decorator']));
    }
