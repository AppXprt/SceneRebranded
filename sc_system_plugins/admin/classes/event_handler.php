...
class ADMIN_CLASS_EventHandler
{
    ...
    public function init()
    {
        ...
        $eventManager->bind(SC_EventManager::ON_AFTER_ROUTE, array($this, "onAfterRoute"));
    ...
    public function onAddAdminNotification( ADMIN_CLASS_NotificationCollector $coll )
    {
        ...
        if ( SC::getConfig()->configExists("base", "cron_is_active") && (int) SC::getConfig()->getValue("base",
                "cron_is_active") == 0 )
        {
            $coll->add($language->text("admin", "warning_cron_is_not_active",
                    array("path" => SC_DIR_ROOT . "sc_cron" . DS . "run.php")),
                ADMIN_CLASS_NotificationCollector::NOTIFICATION_WARNING);
        }

        $items = $storageService->findItemsWithInvalidLicense();
        $licenseRequestUrl = SC::getRouter()->urlFor("ADMIN_CTRL_Storage", "checkItemLicense");
        $backUri = SC::getRouter()->getUri();
    ...
    public function onGetDisableActionList( SC_Event $e )
    {
        $params = $e->getParams();
        $data = $e->getData();
    ...
    public function onGetJoinStampDisableActionList( SC_Event $e )
    {
        $params = $e->getParams();
        $data = $e->getData();
    ...
    public function onAfterRoute( SC_Event $e )
    {
        $pluginService = BOL_PluginService::getInstance();
        $attrs = SC::getRequestHandler()->getHandlerAttributes();

        if ( is_subclass_of($attrs[SC_RequestHandler::ATTRS_KEY_CTRL], "ADMIN_CTRL_Abstract") )
        {
            ...
            {
                $params = array(
                    SC_RequestHandler::ATTRS_KEY_CTRL => "ADMIN_CTRL_Plugins",
                    SC_RequestHandler::ATTRS_KEY_ACTION => "manualUpdateRequest",
                    SC_RequestHandler::CATCH_ALL_REQUEST_KEY_REDIRECT => true
                );

                SC::getRequestHandler()->setCatchAllRequestsAttributes("admin.manual_update", $params);
                SC::getRequestHandler()->addCatchAllRequestsExclude("admin.manual_update", "ADMIN_CTRL_Plugins",
                    "manualUpdateAll");
            }
        }
    }

    public function checkIfAdminPage( SC_Event $e )
    {
        $attrs = SC::getRequestHandler()->getHandlerAttributes();

        if ( empty($attrs) || empty($attrs[SC_RequestHandler::ATTRS_KEY_CTRL]) )
        {
            return null;
        }

        return is_subclass_of($attrs[SC_RequestHandler::ATTRS_KEY_CTRL], "ADMIN_CTRL_Abstract");
    }
}
