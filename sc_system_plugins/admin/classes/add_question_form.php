...
require_once SC_DIR_SYSTEM_PLUGIN . 'admin' . DS . 'classes' . DS . 'form_fields.php';
...
class ADMIN_CLASS_AddQuestionForm extends Form
{
            ...
            else
            {
                ...
                /* if ( result.errors )
                {
                    each( result.errors, function( key, item ) {
                        ...
                        if ( item )
                        {
                            var element = window.scForms['.  json_encode($name).'].getElement(key);
    ...
    protected function init( $params = array() )
    {
        ...
        $event = new SC_Event('base.question.add_question_form.on_get_available_sections', $sectionsArray, $sectionsArray);
        ...
        foreach ( $configList as $config )
        {
            ...
            /* @var $qstConfig SC_FormElement */
            $qstConfig = SC::getClassInstance($className, $config->name);
        ...
        $qstSubmit->addAttribute('class', 'sc_button sc_ic_save');
    ...
    {
        if ( SC_Request::getInstance()->isPost() )
        {
            ...
            if ( $this->isValid($data) )
            {
                ...
                SC::getFeedback()->info(SC::getLanguage()->text( 'admin', 'questions_add_question_message' ));
            ...
            {
                echo json_encode( array( 'result' => false, 'errors' => $this->getErrors(), 'message' => SC::getLanguage()->text( 'admin', 'questions_add_question_error' ) ) );
            }
            exit;

            //SC::getSession()->set(self::ADD_QUESTION_SESSION_VAR, $_POST);
        }
    }
}
