...
class ADMIN_CLASS_MobileNavigationItemSettingsForm extends Form
{
    ...
    public function __construct( BOL_MenuItem $menuItem, $custom = false, $addUrlValidator = true )
    {
        ...
        $this->setAjax();
        $this->setAction(SC::getRouter()->urlFor("ADMIN_CTRL_MobileNavigation", "saveItemSettings"));
        ...
        if ( $custom )
        {
            ...
            $js->addScript('var url = scForms[{$name}].elements["url"]; var validators = url.validators; url.validators = []; $("input[name=type]", "#" + {$id}).change(function() { '
                    . 'if ($(this).val() == "local") { '
