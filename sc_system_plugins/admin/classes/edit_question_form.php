...
class ADMIN_CLASS_EditQuestionForm extends ADMIN_CLASS_AddQuestionForm
{
    ...
    public function loadQuestionData( BOL_Question $question )
    {
        ...
        foreach ( $this->presentations2types as $key => $item )
        {
            ...
            {
                $presentationsLabel[$key] = SC::getLanguage()->text('base', 'questions_question_presentation_' . $key . '_label');
            }
        }
...
//                }
                ...
                $message = SC::getLanguage()->text('admin', 'questions_question_was_not_updated_message');

                if ( $updated )
                {
                    $message = SC::getLanguage()->text('admin', 'questions_update_question_message');
                }
            ...
            {
                echo json_encode( array( 'result' => false, 'errors' => $this->getErrors(), 'message' => SC::getLanguage()->text( 'admin', 'questions_update_error' ) ) );
            }
            exit;
