...
class ADMIN_CTRL_Seo extends ADMIN_CTRL_Abstract
{
    ...
    public function __construct()
    {
        ...
        // activate menu item
//        SC::getDocument()->getMasterPage()->
//                getMenu(SC_Navigation::ADMIN_SETTINGS)->getElement('sidebar_menu_item_seo_settings')->setActive(true);

        $this->setPageHeading(SC::getLanguage()->text('admin', 'seo_page_heading'));
    ...
    public function index()
    {
        ...
        $this->assign("currentUrl", SC::getRouter()->urlForRoute("admin_settings_seo")."?section=#sec#");
        ...
        if( SC::getRequest()->isPost() ){
            ...
            {
                SC::getFeedback()->error($form->getEmptyElementsErrorMessage());
            }
        }
    }
    ...
    public function sitemap()
    {
        ...
        if ( SC::getRequest()->isPost() && $form->isValid($_POST) )
        {
            ...
            // save schedule
            SC::getConfig()->saveConfig('base', 'seo_sitemap_schedule_update', $formValues['schedule']);

            // reload the current page
            SC::getFeedback()->info(SC::getLanguage()->text('admin', 'seo_sitemap_settings_updated'));
            $this->redirect();
        }
    ...
    public function socialMeta()
    {
        ...
        if ( SC::getRequest()->isPost() )
        {
            ...
            if ( !$result["result"] )
            {
                SC::getFeedback()->error($result["message"]);
                $this->redirect();
            }
    ...
    protected function getMenu()
    {
        ...
        $item->setLabel(SC::getLanguage()->text('admin', 'seo_page'));
        $item->setIconClass('sc_ic_files');
        $item->setKey('seo_page');
        $item->setUrl(SC::getRouter()->urlForRoute('admin_settings_seo'));
        $item->setOrder(1);
        $items[] = $item;

        $item = new BASE_MenuItem();
        $item->setLabel(SC::getLanguage()->text('admin', 'seo_sitemap'));
        $item->setIconClass('sc_ic_script');
        $item->setKey('seo_sitemap');
        $item->setUrl(SC::getRouter()->urlForRoute('admin_settings_seo_sitemap'));
        $item->setOrder(2);
        $items[] = $item;

        $item = new BASE_MenuItem();
        $item->setLabel(SC::getLanguage()->text('admin', 'seo_social_meta'));
        $item->setIconClass('sc_ic_flag');
        $item->setKey('seo_social_meta');
        $item->setUrl(SC::getRouter()->urlForRoute('admin_settings_seo_social_meta'));
        $item->setOrder(3);
        $items[] = $item;
