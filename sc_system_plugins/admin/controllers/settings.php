...
class ADMIN_CTRL_Settings extends ADMIN_CTRL_Abstract
{
    ...
    private function getMenu()
    {
        ...
        $item->setUrl(SC::getRouter()->urlForRoute('admin_settings_main'));
        $item->setKey('basics');
        $item->setIconClass('sc_ic_gear_wheel');
        $item->setOrder(0);
        $menuItems[] = $item;

        $item = new BASE_MenuItem();
        $item->setLabel($language->text('admin', 'menu_item_page_settings'));
        $item->setUrl(SC::getRouter()->urlForRoute('admin_settings_page'));
        $item->setKey('page');
        $item->setIconClass('sc_ic_file');
        $item->setOrder(1);
        $menuItems[] = $item;
        ...
        if ( !defined('SC_PLUGIN_XP') )
        {
            ...
            $item->setUrl(SC::getRouter()->urlForRoute('admin_settings_mail'));
            $item->setKey('mail');
            $item->setIconClass('sc_ic_mail');
            $item->setOrder(2);
            $menuItems[] = $item;
        }
    ...
    public function index()
    {
        ...
        {
            SC::getNavigation()->activateMenuItem(SC_Navigation::ADMIN_SETTINGS, 'admin', 'sidebar_menu_item_main_settings');
        }
        ...
        $configs = SC::getConfig()->getValues('base');

        if ( SC::getRequest()->isPost() && $configSaveForm->isValid($_POST) && isset($_POST['save']) )
        {
            $res = $configSaveForm->process();
            SC::getFeedback()->info($language->text('admin', 'main_settings_updated'));
        ...
        if ( !SC::getRequest()->isAjax() )
        {
            SC::getDocument()->setHeading(SC::getLanguage()->text('admin', 'heading_main_settings'));
        ...
        $jsDir = SC::getPluginManager()->getPlugin("admin")->getStaticJsUrl();
        SC::getDocument()->addScript($jsDir . "main_settings.js");

        $script = ' var main_settings = new mainSettings( ' . json_encode(SC::getRouter()->urlFor("ADMIN_CTRL_Settings", "ajaxResponder")) . ' )';
    ...
    public function userInput()
    {
        ...
        if ( SC::getRequest()->isPost() )
        {
            ...
            {
                SC::getFeedback()->error($language->text('admin', 'settings_submit_error_message'));
            }
        ...
        SC::getDocument()->setHeading(SC::getLanguage()->text('admin', 'heading_user_input_settings'));
        SC::getDocument()->setHeadingIconClass('sc_ic_gear_wheel');
    }
    ...
    public function user()
    {
        if ( !SC::getRequest()->isAjax() )
        {
            SC::getNavigation()->activateMenuItem(SC_Navigation::ADMIN_SETTINGS, 'admin', 'sidebar_menu_item_user_settings');
        }
        ...
        if ( isset($_GET['del-avatar']) && in_array($_GET['del-avatar'], array(1, 2)) )
        {
            ...
            if ( $del )
            {
                SC::getFeedback()->info($language->text('admin', 'default_avatar_deleted'));
            }

            $this->redirect(SC::getRouter()->urlForRoute('admin_settings_user'));
        }
        ...
        $userSettingsForm->getElement('user_view_presentation')->
                setValue((SC::getConfig()->getValue('base', 'user_view_presentation') == 'tabs'));

        $this->assign('displayConfirmEmail', !defined('SC_PLUGIN_XP'));

        if ( SC::getRequest()->isPost() && $userSettingsForm->isValid($_POST) )
        {
            ...
            {
                SC::getFeedback()->error($language->text('base', 'not_valid_image'));
                $this->redirect();
            }

            $values = $userSettingsForm->getValues();
            $guestPassword = SC_Config::getInstance()->getValue('base', 'guests_can_view_password');

            if ( (int) $values['guests_can_view'] === 3 && empty($values['password']) && is_null($guestPassword) )
            {
                SC::getFeedback()->error($language->text('admin', 'permission_global_privacy_empty_pass_error_message'));
                $this->redirect();
            }
            else if ( (int) $values['guests_can_view'] === 3 && strlen(trim($values['password'])) < 4 && strlen(trim($values['password'])) > 0 )
            {
                SC::getFeedback()->error($language->text('admin', 'permission_global_privacy_pass_length_error_message'));
                $this->redirect();
            }
            ...
            SC::getFeedback()->info($language->text('admin', 'user_settings_updated'));
            $this->redirect();
        }
        ...
        if ( !SC::getRequest()->isAjax() )
        {
            SC::getDocument()->setHeading(SC::getLanguage()->text('admin', 'heading_user_settings'));
            SC::getDocument()->setHeadingIconClass('sc_ic_gear_wheel');
        }

        SC::getNavigation()->deactivateMenuItems(SC_Navigation::ADMIN_SETTINGS);
    }
    ...
    public function page()
    {
        ...
        if ( !SC::getRequest()->isAjax() )
        {
            SC::getDocument()->setHeading(SC::getLanguage()->text('admin', 'heading_page_settings'));
            SC::getDocument()->setHeadingIconClass('sc_ic_file');
        }
        ...
        $faviconPath = SC::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'favicon.ico';
        $faviconUrl = SC::getPluginManager()->getPlugin('base')->getUserFilesUrl() . 'favicon.ico';
        ...
        if ( SC::getRequest()->isPost() )
        {
            ...
            {
                $data = $form->getValues();
                SC::getConfig()->saveConfig('base', 'html_head_code', $data['head_code']);
                SC::getConfig()->saveConfig('base', 'html_prebody_code', $data['bottom_code']);
                ...
                if ( !empty($_FILES['favicon']['name']) )
                {
                    ...
                    {
                        SC::getFeedback()->error($language->text('admin', 'page_settings_favicon_submit_error_message'));
                    }
                }

                SC::getConfig()->saveConfig('base', 'favicon', !empty($data['enable_favicon']));
                SC::getFeedback()->info($language->text('admin', 'settings_submit_success_message'));
            }
            else
            {
        ...
        $headCode->setValue(SC::getConfig()->getValue('base', 'html_head_code'));
        $bottomCode->setValue(SC::getConfig()->getValue('base', 'html_prebody_code'));
        $enableFavicon->setValue((int) SC::getConfig()->getValue('base', 'favicon'));
        $this->assign('faviconEnabled', SC::getConfig()->getValue('base', 'favicon'));
    ...
    public function mail()
    {
        ...
        if ( SC::getRequest()->isPost() && $mailSettingsForm->isValid($_POST) )
        {
            $res = $mailSettingsForm->process();
            SC::getFeedback()->info($language->text('admin', 'mail_settings_updated'));
            $this->redirect();
        }

        if ( !SC::getRequest()->isAjax() )
        {
            SC::getDocument()->setHeading(SC::getLanguage()->text('admin', 'heading_mail_settings'));
            SC::getDocument()->setHeadingIconClass('sc_ic_mail');
        ...
        {
            $smtpTestresponder = json_encode(SC::getRouter()->urlFor('ADMIN_CTRL_Settings', 'ajaxSmtpTestConnection'));
            $readyJs = "
                jQuery('#smtp_test_connection').click(function(){
                    window.SC.inProgressNode(this);
                    var self = this;
                    jQuery.get($smtpTestresponder, function(r){
                        window.SC.activateNode(self);
                        alert(r);
                    });
                });
            ";
            SC::getDocument()->addOnloadScript($readyJs);
            $smtpEnabled = true;
        }
    ...
    public function ajaxSmtpTestConnection()
    {
        ...
        if ( $result )
        {
            $responce = SC::getLanguage()->text('admin', 'smtp_test_connection_success');
        }
        else
        {
            $responce = SC::getLanguage()->text('admin', 'smtp_test_connection_failed');
        }
    ...
    public function ajaxResponder()
    {
        ...
        switch ( $command )
        {
            case 'sendVerifyEmail':
                ...
                {
                    SC::getConfig()->saveConfig('base', 'unverify_site_email', $email);

                    $siteEmail = SC::getConfig()->getValue('base', 'site_email');

                    if ( $siteEmail !== $email )
                    {
                        $type = 'info';
                        BOL_EmailVerifyService::getInstance()->sendSiteVerificationMail(false);
                        $message = SC::getLanguage()->text('base', 'email_verify_verify_mail_was_sent');
                        $result = true;
                    }
                    else
                    {
                        $type = 'warning';
                        $message = SC::getLanguage()->text('admin', 'email_already_verify');
                    }
                }
...
class ConfigSaveForm extends Form
{
    ...
    public function __construct()
    {
        ...
        $dateFormatValue = SC::getConfig()->getValue('base', 'date_field_format');
...
//
//        $imagesAllowPicUpload->setLabel(SC::getLanguage()->text('base', 'tf_allsc_pics'))
//            ->setValue(SC::getConfig()->getValue('base', 'tf_allsc_pic_upload'));
//
//        $this->addElement($imagesAllowPicUpload);
//
//        $imageMaxSizeField = new TextField('tf-max-image-size');
//
//        $imageMaxSizeField->setValue(SC::getConfig()->getValue('base', 'tf_max_pic_size'))
//            ->setLabel(SC::getLanguage()->text('base', 'tf_max_img_size'))
...
//        $config->saveConfig('base', 'tf_allsc_pic_upload', $values['tf-allow-pic-upload']);
...
class UserSettingsForm extends Form
{
    ...
    public function __construct($maxUploadMaxFilesize)
    {
        ...
        if ( !defined('SC_PLUGIN_XP') )
        {
            ...
            $confirmEmail->setValue(SC::getConfig()->getValue('base', 'confirm_email'));
        ...
        $joinConfigField->setValue(SC::getConfig()->getValue('base', 'join_display_photo_upload'));
        $this->addElement($joinConfigField);

        // --

        $joinConfigField = new CheckboxField('join_display_terms_of_use');
        $joinConfigField->setValue(SC::getConfig()->getValue('base', 'join_display_terms_of_use'));
        $this->addElement($joinConfigField);
    ...
    public function process()
    {
        ...
        {
            $adminEmail = SC::getUser()->getEmail();
            ...
            $mail->setSubject(SC::getLanguage()->text( 'admin', 'site_password_letter_subject', array()));
            $mail->setTextContent( SC::getLanguage()->text( 'admin', 'site_password_letter_template_text', array('password' => $values['password'])));
            $mail->setHtmlContent( SC::getLanguage()->text( 'admin', 'site_password_letter_template_html', array('password' => $values['password'])));
            try
            {
            ...
            {
                $logger = SC::getLogger('admin.send_password_message');
                $logger->addEntry($e->getMessage());
                $logger->writeLog();
            }
            
            $values['password'] = crypt($values['password'], SC_PASSWORD_SALT);
 ...
 * @since 1.8.4
 */
class FileExtensionValidator extends SC_Validator
{
    /**
    ...
    public function __construct()
    {
        $this->errorMessage = SC::getLanguage()->text('admin', 'wrong_file_extension', array(
...
class MailSettingsForm extends Form
{
    ...
    public function __construct()
    {
        ...
        $smtpField->setInvitation(SC::getLanguage()->text('admin', 'mail_smtp_secure_invitation'));
