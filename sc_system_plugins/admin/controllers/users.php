 ...
 *
 * @package ow.sc_system_plugins.admin.controllers
 * @since 1.0
 */
class ADMIN_CTRL_Users extends ADMIN_CTRL_Abstract
{
    ...
    private function getMenu()
    {
        ...
        foreach ( $keys as $ord => $key )
        {
            ...
            $item->setUrl(SC::getRouter()->urlForRoute('admin_users_browse', $urlParams));
            $item->setKey($key);
            $item->setIconClass('sc_ic_' . $icons[$ord]);
            $item->setOrder($ord);
    ...
    public function index( array $params )
    {
        ...
        SC::getDocument()->getMasterPage()->getMenu(SC_Navigation::ADMIN_USERS)->getElement('sidebar_menu_item_users')->setActive(true);
        ...
        $emails->setInvitation($language->text('admin', 'invite_members_textarea_invitation_text', array('limit' => (int)SC::getConfig()->getValue('base', 'user_invites_limit'))));
        ...
        if ( SC::getRequest()->isPost() && isset($_POST['invite_members']) )
        {
            if ( $form->isValid($_POST) )
            {
                ...
                if ( sizeof($emailList) > (int)SC::getConfig()->getValue('base', 'user_invites_limit') )
                {
                    SC::getFeedback()->error($language->text('admin', 'invite_members_max_limit_message', array('limit' => (int)SC::getConfig()->getValue('base', 'user_invites_limit'))));
                ...
                if ( empty($emailList) )
                {
                    SC::getFeedback()->error($language->text('admin', 'invite_members_min_limit_message'));
                    $form->getElement('emails')->setValue($data['emails']);
                    $this->redirect();
                }
                ...
                SC::getFeedback()->info($language->text('admin', 'invite_members_success_message'));
                $this->redirect();
            }
        }
        ...
        $buttons['suspend'] = array('name' => 'suspend', 'id' => 'suspend_user_btn', 'label' => $language->text('base', 'suspend_user_btn'), 'class' => 'sc_mild_red');
        $buttons['suspend']['js'] = ' $("#suspend_user_btn").click(function(e){ 
            e.preventDefault();
            SC.ajaxFloatBox("ADMIN_CMP_SetSuspendMessage", [],{width: 520, title: SC.getLanguageText(\'admin\', \'admin_suspend_floatbox_title\')}); 
            return false;
        }); ';
        
        $buttons['unverify'] = array('name' => 'email_unverify', 'id' => 'email_unverify_user_btn', 'label' => $language->text('base', 'mark_email_unverified_btn'), 'class' => 'sc_mild_red');
        $buttons['unsuspend'] = array('name' => 'reactivate', 'id' => 'unsuspend_user_btn', 'label' => $language->text('base', 'unsuspend_user_btn'), 'class' => 'sc_mild_green');
        $buttons['verify'] = array('name' => 'email_verify', 'id' => 'email_verify_user_btn', 'label' => $language->text('base', 'mark_email_verified_btn'), 'class' => 'sc_mild_green');
        $buttons['approve'] = array('name' => 'approve', 'id' => 'approve_user_btn', 'label' => $language->text('base', 'approve_user_btn'), 'class' => 'sc_mild_green');
        //$buttons['disapprove'] = array('name' => 'disapprove', 'id' => 'disapprove_user_btn', 'label' => $language->text('base', 'disapprove_user_btn'), 'class' => 'sc_mild_red');
        ...
        $usersCmp = SC::getClassInstance("ADMIN_CMP_UserList", $par);
        $this->addComponent('userList', $usersCmp);

        if ( !SC::getRequest()->isAjax() )
        {
            SC::getDocument()->setHeading(SC::getLanguage()->text('admin', 'heading_browse_users'));
            SC::getDocument()->setHeadingIconClass('sc_ic_user');
        ...
        $question = SC::getConfig()->getValue('base', 'display_name_question');
        ...
        $this->assign('userSearchUrl', SC::getRouter()->urlForRoute('admin_users_browse'));
    }
    ...
    {        
        ...
        if ( SC::getRequest()->isPost() && $addRoleForm->isValid( $_POST ) )
        {
        ...
        SC::getLanguage()->addKeyForJs('admin', 'permissions_edit_role_btn');

        SC::getDocument()->setHeadingIconClass('sc_ic_user');
        SC::getDocument()->setHeading(SC::getLanguage()->text('admin', 'heading_user_roles'));
        
        // users roles
        $service = BOL_AuthorizationService::getInstance();
        $this->assign('formAction', SC::getRouter()->urlFor('ADMIN_CTRL_Permissions', 'savePermissions'));
        ...
        {
            if ( count($value['actions']) === 0 || !SC::getPluginManager()->isPluginActive($value['name']) )
            {
    ...
    public function role( array $params )
    {
        if ( !empty($params['roleId']) )
        {
            ...
            $roleLabel = SC::getLanguage()->text('base', 'authorization_role_' . $role->name);

            SC::getDocument()->setHeading(SC::getLanguage()->text('admin', 'heading_user_role', array('role' => $roleLabel)));
        }

        SC::getDocument()->setHeadingIconClass('sc_ic_user');

        $js = UTIL_JsGenerator::newInstance()
                ->newVariable('rolesUrl', SC::getRouter()->urlForRoute('admin_user_roles'))
    ...
    public function deleteRoles()
    {
        ...
        {
            $this->redirect(SC::getRouter()->urlFor('ADMIN_CTRL_Users', 'roles'));
        }
        ...
        SC::getFeedback()->info(SC::getLanguage()->text('admin', 'permissions_roles_deleted_msg'));
...
class AddRoleForm extends Form
{
    ...
    public function __construct()
    {
        ...
        $this->addElement($textField->setRequired(true)->setLabel(SC::getLanguage()->text('admin', 'permissions_add_form_role_lbl')));

        $submit = new Submit('submit');

        $submit->setValue(SC::getLanguage()->text('admin', 'permissions_add_role_btn'));
    ...
    public function process( $data )
    {
        ...
        SC::getFeedback()->info(SC::getLanguage()->text('admin', 'permissions_role_added_msg'));
    }
}
