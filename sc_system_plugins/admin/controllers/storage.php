...
class ADMIN_CTRL_Storage extends ADMIN_CTRL_StorageAbstract
{
    ...
    public function checkItemLicense()
    {
        ...
        if ( isset($params["back-button-uri"]) )
        {
            ...
            $redirectUrl = UTIL_HtmlTag::escapeJs(SC_URL_HOME . urldecode($params["back-button-uri"]));
        ...
        if ( SC::getRequest()->isPost() )
        {
            if ( $form->isValid($_POST) )
            {
                ...
                if ( $result )
                {
                    ...
                    SC::getFeedback()->info($language->text("admin", "plugins_manage_license_key_check_success"));
                    $this->redirectToBackUri($params);
                    $this->redirect();
                }
                else
                {
                    SC::getFeedback()->error($language->text('admin', 'plugins_manage_invalid_license_key_error_message'));
                    $this->redirect();
                }
            }
        }
    ...
    {
        if ( !(bool) SC::getConfig()->getValue("base", "update_soft") )
        {
...
//TODO check if the result is false | null
        $params = array(
            "oldVersion" => SC::getConfig()->getValue("base", "soft_version"),
            "newVersion" => $newPlatformInfo["version"],
            "info" => $newPlatformInfo["info"]
        );
        $this->assign("text", SC::getLanguage()->text("admin", "manage_plugins_core_update_request_text", $params));
        $this->assign("redirectUrl", SC::getRouter()->urlFor(__CLASS__, "platformUpdate"));
        $this->assign("returnUrl", SC::getRouter()->urlForRoute("admin_default"));
        ...
        {
            $this->assign("phpVersionInvalidText", SC::getLanguage()->text("admin", "plugin_update_platform_invalid_php_version_msg",
    ...
    public function platformUpdate()
    {
        ...
        $language = SC::getLanguage();
        $tempDir = SC_DIR_PLUGINFILES . "ow" . DS . "core" . DS;

        $ftp = $this->getFtpConnection();

        $errorMessage = false;

        SC::getApplication()->setMaintenanceMode(true);
        ...
        else
        {
            ...
            if ( $zopen === true && file_exists($tempDir) )
            {
                ...
                $zip->close();
                $ftp->uploadDir($tempDir, SC_DIR_ROOT);
                $ftp->chmod(0777, SC_DIR_STATIC);
                $ftp->chmod(0777, SC_DIR_STATIC_PLUGIN);
            }
            else
            {
        ...
        {
            SC::getApplication()->setMaintenanceMode(false);
            SC::getFeedback()->error($errorMessage);
            $this->redirect(SC::getRouter()->urlFor("ADMIN_CTRL_Index", "index"));
        }
    ...
    public function checkUpdates()
    {
        ...
        {
            SC::getFeedback()->info(SC::getLanguage()->text("admin", "check_updates_success_message"));
        }
        else
        {
            SC::getFeedback()->error(SC::getLanguage()->text("admin", "check_updates_fail_error_message"));
        }

        $backUrl = SC::getRouter()->urlForRoute("admin_default");

        if ( isset($_GET[BOL_StorageService::URI_VAR_BACK_URI]) )
        {
            $backUrl = SC_URL_HOME . urldecode($_GET[BOL_StorageService::URI_VAR_BACK_URI]);
        }
    ...
    public function ftpAttrs()
    {
        ...
        if ( SC::getRequest()->isPost() )
        {
            if ( $form->isValid($_POST) )
            {
                ...
                SC::getSession()->set("ftpAttrs", $ftpAttrs);
