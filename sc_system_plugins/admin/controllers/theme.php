...
class ADMIN_CTRL_Theme extends ADMIN_CTRL_Abstract
{
    ...
    public function init()
    {
        ...
        $pageActions = array(array('name' => 'settings', 'iconClass' => 'sc_ic_gear_wheel'), array('name' => 'css', 'iconClass' => 'sc_ic_files'), array('name' => 'graphics', 'iconClass' => 'sc_ic_picture'));
        ...
        foreach ( $pageActions as $key => $item )
        {
            ...
            $menuItem->setKey($item['name'])->setLabel(SC::getLanguage()->text('admin', 'sidebar_menu_item_' . $item['name']))->setOrder($key)->setUrl($router->urlForRoute('admin_theme_' . $item['name']));
        ...
        SC::getNavigation()->activateMenuItem(SC_Navigation::ADMIN_APPEARANCE, 'admin', 'sidebar_menu_item_theme_edit');
        $this->setPageHeading(SC::getLanguage()->text('admin', 'themes_settings_page_title'));
    }

    public function settings()
    {
        $dto = $this->themeService->findThemeByKey(SC::getConfig()->getValue('base', 'selectedTheme'));

        if ( $dto === null )
        {
            throw new LogicException("Can't find theme `" . SC::getConfig()->getValue('base', 'selectedTheme') . "`");
        }
        ...
        $this->assign('resetUrl', SC::getRouter()->urlFor(__CLASS__, 'reset'));
    ...
    public function css()
    {
        if ( SC::getRequest()->isAjax() )
        {
            ...
            echo json_encode(array('message' => SC::getLanguage()->text('admin', 'css_edit_success_message')));
        }

        if ( !SC::getRequest()->isAjax() )
        {
            SC::getDocument()->getMasterPage()->getMenu(SC_Navigation::ADMIN_APPEARANCE)->getElement('sidebar_menu_item_themes_customize')->setActive(true);
        }

        SC::getDocument()->addScript(SC::getPluginManager()->getPlugin('admin')->getStaticJsUrl() . 'prettify.js');
        SC::getDocument()->addScript(SC::getPluginManager()->getPlugin('admin')->getStaticJsUrl() . 'lang-css.js');
        SC::getDocument()->addStyleSheet(SC::getPluginManager()->getPlugin('admin')->getStaticCssUrl() . 'prettify.css');
        SC::getDocument()->addOnloadScript("prettyPrint();");

        $fileString = file_get_contents(SC::getThemeManager()->getSelectedTheme()->getRootDir() . BOL_ThemeService::CSS_FILE_NAME);
    ...
    {
        return array(
            'url' => SC::getStorage()->getFileUrl($this->themeService->getUserfileImagesDir() . $image->getFilename()),
            'delUrl' => SC::getRouter()->urlFor(__CLASS__, 'deleteImage', array('image-id' => $image->getId())),
    ...
    public function graphics()
    {
        ...
        $this->assign('confirmMessage', SC::getLanguage()->text('admin', 'theme_graphics_image_delete_confirm_message'));

        $cmp = SC::getClassInstance('ADMIN_CMP_UploadedFileList');
        ...
        if ( SC::getRequest()->isPost() )
        {
            ...
            {
                SC::getFeedback()->error(SC::getLanguage()->text('admin', 'theme_graphics_upload_form_fail_message'));
                $this->redirect();
            }

            SC::getFeedback()->info(SC::getLanguage()->text('admin', 'theme_graphics_upload_form_success_message'));
            $this->redirect();
        }
    }
    ...
    public function bulkOptions()
    {
        ...
        switch ($action)
        {
            case 'delete':
                ...
                if (is_null($items))
                {
                    $result = json_encode(array('error' => SC::getLanguage()->text('admin', 'not_enough_params')));
                }
                else
                {
                    foreach ($items as $item)
                    {
                        ...
                        SC::getFeedback()->info(SC::getLanguage()->text('admin', 'theme_graphics_delete_success_message'));
                    }
                    $result = json_encode(array(
                        'success' => SC::getLanguage()->text('admin', 'theme_graphics_delete_multiple_success_message'),
                        'reload' => true
                    ));
                }
                break;
            default:
                $result = json_encode(array('error' => SC::getLanguage()->text('admin', 'undefined_action')));
                break;
        }
        echo $result;
        exit();
    ...
    private function prepareMarkup( $photo, $layout = null )
    {
        ...
        $photo->url = SC::getStorage()->getFileUrl($this->themeService->getUserfileImagesDir() . $photo->getFilename());
    ...
    {
        $this->themeService->resetImageControl(SC::getThemeManager()->getSelectedTheme()->getDto()->getId(), trim($_GET['name']));
        $this->redirect(SC::getRouter()->urlForRoute('admin_themes_edit'));
    }
    ...
    public function deleteImage( $params )
    {
        ...
        SC::getFeedback()->info(SC::getLanguage()->text('admin', 'theme_graphics_delete_success_message'));
        $this->redirect(SC::getRouter()->urlForRoute('admin_theme_graphics'));
    }
    ...
    public function ajaxDeleteImage( $params )
    {
        ...
        return array(
            'result' => true,
            'msg' => SC::getLanguage()->text('admin', 'theme_graphics_delete_success_message'),
            'imageId' => $imageId
        );
    }
    ...
    private function initFloatbox( $params )
    {
        ...
        $document = SC::getDocument();
        $basePlugin = SC::getPluginManager()->getPlugin('base');
        ...
        $language->addKeyForJs('admin', 'slideshsc_interval');
        ...
        $document->addScriptDeclarationBeforeIncludes(
            UTIL_JsGenerator::composeJsString('
                ;window.photoViewParams = Object.defineProperties({}, {
                    ...
                    'ajaxResponder' => SC::getRouter()->urlFor('ADMIN_CTRL_Theme', 'ajaxResponder'),
...
class UploadGraphicsForm extends Form
{
    ...
    public function __construct()
    {
        ...
        $submit->setValue(SC::getLanguage()->text('admin', 'theme_graphics_upload_form_submit_label'));
...
class AddCssForm extends Form
{
    ...
    public function __construct()
    {
        ...
        $dto = BOL_ThemeService::getInstance()->findThemeByKey(SC::getConfig()->getValue('base', 'selectedTheme'));
        $text->setValue($dto->getCustomCss());
        $this->addElement($text);

        $submit = new Submit('submit');
        $submit->setValue(SC::getLanguage()->text('admin', 'theme_css_edit_submit_label'));
        $this->addElement($submit);

        $this->setAjax(true);
        $this->setAjaxResetOnSuccess(false);
        $this->bindJsFunction(Form::BIND_SUCCESS, 'function(data){SC.info(data.message)}');
    }
}
...
class ThemeEditForm extends Form
{
    ...
    public function __construct( $controls )
    {
        ...
        $submit = new Submit('submit');
        $submit->setValue(SC::getLanguage()->text('admin', 'theme_settings_form_submit_label'));
    ...
    public function renderInput( $params = null )
    {
        ...
        if ( $this->value !== null && ( trim($this->value) !== 'none' ) )
        {
            if ( !strstr($this->value, 'http') )
            {
                ...
                $this->value = 'url(' . SC::getThemeManager()->getSelectedTheme()->getStaticImagesUrl($this->mobile) . substr($resultString, 0, strpos($resultString, ')')) . ')';
            }

            $randId = 'if' . rand(10, 10000000);

            $script = "$('#" . $randId . "').click(function(){
                new SC_FloatBox({\$title:'" . SC::getLanguage()->text('admin', 'themes_settings_graphics_preview_cap_label') . "', \$contents:$('#image_view_" . $this->getName() . "'), width:'550px'});
            });";
            ...
            $output .= '<div class="clearfix"><a id="' . $randId . '" href="javascript://" class="theme_control theme_control_image" style="background-image:' . $this->value . ';"></a>
                <div style="float:left;padding:10px 0 0 10px;"><a href="javascript://" onclick="window.location=\'' . SC::getRequest()->buildUrlQueryString(SC::getRouter()->urlFor('ADMIN_CTRL_Theme', 'resetGraphics'), array('name' => $this->getName())) . '\'">' . SC::getLanguage()->text('admin', 'themes_settings_reset_label') . '</a></div></div>
