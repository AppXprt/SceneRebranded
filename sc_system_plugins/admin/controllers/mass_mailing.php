...
class ADMIN_CTRL_MassMailing extends ADMIN_CTRL_Abstract
{
    ...
    public function __construct()
    {
        ...
        $this->ajaxResponderUrl = SC::getRouter()->urlFor("ADMIN_CTRL_MassMailing", "ajaxResponder");
    ...
    public function index( $params = array() )
    {
        ...
        $subject->addAttribute('class', 'sc_text');
        ...
        $body->addAttribute('class', 'sc_text');
        ...
        {
            $massMailingTimestamp = SC::getConfig()->getValue( 'admin', 'mass_mailing_timestamp' );
        ...
        if ( SC::getRequest()->isPost() && $isActive && isset($_POST['startMailing']) )
        {
            if ( $massMailingForm->isValid($_POST) )
            {
                ...
                while ( $start < $total )
                {
                    ...
                    foreach ( $result as $key => $user )
                    {
                        ...
                        $vars['unsubscribe_url'] = SC::getRouter()->urlForRoute('base_massmailing_unsubscribe', array('id' => $user->id, 'code' => $code));
                    ...
                    $start += $count;
                    //printVar($mails);
                    SC::getMailer()->addListToQueue($mails);
                }

                SC::getFeedback()->info($language->text('admin', 'massmailing_send_mails_message', array('count' => $mailCount)));

                if ( defined( 'SC_PLUGIN_XP' ) )
                {
                    SC::getConfig()->saveConfig( 'admin', 'mass_mailing_timestamp', time() );
                }
        ...
        SC::getDocument()->addScript($jsDir . "mass_mailing.js");
    }
