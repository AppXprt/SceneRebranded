...
class ADMIN_CTRL_Permissions extends ADMIN_CTRL_Abstract
{
    ...
    public function __construct()
    {
        ...
        $this->setPageHeading(SC::getLanguage()->text('admin', 'permissions_page_heading'));
        $this->setPageHeadingIconClass('sc_ic_lock');
    }
        ...
        if ( SC::getRequest()->isPost() )
        {
            if ( $form->isValid($_POST) )
            {
                ...
                else
                {
                    ...
                    $mail->setSubject(SC::getLanguage()->text( 'admin', 'site_password'));
                    $mail->setTextContent( SC::getLanguage()->text( 'admin', 'admin_password', array('password' => $data['password'])));
                    try
                    {
                ...
                SC::getFeedback()->info($language->text('admin', 'permission_global_privacy_settings_success_message'));
                $this->redirect();
            }
        }
    ...
    public function roles()
    {
        ...
        $this->assign('formAction', SC::getRouter()->urlFor(__CLASS__, 'savePermissions'));
    ...
    {
        $service = BOL_AuthorizationService::getInstance();
        $this->assign('formAction', SC::getRouter()->urlFor(__CLASS__, 'process'));
        ...
        foreach ( $moderators as $moderator )
        {
            ...
            $deleteModerUrls[$moderator->userId] = SC::getRouter()->urlFor(__CLASS__, 'deleteModerator', array('id' => $moderator->id));
        }
        ...
        {
            if ( !SC::getPluginManager()->isPluginActive($group->name) )
            {
                unset($groups[$key]);
            }
        }
        ...
        $this->assign('myModeratorId', $service->getModeratorIdByUserId(SC::getUser()->getId()));
        ...
        $this->assign('addFormAction', SC::getRouter()->urlFor(__CLASS__, 'addModerator'));
        ...
        $this->setPageHeading(SC::getLanguage()->text('admin', 'sidebar_menu_item_permission_moders'));
    }

    public function process()
    {
        if ( SC::getRequest()->isPost() && !empty($_POST['perm']) )
        {
            $perms = array();
            ...
            BOL_AuthorizationService::getInstance()->saveModeratorPermissionList($perms, SC::getUser()->getId());
        }

        SC::getFeedback()->info(SC::getLanguage()->text('admin', 'permissions_successfully_updated'));
        $this->redirect(SC::getRouter()->urlForRoute('admin_permissions_moderators'));
    }
    ...
    public function addModerator()
    {
        if ( SC::getRequest()->isPost() )
        {
            ...
            if ( $user === null )
            {
                SC::getFeedback()->error(SC::getLanguage()->text('admin', 'permissions_feedback_user_not_found'));
            }
            else
            {
                if ( BOL_AuthorizationService::getInstance()->addModerator($user->id) )
                {
                    SC::getFeedback()->info(SC::getLanguage()->text('admin', 'permissions_feedback_moderator_added'));
                }
                else
                {
                    SC::getFeedback()->warning(SC::getLanguage()->text('admin', 'permissions_feedback_user_is_already_moderator', array('username' => $username)));
                }
            }
        }
    ...
    public function deleteModerator( array $params )
    {
        ...
        if ( isset($params['id']) )
        {
            ...
            if ( $removed )
            {
                SC::getFeedback()->info(SC::getLanguage()->text('admin', 'permissions_feedback_user_kicked_from_moders'));
            }
            else
            {
                SC::getFeedback()->error(SC::getLanguage()->text('admin', 'permissions_feedback_cant_remove_moder'));
            }
        }
        else
        {
    ...
    public function savePermissions()
    {
        ...
        $this->redirect(SC::getRouter()->urlForRoute('admin_user_roles'));
    }
}