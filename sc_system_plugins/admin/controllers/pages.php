    ...
    public function maintenance()
    {
        ...
        if ( SC::getRequest()->isPost() )
        {
            if ( $form->isValid($_POST) )
            {
                ...
                SC::getConfig()->saveConfig('base', 'maintenance', (bool) $data['maintenance_enable']);
                ...
                SC::getConfig()->saveConfig('base', 'splash_leave_url', $url);
                SC::getConfig()->saveConfig('base', 'splash_screen', (bool) $data['splash_screen']);

                SC::getFeedback()->info($language->text('admin', 'maintenance_submit_success_message'));
                $this->redirect();
            }
        }

        $form->getElement('maintenance_text')->setValue($language->text('admin', 'maintenance_text_value'));
        $form->getElement('maintenance_enable')->setValue((bool) SC::getConfig()->getValue('base', 'maintenance'));
        ...
        $form->getElement('leave_url')->setValue(SC::getConfig()->getValue('base', 'splash_leave_url'));
        $form->getElement('splash_screen')->setValue((bool) SC::getConfig()->getValue('base', 'splash_screen'));
    }
...
class SaveForm extends Form
{
    ...
    public function __construct( SC_Renderable $rendrable )
    {
        ...
        $titleTextField->setLabel(SC::getLanguage()->text('admin', 'pages_edit_local_page_title'))
                ->addAttribute('class', 'sc_text');
        ...
        $nameTextField->setLabel(SC::getLanguage()->text('admin', 'pages_add_menu_name'))
                ->setRequired(true)
                ->addAttribute('class', 'sc_text');
        ...
        $visibleForCheckboxGroup->setLabel(SC::getLanguage()->text('admin', 'pages_edit_local_visible_for'));
...
//                ->setId('meta-tags')
//                ->setDescription(SC::getLanguage()->text('admin', 'pages_page_field_meta_desc'));
        ...
        $contentTextArea->setLabel(SC::getLanguage()->text('admin', 'pages_add_page_content'))
                ->setId('content')
                ->setDescription(
                        SC::getLanguage()->text('admin', 'pages_page_field_content_desc', array(
                            'src' => SC::getThemeManager()->getCurrentTheme()->getStaticImagesUrl() . 'question.png',
                            'url' => '#'
                                )
                        )
        );
        ...
        $type = (SC::getRequest()->isPost() && $_POST['type']) ? $_POST['type'] : 'local';
        ...
        $typeHiddenField->setLabel(SC::getLanguage()->text('admin', 'page_add_page_address'));
        ...
        $extOpenInNewWindow->setLabel(SC::getLanguage()->text('admin', 'pages_edit_external_url_open_in_new_window'));

        $this->addElement($extOpenInNewWindow);

        $submit = new Submit('submit');

        $this->addElement($submit->setValue(SC::getLanguage()->text('base', 'pages_add_submit')));
    }
    ...
    public function process( $params )
    {
        ...
        switch ( $_POST['type'] )
        {
            case 'local' :
                ...
                $url = str_replace(UTIL_String::removeFirstAndLastSlashes(SC::getRouter()->getBaseUrl()), '', UTIL_String::removeFirstAndLastSlashes($_POST['local-url']));
...
//~
        ...
        header('location: ' . SC::getRouter()->urlForRoute('admin_pages_main'));
        exit();
    }

}

class PageTitleValidator extends SC_Validator
{

    public function __construct()
    {
        $this->setErrorMessage(SC::getLanguage()->text('base', 'form_validator_required_error_message'));
    }
...
class LocalPageUniqueValidator extends SC_Validator
{

    public function __construct()
    {
        $this->setErrorMessage(SC::getLanguage()->text('base', 'unique_local_page_error'));
    }
...
class LocalPageUrlValidator extends SC_Validator
{
    ...
    public function isValid( $value )
    {
        ...
        $value = str_replace(UTIL_String::removeFirstAndLastSlashes(SC::getRouter()->getBaseUrl()), '', UTIL_String::removeFirstAndLastSlashes($value));
    ...
    public function getError()
    {
        if ( $this->errCode == 2 )
        {
            return SC::getLanguage()->text('base', 'pages_wrong_local_url');
        }
        ...
        return "{
        	validate : function( value ){
                if( $('#address2').attr('checked') &&  !value ){
                	throw " . json_encode(SC::getLanguage()->text('base', 'form_validator_required_error_message')) . ";
    			}

                return true;
        },
        	getErrorMessage : function(){ return " . json_encode(SC::getLanguage()->text('base', 'form_validator_required_error_message')) . " }
        }";
    }
