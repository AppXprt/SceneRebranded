...
class ADMIN_CTRL_Plugins extends ADMIN_CTRL_StorageAbstract
{
    ...
    {
        SC::getNavigation()->activateMenuItem(SC_Navigation::ADMIN_PLUGINS, "admin", "sidebar_menu_plugins_installed");
        ...
        foreach ( $plugins as $plugin )
        {
            ...
            if ( $plugin->getLicenseCheckTimestamp() > 0 )
            {
                ...
                $array["license_url"] = SC::getRequest()->buildUrlQueryString($router->urlFor("ADMIN_CTRL_Storage",
        ...
        $event = new SC_Event("admin.plugins_list_view", array("ctrl" => $this, "type" => "index"), $arrayToAssign);
    ...
    {
        SC::getNavigation()->activateMenuItem(SC_Navigation::ADMIN_PLUGINS, "admin", "sidebar_menu_plugins_available");
        ...
        foreach ( $arrayToAssign as $key => $plugin )
        {
            ...
            $arrayToAssign[$key]["inst_url"] = SC::getRequest()->buildUrlQueryString(SC::getRouter()->urlFor(__CLASS__,
                    "install"), $params);
            $arrayToAssign[$key]["del_url"] = SC::getRouter()->urlFor(__CLASS__, "delete",
                array("key" => $plugin["key"]));
        }

        $event = new SC_Event("admin.plugins_list_view", array("ctrl" => $this, "type" => "available"), $arrayToAssign);
        SC::getEventManager()->trigger($event);
        $arrayToAssign = $event->getData();
        $this->assign("plugins", $arrayToAssign);
    }
    ...
    public function add()
    {
        SC::getNavigation()->activateMenuItem(SC_Navigation::ADMIN_PLUGINS, "admin", "sidebar_menu_plugins_add");
        ...
        if ( SC::getRequest()->isPost() )
        {
            if ( $form->isValid($_POST) )
            {
                ...
                $pluginfilesDir = SC::getPluginManager()->getPlugin("base")->getPluginFilesDir();
                ...
                $this->redirect(SC::getRequest()->buildUrlQueryString(SC::getRouter()->urlFor(__CLASS__, "processAdd"),
    ...
    public function processAdd()
    {
        ...
        if ( $pluginWithSameKey !== null )
        {
            ...
            {
                $pluginDir = SC_DIR_PLUGIN . $pluginWithSameKey->getModule() . DS;
            }
            else
            {
                // show error, can't have 2 plugins with the same key
                SC::getFeedback()->error(SC::getLanguage()->text("admin", "manage_plugin_cant_add_duplicate_key_error"));
                $this->redirectToAction("index");
            }
        }
        else
        {
            ...
            if ( $pluginDir == null )
            {
                $pluginDir = SC_DIR_PLUGIN . basename($localPluginRootPath);
        ...
        SC::getFeedback()->info($language->text("base", "manage_plugins_add_success_message"));
    ...
    public function deactivate( array $params )
    {
        ...
        SC::getFeedback()->info($language->text('admin', 'manage_plugins_deactivate_success_message',
    ...
    public function activate( array $params )
    {
        $pluginDto = $this->getPluginDtoByKeyInParamsArray($params);
        $language = SC::getLanguage();
        $urlRedirect = SC::getRouter()->urlForRoute("admin_plugins_installed");
        ...
        foreach ( $invalidItems as $item )
        {
            ...
            {
                SC::getFeedback()->error($language->text("admin",
        ...
        SC::getFeedback()->info($language->text("admin", "manage_plugins_activate_success_message",
                array("plugin" => $pluginDto->getTitle())));
        $this->redirect($urlRedirect);
    }
    ...
    public function updateRequest( array $params )
    {
                ...
                $pluginDto->getDeveloperKey(), $pluginDto->getLicenseKey())) )
        {
            if ( !isset($_GET[BOL_StorageService::URI_VAR_LICENSE_CHECK_COMPLETE]) )
            {
                ...
                $this->redirect(SC::getRequest()->buildUrlQueryString($router->urlFor("ADMIN_CTRL_Storage",
        ...
        if ( SC::getConfig()->getValue("base", "update_soft") )
        {
            $this->assign("platformUpdateAvail", true);
            $this->assign("platformUpdateUrl", SC::getRouter()->urlForRoute("admin_core_update_request"));
        }
    }
    ...
    public function update( array $params )
    {
        ...
        $language = SC::getLanguage();
        $feedback = SC::getFeedback();
        $redirectUrl = SC::getRouter()->urlForRoute("admin_plugins_installed");
        ...
        if ( !empty($_GET["mode"]) )
        {
            switch ( trim($_GET["mode"]) )
            {
                ...
                case "plugin_update_success":
                    SC::getEventManager()->trigger(new SC_Event(SC_EventManager::ON_AFTER_PLUGIN_UPDATE,
        ...
        {
            $feedback->error(SC::getLanguage()->text("admin", "plugin_update_download_error"));
            $this->redirect($redirectUrl);
        }

        $zip = new ZipArchive();
        $tempDirPath = SC::getPluginManager()->getPlugin("base")->getPluginFilesDir() . "plugin_update" . UTIL_String::getRandomString(5,
        ...
        try
        {
            $plugin = SC::getPluginManager()->getPlugin($pluginDto->getKey());
        }
        ...
        $params = array("plugin" => $pluginDto->getKey(), "back-uri" => urlencode(SC::getRequest()->getRequestUri()));
        $this->redirect(SC::getRequest()->buildUrlQueryString($this->storageService->getUpdaterUrl(), $params));
    }
    ...
    public function manualUpdateRequest( array $params )
    {
        $language = SC::getLanguage();
        $feedback = SC::getFeedback();
        $urlToRedirect = SC::getRouter()->urlForRoute("admin_plugins_installed");
        $pluginDto = null;
        ...
        {
            $feedback->warning(SC::getLanguage()->text("admin", "no_plugins_for_manual_updates"));
            $this->redirect($urlToRedirect);
        }

        $this->assign("text",
            $language->text("admin", "manage_plugins_manual_update_request", array("name" => $pluginDto->getTitle())));
        $params = array("plugin" => $pluginDto->getKey(), "back-uri" => urlencode(SC::getRequest()->getRequestUri()), "addParam" => UTIL_String::getRandomString());
        $this->assign("redirectUrl",
            SC::getRequest()->buildUrlQueryString($this->storageService->getUpdaterUrl(), $params));
    }
    ...
    public function install()
    {
        ...
        else
        {
            ...
            {
                $params[BOL_StorageService::URI_VAR_BACK_URI] = urlencode(SC::getRequest()->getRequestUri());
                $params["back-button-uri"] = urlencode(SC::getRouter()->uriForRoute("admin_plugins_available"));
        ...
        if ( $installPlugin )
        {
            ...
            {
                SC::getLogger()->addEntry($e->getTraceAsString());
    ...
    public function uninstall( array $params )
    {
        ...
        if ( empty($params["key"]) )
        {
            SC::getFeedback()->error($language->text('admin', 'manage_plugins_uninstall_error_message'));
            $this->redirect(SC::getRouter()->urlForRoute("admin_plugins_installed"));
        }
        ...
        SC::getFeedback()->info($language->text("admin", "manage_plugins_uninstall_success_message",
                array("plugin" => $pluginDto->getTitle())));
        $this->redirect(SC::getRouter()->urlForRoute("admin_plugins_installed"));
    }
    ...
    public function uninstallRequest( array $params )
    {
        ...
        $this->assign("redirectUrl", SC::getRouter()->urlFor("ADMIN_CTRL_Plugins", "uninstall", $params));
    }
    ...
    public function delete( array $params )
    {
        ...
        {
            SC::getFeedback()->error(SC::getLanguage()->text("admin", "manage_plugins_plugin_not_found"));
            $this->redirectToAction('available');
        }

        $ftp->rmDir($availablePlugins[$key]["path"]);

        SC::getFeedback()->info(SC::getLanguage()->text("admin", "manage_plugins_delete_success_message",
