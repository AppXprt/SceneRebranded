...
class ADMIN_CMP_EditQuestionValueLabel extends SC_Component
{
    public function  __construct( $value, $languageData )
    {
        parent::__construct();
        $this->setTemplate(SC::getPluginManager()->getPlugin('base')->getCmpViewDir() . 'language_value_edit.html');
        ...
        $jsString = '
           ...
           scForms[{$formName}].submitForm = function(){
            ...
            scForms[{$formName}].bind("submit", function($data){
                SC.trigger("admin.questions_edit_question_value", [$data], this);
                return false;
            })';
