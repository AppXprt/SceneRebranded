...
class ADMIN_CMP_AddQuestion extends SC_Component
{
    ...
    public function __construct()
    {
        ...
        $addForm = SC::getClassInstance('ADMIN_CLASS_AddQuestionForm', 'qst_add_form', SC::getRouter()->urlFor("ADMIN_CTRL_Questions", "ajaxResponder"));
       ...
       SC.bind("admin.questions_edit_question_value", function(data) {
            ...
            if ( storage && data )
            {
                ...
                $.each(data , function( key, value ) {
                    ...
                    if ( match && match[0] && value )
                    {
                        ...
                        if ( langId == ' . json_encode(SC::getLanguage()->getCurrentId()) . ' )
                        {
        ...
        SC.bind("question.value.add", function(data) {
            var currentLang = ' . json_encode(SC::getLanguage()->getCurrentId()) . ';
        ...
        SC.bind("question.value.delete", function(data) {
        ...
        $("form[name=qst_add_form]").on( "dblclick", ".values_list .tag .label", function() {
            ...
            SC.ajaxFloatBox("ADMIN_CMP_EditQuestionValueLabel", [value, data[value]], { title: ' . json_encode(SC::getLanguage()->text('base', 'questions_edit_question_value_title')) . ' } );
        } );
