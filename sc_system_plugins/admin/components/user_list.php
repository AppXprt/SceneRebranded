 ...
 * @package ow.sc_system_plugins.admin.components
 * @since 1.0
 */
class ADMIN_CMP_UserList extends SC_Component
{    
    /**
     * Constructor.
     * 
    ...
    public function __construct( ADMIN_UserListParams $params )
    {
        ...
        // handle form
        if ( SC::getRequest()->isPost() && !empty($_POST['users']) )
        {
            $users = $_POST['users'];
            ...
            if ( isset($_POST['suspend']) )
            {
                ...
                SC::getFeedback()->info($language->text('admin', 'user_feedback_profiles_suspended'));
            }
            else if ( isset($_POST['reactivate']) )
            {
                ...
                SC::getFeedback()->info($language->text('admin', 'user_feedback_profiles_unsuspended'));                
            }
            else if ( isset($_POST['delete']) )
            {
                ...
                SC::getFeedback()->info($language->text('admin', 'user_delete_msg', array('count' => $deleted)));
            }
            else if ( isset($_POST['email_verify']) )
            {
                ...
                SC::getFeedback()->info($language->text('admin', 'user_feedback_email_verified'));
            }
            else if ( isset($_POST['email_unverify']) )
            {
                ...
                SC::getFeedback()->info($language->text('admin', 'user_feedback_email_unverified'));
            }
            else if ( isset($_POST['disapprove']) )
            {
                ...
                SC::getFeedback()->info($language->text('admin', 'user_feedback_profiles_disapproved'));
            }
            else if ( isset($_POST['approve']) )
            {
                ...
                SC::getFeedback()->info($language->text('admin', 'user_feedback_profiles_approved'));
            }
        ...
        $event = new SC_Event('admin.amount_users_per_page', array(), array("amountOnPage" => $onPage));
        ...
        if ( !$userList && $page > 1 )
        {
            SC::getApplication()->redirect(SC::getRequest()->buildUrlQueryString(null, array('page' => $page - 1)));
        }
        ...
        $this->assign('adminId', SC::getUser()->getId());
    ...
    private function reloadParentPage()
    {
        ...
        SC::getApplication()->redirect(SC::getRequest()->buildUrlQueryString());
    }
}
