 ...
 */
class BASE_Cron extends SC_Cron
{
    ...
    public function generateSitemap()
    {
        ...
        $inProgress = (int) SC::getConfig()->getValue('base', 'seo_sitemap_in_progress');
        $inProgressTime = (int) SC::getConfig()->getValue('base', 'seo_sitemap_in_progress_time');

        // is it possible to start sitemap generating?
        if ( (!$inProgress || time() - $inProgressTime >= 3600) && $service->isSitemapReadyForNextBuild() )
        {
            SC::getConfig()->saveConfig('base', 'seo_sitemap_in_progress', 1);
            SC::getConfig()->saveConfig('base', 'seo_sitemap_in_progress_time', time());

            $service->generateSitemap();

            SC::getConfig()->saveConfig('base', 'seo_sitemap_in_progress', 0);
        }
    }
    ...
    {
        if ( SC::getConfig()->configExists('base', 'cron_is_active') && (int) SC::getConfig()->getValue('base', 'cron_is_active') === 0 )
        {
            SC::getConfig()->saveConfig('base', 'cron_is_active', 1);
        }
    }
    ...
    {
        SC::getCacheManager()->clean(array(), SC_CacheManager::CLEAN_OLD);
    }
    ...
    public function dropLogFile()
    {
        ...
        if ( file_exists($logFilePath) )
        {
            ...
            if ( $logFileSize !== false && $logFileSize / 1024 / 1024 >= (int) SC::getConfig()->getValue('base', 'log_file_max_size_mb') )
            {
                unlink($logFilePath);
            }
        }
