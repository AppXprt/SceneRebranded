 ...
 * @since 1.0
 */
require( SC_DIR_LIB . 'amazonS3' . DS . 'S3.php' );

class BASE_CLASS_AmazonCloudStorage implements SC_Storage
{
    ...
    {
        $this->cloudfilesTmpDir = SC_DIR_PLUGINFILES . 'base' . DS . 'cloudfiles' . DS;

        // Connect to Rackspace Cloud Files
        $this->s3 = new S3(SC_AMAZON_S3_ACCESS_KEY, SC_AMAZON_S3_SECRET_KEY);
        $this->bucketName = SC_AMAZON_S3_BUCKET_NAME;
        $this->bucketUrl = SC_AMAZON_S3_BUCKET_URL;
    ...
    public function removeFile( $path )
    {
        ...
        $object = $this->s3->getObjectInfo($this->bucketName, $cloudPath);
        $result = $this->s3->deleteObject($this->bucketName, $cloudPath);

        $this->triggerFileDeleteEvent($path, $object['size']);
    ...
    private function getCloudFilePath( $path )
    {
        ...
        {
            $cloudPath = str_replace(SC_DIR_ROOT, '', $path);
    ...
    private function getLocalFSPath( $cloudPath )
    {
        ...
        $result = SC_DIR_ROOT . str_replace('/', DS, $cloudPath);
    ...
    private function triggerFileUploadEvent( $path, $size )
    {
        ...
        $event = new SC_Event(self::EVENT_ON_FILE_UPLOAD, $params);
        SC::getEventManager()->trigger($event);
    }

    private function triggerFileDeleteEvent( $path, $size )
    {
        if ( empty($path) )
        {
            return;
        ...
        $event = new SC_Event(self::EVENT_ON_FILE_DELETE, $params);
        SC::getEventManager()->trigger($event);
    }
