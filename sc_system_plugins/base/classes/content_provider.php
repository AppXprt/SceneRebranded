...
class BASE_CLASS_ContentProvider
{
    ...
    public function onCollectTypes( BASE_CLASS_EventCollector $event )
    {
        $event->add(array(
            ...
            "groupLabel" => SC::getLanguage()->text("base", "content_profiles_label"),
            "entityLabel" => SC::getLanguage()->text("base", "content_profile_label"),
            "displayFormat" => "empty"
        ));
        ...
        $event->add(array(
            ...
            "groupLabel" => SC::getLanguage()->text("base", "content_comments_label"),
            "entityLabel" => SC::getLanguage()->text("base", "content_comment_label"),
        ...
        $event->add(array(
            ...
            "groupLabel" => SC::getLanguage()->text("base", "content_avatars_label"),
            "entityLabel" => SC::getLanguage()->text("base", "content_avatar_label")
        ));
    }
    
    public function onGetInfo( SC_Event $event )
    {
    ...
    private function getCommentInfo( $commentIds )
    {
        ...
        foreach ( $comments as $comment )
        {
            ...
            if ( !empty($contentInfo) )
            {
                ...
                $info["label"] = SC::getLanguage()->text("base", "comment_content_label", array(
    ...
    public function onUpdateInfo( SC_Event $event )
    {
        $params = $event->getParams();
        $data = $event->getData();
    ...
    private function updateAvatar( $data )
    {
        foreach ( $data as $avatarId => $info )
        {
            ...
            if ( $avatar->status != $info['status'] )
            {
                ...
                $event = new SC_Event('base.after_avatar_update', $params);
                SC::getEventManager()->trigger($event);
            }
        }
    }
    
    public function onDelete( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onCommentAdd( SC_Event $event )
    {
        $params = $event->getParams();
        ...
        SC::getEventManager()->trigger(new SC_Event(BOL_ContentService::EVENT_AFTER_ADD, array(
            "entityType" => self::ENTITY_TYPE_COMMENT,
    ...
    public function onUserJoin( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function afterUserEdit( SC_Event $event )
    {
        $params = $event->getParams();
        ...
        if ( $isModerate ) {
            ...
            SC::getEventManager()->trigger(new SC_Event(BOL_ContentService::EVENT_AFTER_CHANGE, array(
                "entityType" => self::ENTITY_TYPE_PROFILE,
                "entityId" => $userId
            ), array(
    ...
    public function onUserDeleted( SC_Event $event )
    {
        $params = $event->getParams();
        $userId = $params["userId"];
        
        SC::getEventManager()->trigger(new SC_Event(BOL_ContentService::EVENT_BEFORE_DELETE, array(
            "entityType" => self::ENTITY_TYPE_PROFILE,
            "entityId" => $userId
        )));
    }
    
    public function onUserApprove( SC_Event $event )
    {
        $params = $event->getParams();
        $userId = $params["userId"];
        
        SC::getEventManager()->trigger(new SC_Event("moderation.approve", array(
            "entityType" => self::ENTITY_TYPE_PROFILE,
            "entityId" => $userId
        )));
    }
    ...
    public function onAvatarChange( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onAvatarDelete( SC_Event $event )
    {
        $params = $event->getParams();
        $avatarId = $params["avatarId"];
    ...
    {
        SC::getEventManager()->bind('base.after_avatar_update', array($this, "onAvatarChange"), 10000);
        SC::getEventManager()->bind('base.before_user_avatar_delete', array($this, "onAvatarDelete"));
        
        SC::getEventManager()->bind(SC_EventManager::ON_USER_APPROVE, array($this, "onUserApprove"));
        SC::getEventManager()->bind(SC_EventManager::ON_USER_EDIT, array($this, "afterUserEdit"));
        
        SC::getEventManager()->bind(SC_EventManager::ON_USER_REGISTER, array($this, "onUserJoin"));
        SC::getEventManager()->bind(SC_EventManager::ON_USER_UNREGISTER, array($this, "onUserDeleted"));
        SC::getEventManager()->bind("base_add_comment", array($this, "onCommentAdd"));
        
        SC::getEventManager()->bind(BOL_ContentService::EVENT_COLLECT_TYPES, array($this, "onCollectTypes"));
        SC::getEventManager()->bind(BOL_ContentService::EVENT_GET_INFO, array($this, "onGetInfo"));
        SC::getEventManager()->bind(BOL_ContentService::EVENT_UPDATE_INFO, array($this, "onUpdateInfo"));
        SC::getEventManager()->bind(BOL_ContentService::EVENT_DELETE, array($this, "onDelete"));
    }
}