...
class BASE_CLASS_InvitationEventHandler
{
    ...
    public function collectItems( BASE_CLASS_ConsoleItemCollector $event )
    {
        ...
        $allInvitationCount = $this->service->findInvitationCount(SC::getUser()->getId());
        $item->setIsHidden( empty($allInvitationCount) );

        $event->addItem($item, 6);
    }

    public function addInvitation( SC_Event $event )
    {
        ...
        else
        {
            ...
            $dublicateEvent = new SC_Event('invitations.on_dublicate', $dublicateParams, $data);
            SC::getEventManager()->trigger($dublicateEvent);
    ...
    public function removeInvitation( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function loadList( BASE_CLASS_ConsoleListEvent $event )
    {
        ...
        {
            $itemEvent = new SC_Event('invitations.on_item_render', array(
            ...
            SC::getEventManager()->trigger($itemEvent);
    ...
    private function processDataInterface( $params, $data )
    {
        ...
        foreach ( array('string', 'conten') as $langProperty )
        {
            if ( !empty($data[$langProperty]) && is_array($data[$langProperty]) )
            {
                ...
                {
                    $data[$langProperty] = SC::getLanguage()->text($key[0], $key[1], $vars);
                }
            }
        }
    ...
    public function renderItem( SC_Event $event )
    {
        $params = $event->getParams();
        $data = $event->getData();
        ...
        if ( $interface['viewed'] )
        {
            $item->addClass('sc_console_new_message');
        }
    ...
    public function pluginActivate( SC_Event $e )
    {
        $params = $e->getParams();
    ...
    public function pluginDeactivate( SC_Event $e )
    {
        $params = $e->getParams();
        $pluginKey = $params['pluginKey'];
    ...
    public function pluginUninstall( SC_Event $e )
    {
        $params = $e->getParams();
        $pluginKey = $params['pluginKey'];
    ...
    {
        SC::getEventManager()->bind('invitations.on_item_render', array($this, 'renderItem'));
        SC::getEventManager()->bind('invitations.add', array($this, 'addInvitation'));

        SC::getEventManager()->bind('invitations.remove', array($this, 'removeInvitation'));
    }

    public function init()
    {
        SC::getEventManager()->bind(SC_EventManager::ON_PLUGINS_INIT, array($this, 'afterInits'));

        SC::getEventManager()->bind(SC_EventManager::ON_AFTER_PLUGIN_ACTIVATE, array($this, 'pluginActivate'));
        SC::getEventManager()->bind(SC_EventManager::ON_BEFORE_PLUGIN_DEACTIVATE, array($this, 'pluginDeactivate'));
        SC::getEventManager()->bind(SC_EventManager::ON_BEFORE_PLUGIN_UNINSTALL, array($this, 'pluginUninstall'));

        SC::getEventManager()->bind('console.load_list', array($this, 'loadList'));
        SC::getEventManager()->bind('console.ping', array($this, 'ping'));
        SC::getEventManager()->bind('console.collect_items', array($this, 'collectItems'));

        SC::getEventManager()->bind('notifications.send_list', array($this, 'sendList'));
    }
