...
class BASE_CLASS_EventHandler
{
    ...
    public function genericInit()
    {
        ...
        $eventManager->bind(SC_EventManager::ON_USER_LOGIN, array($this, 'onUserLoginSaveStatistics'));
        ...
        $eventManager->bind(SC_EventManager::ON_USER_UNREGISTER, array($this, 'onDeleteUserContent'));
        $eventManager->bind(SC_EventManager::ON_USER_LOGIN, array($this, 'onUserLogin'));
        $eventManager->bind(SC_EventManager::ON_USER_LOGOUT, array($this, 'onUserLogout'));
        $eventManager->bind(SC_EventManager::ON_USER_REGISTER, array($this, 'onJoinMandatoryUserApprove'));
        $eventManager->bind(SC_EventManager::ON_USER_EDIT, array($this, 'onUserEditFeed'));
        $eventManager->bind(SC_EventManager::ON_USER_REGISTER, array($this, 'onJoinFeed'));
        ...
        $eventManager->bind(SC_EventManager::ON_USER_REGISTER, array($this, 'onUserRegisterWelcomeLetter'));
        $eventManager->bind(SC_EventManager::ON_USER_UNREGISTER, array($this, 'onUserUnregisterDeleteDisaproved'));
        ...
        $eventManager->bind(SC_EventManager::ON_USER_UNREGISTER, array($this, 'onUserUnregisterRemovePreference'));
        $eventManager->bind('base.update_entity_items_status', array($this, 'onUpdateEntityItemsStatus'));
        $eventManager->bind(SC_EventManager::ON_USER_UNREGISTER, array($this, 'onDeleteMediaPanelFiles'));
        $eventManager->bind(SC_EventManager::ON_USER_UNREGISTER, array($this, 'clearUserListQueryCache'));
        $eventManager->bind(SC_EventManager::ON_USER_SUSPEND, array($this, 'clearUserListQueryCache'));
        $eventManager->bind(SC_EventManager::ON_USER_SUSPEND, array($this, 'sendSuspendNotification'));
        $eventManager->bind(SC_EventManager::ON_USER_UNSUSPEND, array($this, 'clearUserListQueryCache'));
        $eventManager->bind(SC_EventManager::ON_USER_APPROVE, array($this, 'clearUserListQueryCache'));
        $eventManager->bind(SC_EventManager::ON_USER_DISAPPROVE, array($this, 'clearUserListQueryCache'));
        $eventManager->bind(SC_EventManager::ON_USER_MARK_FEATURED, array($this, 'clearUserListQueryCache'));
        $eventManager->bind(SC_EventManager::ON_USER_UNMARK_FEATURED, array($this, 'clearUserListQueryCache'));
        ...
        $eventManager->bind(SC_EventManager::ON_USER_LOGIN, array($this, 'onUserLoginSetAdminCookie'));
        ...
        $eventManager->bind(SC_EventManager::ON_USER_UNREGISTER, array($this, 'onUserUnregisterClearMailQueue'));

        $eventManager->bind('socialsharing.get_entity_info', array($this, 'sosialSharingGetUserInfo'));

        $eventManager->bind(SC_EventManager::ON_USER_REGISTER, array($this, 'setAccountTypeUserRoleOnUserRegister'));
        $eventManager->bind(SC_EventManager::ON_USER_REGISTER, array($this, 'deleteInviteCode'));
        ...
        $eventManager->bind(SC_EventManager::ON_JOIN_FORM_RENDER, array($this, 'onInviteMembersProcessJoinForm'));
    ...
    public function init()
    {
        ...
        $eventManager->bind(SC_EventManager::ON_FINALIZE, array($this, 'addJsDeclarations'));
        ...
        $eventManager->bind(SC_EventManager::ON_BEFORE_PLUGIN_UNINSTALL, array($this, 'onPluginUninstallDeleteComments'));
        ...
        $eventManager->bind(SC_EventManager::ON_BEFORE_PLUGIN_UNINSTALL, array($this, 'onPluginsUninstallDeleteGateway'));
        $eventManager->bind(SC_EventManager::ON_USER_UNREGISTER, array($this, 'onDeleteUserAttachments'));
        $eventManager->bind(SC_EventManager::ON_FINALIZE, array($this, 'onFinalizeAddScrollJs'));
        $eventManager->bind('join.get_captcha_field', array($this, 'getCaptcha'));
        $eventManager->bind(SC_EventManager::ON_FINALIZE, array($this, 'onFinalizeCheckIfSiteFullyInstalled'));
        $eventManager->bind(SC_EventManager::ON_AFTER_ROUTE, array($this, 'onPluginsInitCheckUserStatus'));
    ...
    public function onCollectQuickLinks( BASE_CLASS_EventCollector $event )
    {
        ...
        if ( $userId )
        {
            ...
            {
                $event->add(array(
                    BASE_CMP_QuickLinksWidget::DATA_KEY_LABEL => SC::getLanguage()->text('base', 'my_blocked_users'),
                    BASE_CMP_QuickLinksWidget::DATA_KEY_URL => SC::getRouter()->urlForRoute('users-blocked'),
                    BASE_CMP_QuickLinksWidget::DATA_KEY_COUNT => $blockedCount,
                    BASE_CMP_QuickLinksWidget::DATA_KEY_COUNT_URL => SC::getRouter()->urlForRoute('users-blocked')
                ));
            }
        }
    }
     ...
     * Get sitemap urls
     *
     * @param SC_Event $event
     * @return void
     */
    public function onSitemapGetUrls( SC_Event $event )
    {
        ...
        switch ( $params['entity'] )
        {
            ...
            case 'base_pages' :
                ...
                $urls = array(
                    SC_URL_HOME,
                    SC::getRouter()->urlForRoute('base.mobile_version'),
                    SC::getRouter()->urlForRoute('base_join'),
                    SC::getRouter()->urlForRoute('static_sign_in'),
                    SC::getRouter()->urlForRoute('base_forgot_password')
                );
            ...
            case 'user_list' :
                ...
                {
                    $event->setData(array(
                        SC::getRouter()->urlForRoute('users'),
                        SC::getRouter()->urlForRoute('base_user_lists', array(
                            'list' => 'latest'
                        )),
                        SC::getRouter()->urlForRoute('base_user_lists', array(
                            'list' => 'featured'
    ...
    public function onAfterAdd( SC_Event $event )
    {
        $params = $event->getParams();
        $entityTypes = explode(',',
            SC::getConfig()->getValue('base', 'site_statistics_disallowed_entity_types'));
    ...
    public function onGetClassInstance( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function deleteEntityFlags( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onCollectModerationWidgetContent( BASE_CLASS_EventCollector $event )
    {
        ...
        $event->add(array(
            "name" => "flags",
            "label" => SC::getLanguage()->text("base", "flagged_content"),
    ...
    public function onCollectModerationToolsMenu( BASE_CLASS_EventCollector $event )
    {
        ...
        $event->add(array(
            "url" => SC::getRouter()->urlForRoute("base.moderation_flags_index"),
            "label" => SC::getLanguage()->text("base", "flagged_content"),
            "iconClass" => "sc_ic_clock",
            "key" => "flags"
        ));
    }

    public function deleteInviteCode( SC_Event $e )
    {
        $params = $e->getParams();
    ...
    public function onEmergencyExit( SC_Event $e )
    {
        if ( !empty($_COOKIE['adminToken']) && trim($_COOKIE['adminToken']) == SC::getConfig()->getValue('base', 'admin_cookie') )
        {
            SC::getSession()->set('errorData', serialize($e->getParams()));
        }
    }

    public function onAfterBillingDeliverSale( SC_Event $event )
    {
        $params  = $event->getParams();
    ...
    public function onUserLoginSaveStatistics( SC_Event $event )
    {
        $params = $event->getParams();
        BOL_SiteStatisticService::getInstance()->addEntity('user_login', $params['userId']);
    }

    public function onUserLoginSetAdminCookie( SC_Event $event )
    {
        $params = $event->getParams();
        ...
        if ( BOL_AuthorizationService::getInstance()->isSuperModerator($params['userId']) )
        {
            ...
            SC::getConfig()->saveConfig('base', 'admin_cookie', $newToken);
            setcookie('adminToken', $newToken, time() + 3600 * 24 * 100, '/', null, false, true);
        }
    }

    public function onUserLogout( SC_Event $e )
    {
        $params = $e->getParams();
    ...
    public function onUserLogin( SC_Event $e )
    {
        $params = $e->getParams();
        $userId = (int) $params['userId'];
        ...
        BOL_UserService::getInstance()->onLogin($userId, SC::getApplication()->getContext());
    }
    ...
    public function onPluginsInitCheckUserStatus()
    {
        ...
        {
            $user = BOL_UserService::getInstance()->findUserById(SC::getUser()->getId());

            if ( SC::getConfig()->getValue('base', 'mandatory_user_approve') && SC::getUser()->isAuthenticated() && !BOL_UserService::getInstance()->isApproved() )
            {
                SC::getRequestHandler()->setCatchAllRequestsAttributes('base.wait_for_approval', array('controller' => 'BASE_CTRL_WaitForApproval', 'action' => 'index'));
                SC::getRequestHandler()->addCatchAllRequestsExclude('base.wait_for_approval', 'BASE_CTRL_User', 'signOut');
            }

            if ( $user !== null )
            {
                if ( BOL_UserService::getInstance()->isSuspended($user->getId()) && !SC::getUser()->isAdmin() )
                {
                    SC::getRequestHandler()->setCatchAllRequestsAttributes('base.suspended_user', array('controller' => 'BASE_CTRL_SuspendedUser', 'action' => 'index'));
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_CTRL_User', 'signOut');
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_CTRL_Avatar');
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_CTRL_Edit');
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_CTRL_DeleteUser');
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_CTRL_Captcha');
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_CTRL_Console');
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_CTRL_AjaxLoader');
                }

                if ( (int) $user->emailVerify === 0 && SC::getConfig()->getValue('base', 'confirm_email') )
                {
                    SC::getRequestHandler()->setCatchAllRequestsAttributes('base.email_verify', array(SC_RequestHandler::CATCH_ALL_REQUEST_KEY_CTRL => 'BASE_CTRL_EmailVerify', SC_RequestHandler::CATCH_ALL_REQUEST_KEY_ACTION => 'index'));

                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.email_verify', 'BASE_CTRL_User', 'signOut');
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.email_verify', 'BASE_CTRL_EmailVerify');
                }
                ...
                $attrs = SC::getRequestHandler()->getHandlerAttributes();
                if ( !empty($attrs[SC_RequestHandler::ATTRS_KEY_CTRL]) )
                {
                    $parents = class_parents($attrs[SC_RequestHandler::ATTRS_KEY_CTRL], true);
                ...
                if ( !$isAdminUrl )
                {
                    ...
                    {
                        SC::getRequestHandler()->setCatchAllRequestsAttributes('base.complete_profile.account_type', array('controller' => 'BASE_CTRL_CompleteProfile', 'action' => 'fillAccountType'));
                        SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'BASE_CTRL_Console', 'listRsp');
                        SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'BASE_CTRL_User', 'signOut');
                        SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'INSTALL_CTRL_Install');
                        SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'BASE_CTRL_BaseDocument', 'installCompleted');
                        SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'BASE_CTRL_AjaxLoader');
                        SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'BASE_CTRL_AjaxComponentAdminPanel');
                    }
                    else
                    {
                        $questionsEditStamp = SC::getConfig()->getValue('base', 'profile_question_edit_stamp');
                        $updateDetailsStamp = BOL_PreferenceService::getInstance()->getPreferenceValue('profile_details_update_stamp', SC::getUser()->getId());

                        if ( $questionsEditStamp >= (int) $updateDetailsStamp )
                        {
                            require_once SC_DIR_CORE . 'validator.php';
                            $questionList = BOL_QuestionService::getInstance()->getEmptyRequiredQuestionsList($user->id);

                            if ( !empty($questionList) )
                            {
                                SC::getRequestHandler()->setCatchAllRequestsAttributes('base.complete_profile', array('controller' => 'BASE_CTRL_CompleteProfile', 'action' => 'fillRequiredQuestions'));
                                SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'BASE_CTRL_Console', 'listRsp');
                                SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'BASE_CTRL_User', 'signOut');
                                SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'INSTALL_CTRL_Install');
                                SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'BASE_CTRL_BaseDocument', 'installCompleted');
                                SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'BASE_CTRL_AjaxLoader');
                                SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'BASE_CTRL_AjaxComponentAdminPanel');
                            }
                            else
                            {
                                BOL_PreferenceService::getInstance()->savePreferenceValue('profile_details_update_stamp', time(), SC::getUser()->getId());
                            }
                        }
                    }
                }
    ...
    public function addPageBanner( BASE_CLASS_EventCollector $event )
    {
        ...
        {
            $event->add(SC_ADS_XP_TOP);
        }
        elseif ( $params['key'] == 'base.content_bottom' )
        {
            $event->add(SC_ADS_XP_BOT);
        }
    }

    public function getQuestionLabel( SC_Event $event )
    {
        $params = $event->getParams();
        ...
        {
            $event->setData(SC::getLanguage()->text('base', 'questions_question_birthday_label_presentation_age'));
        }
    }

    public function onFinalizeCheckIfSiteFullyInstalled( SC_Event $event )
    {
        if ( !SC::getConfig()->getValue("base", "install_complete") )
        {
            SC::getApplication()->redirect(SC::getRouter()->urlForRoute("base_page_install_completed") . "?redirect=1");
        }
    }

    public function getCaptcha( SC_Event $e )
    {
    ...
    public function onFinalizeAddScrollJs( $e )
    {
        ...
        SC::getDocument()->addScript($plugin->getStaticJsUrl() . 'jquery.mousewheel.js');
        SC::getDocument()->addScript($plugin->getStaticJsUrl() . 'jquery.jscrollpane.js');
    }

    public function clearUserListQueryCache( SC_Event $event )
    {
        $params = $event->getParams();
        $userId = (int) $params['userId'];

        SC::getCacheManager()->clean(array(BOL_UserDao::CACHE_TAG_ALL_USER_LIST));
    }

    public function sendSuspendNotification( SC_Event $event )
    {
        $params = $event->getParams();
        $userId = (int) $params['userId'];
        ...
        $txt = SC::getLanguage()->text('base', 'suspend_notification_text', array('realName' => $displayName, 'suspendReason' => $message));
        $html = SC::getLanguage()->text('base', 'suspend_notification_html', array('realName' => $displayName, 'suspendReason' => $message));

        $subject = SC::getLanguage()->text('base', 'suspend_notification_subject');
    ...
    public function onDeleteUserAttachments( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onDeleteMediaPanelFiles( SC_Event $e )
    {
        $params = $e->getParams();
        $userId = (int) $params['userId'];
    ...
    public function onPluginsUninstallDeleteGateway( SC_Event $e )
    {
        $params = $e->getParams();
    ...
    public function onSaveAttachmentImage( SC_Event $event )
    {
        $params = $event->getParams();
...
//    public function onDeleteAttachmentImage( SC_Event $event )
//    {
//        $params = $event->getParams();
//
//        if ( !empty($params["id"]) )
...
//        }
//        else if ( !empty($params['url']) && strstr($params['url'], SC::getStorage()->getFileUrl(SC::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'attachments')) )
//        {
...
//    }
    ...
    public function onUpdateEntityItemsStatus( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onUserUnregisterRemovePreference( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onPluginUninstallDeleteComments( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    {
        if ( SC::getConfig()->getValue('base', 'cron_is_configured') || defined('SC_PLUGIN_XP') )
        {
            return;
        }

        $coll->add(SC::getLanguage()->text('admin', 'cron_configuration_required_notice', array(
            'helpUrl' => 'http://docs.scene.org/install:cron'
        )));
    }

    public function addJsDeclarations( SC_Event $e )
    {
        //Langs
        SC::getLanguage()->addKeyForJs('base', 'ajax_floatbox_users_title');
        SC::getLanguage()->addKeyForJs('base', 'flag_as');
        SC::getLanguage()->addKeyForJs('base', 'delete_user_confirmation_label');
        SC::getLanguage()->addKeyForJs('base', 'authorization_limited_permissions');
        SC::getLanguage()->addKeyForJs('base', 'avatar_change');
        SC::getLanguage()->addKeyForJs('base', 'avatar_crop');

        $scriptGen = UTIL_JsGenerator::newInstance()->setVariable(array('SC', 'ajaxComponentLoaderRsp'), SC::getRouter()->urlFor('BASE_CTRL_AjaxLoader', 'component'));
        $scriptGen->setVariable(array('SC', 'ajaxAttachmentLinkRsp'), SC::getRouter()->urlFor('BASE_CTRL_Attachment', 'addLink'));

        //Ping
        $scriptGen->addScript('SC.getPing().setRspUrl({$url});', array(
            'url' => SC::getRouter()->urlFor('BASE_CTRL_Ping', 'index')
        ));

        //UsersApi
        $scriptGen->newObject(array('SC', 'Users'), 'SC_UsersApi', array(array(
            "rsp" => SC::getRouter()->urlFor('BASE_CTRL_AjaxUsersApi', 'rsp')
        )));

        SC::getDocument()->addScriptDeclaration($scriptGen->generateJs());

        //Light Cron
        $cronReady = SC::getConfig()->configExists('base', 'cron_is_configured') && SC::getConfig()->getValue('base', 'cron_is_configured');

        if ( !$cronReady && !defined('SC_PLUGIN_XP') )
        {
            SC::getDocument()->addOnloadScript(UTIL_JsGenerator::composeJsString(
                '$.get({$cron});'
                , array(
                'cron' => SC::getRequest()->buildUrlQueryString(SC_URL_HOME . 'sc_cron/run.php', array(
                    'ow-light-cron' => 1
                ))
            )));
        }
    ...
    {
        $e->add(array(
            'title' => SC::getLanguage()->text('base', 'console_item_label_dashboard'),
            'iconClass' => 'sc_ic_house',
            'url' => SC::getRouter()->urlForRoute('base_member_dashboard'),
            'order' => 1
        ));

        $e->add(array(
            'title' => SC::getLanguage()->text('base', 'console_item_label_profile'),
            'iconClass' => 'sc_ic_user',
            'url' => SC::getRouter()->urlForRoute('base_member_profile'),
            'order' => 3
        ));
    }

    public function onAddComment( SC_Event $event )
    {
        $params = $event->getParams();
        ...
        $url = SC::getRouter()->urlForRoute('base_user_profile', array('username' => BOL_UserService::getInstance()->getUserName($entityId)));

        $avatars = BOL_AvatarService::getInstance()->getDataForUserAvatars(array($userId));
        $avatar = $avatars[$userId];

        $event = new SC_Event('notifications.add', array(
            'pluginKey' => 'base',
    ...
    public function onNotifyActions( BASE_CLASS_EventCollector $e )
    {
        $e->add(array(
            'section' => 'base',
            'sectionLabel' => SC::getLanguage()->text('base', 'notification_section_label'),
            'action' => 'base_add_user_comment',
            'description' => SC::getLanguage()->text('base', 'email_notifications_setting_user_comment'),
            'sectionIcon' => 'sc_ic_file',
            'selected' => true
        ));
    }
    ...
    public function onPreferenceMenuItem( BASE_CLASS_EventCollector $event )
    {
        ...
        $menuItem->setIconClass('sc_ic_gear_wheel');
    ...
    public function onDashboardMenuItem( BASE_CLASS_EventCollector $event )
    {
        ...
        $menuItem->setIconClass('sc_ic_house');
        ...
        $menuItem->setIconClass('sc_ic_user');
    ...
    public function onUserUnregisterDeleteDisaproved( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onUserUnregisterClearMailQueue( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onInviteMembersProcessJoinForm( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onActionToolbarAddUserBlockActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        if ( $params['userId'] == SC::getUser()->getId() )
        {
            return;
        }
        ...
        $uniqId = uniqid("block-");
        $isBlocked = BOL_UserService::getInstance()->isBlocked($userId, SC::getUser()->getId());

        $resultArray[BASE_CMP_ProfileActionToolbar::DATA_KEY_LABEL] = $isBlocked ? SC::getLanguage()->text('base', 'user_unblock_btn_lbl') : SC::getLanguage()->text('base', 'user_block_btn_lbl');

        $toggleText = !$isBlocked ? SC::getLanguage()->text('base', 'user_unblock_btn_lbl') : SC::getLanguage()->text('base', 'user_block_btn_lbl');

        $resultArray[BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_CLASS] = $isBlocked ? 'sc_mild_green' : 'sc_mild_red';

        $toggleClass = !$isBlocked ? 'sc_mild_green' : 'sc_mild_red';
        ...
        $js->jQueryEvent("#" . $uniqId, "click", 'var self = $(this); toggle = function() {
            SC.Utils.toggleText(self, e.data.toggleText);
            SC.Utils.toggleAttr(self, "class", e.data.toggleClass);
            SC.Utils.toggleAttr(self, "data-command", e.data.toggleCommand);
        };
        if ( self.attr("data-command") == "block" )
            SC.Users.blockUserWithConfirmation(e.data.userId, toggle);
        else {
            SC.Users.unBlockUser(e.data.userId);
            toggle();
        }'
            , array("e"), array(
                "userId" => $userId,
    ...
    public function onActionToolbarAddUserFeatureActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $action = array(
            ...
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_GROUP_LABEL => SC::getLanguage()->text('base', 'profile_toolbar_group_moderation'),
        ...
        $action[BASE_CMP_ProfileActionToolbar::DATA_KEY_LABEL] = $isFeatured ? SC::getLanguage()->text('base', 'user_action_unmark_as_featured') : SC::getLanguage()->text('base', 'user_action_mark_as_featured');

        $toggleText = !$isFeatured ? SC::getLanguage()->text('base', 'user_action_unmark_as_featured') : SC::getLanguage()->text('base', 'user_action_mark_as_featured');
        ...
        $js = UTIL_JsGenerator::newInstance();
        $js->jQueryEvent("#" . $uniqId, "click", 'SC.Users[$(this).attr("data-command") == "feature" ? "featureUser" : "unFeatureUser"](e.data.userId);
         SC.Utils.toggleText(this, e.data.toggleText);
         SC.Utils.toggleAttr(this, "data-command", e.data.toggleCommand);'
            , array("e"), array(
                "userId" => $userId,
                "toggleText" => $toggleText,
                "toggleCommand" => $toggleCommand
    ...
    public function onActionToolbarAddUserApproveActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $action = array(
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_GROUP_KEY => 'base.moderation',
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_GROUP_LABEL => SC::getLanguage()->text('base', 'profile_toolbar_group_moderation'),
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_HREF => SC::getRouter()->urlFor('BASE_CTRL_User', 'approve', array('userId' => $userId)),
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LABEL => SC::getLanguage()->text('base', 'profile_toolbar_user_approve_label'),
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_CLASS => 'sc_mild_green',
    ...
    public function onActionToolbarAddAuthActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $action = array(
            ...
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LABEL => SC::getLanguage()->text('base', 'authorization_give_user_role'),
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_CLASS => 'sc_mild_green',
        ...
        $js = UTIL_JsGenerator::newInstance()->jQueryEvent('#' . $uniqId, 'click', 'window.baseChangeUserRoleFB = SC.ajaxFloatBox("BASE_CMP_GiveUserRole", [e.data.userId], { width:556, title: e.data.title });', array('e'), array(
            'userId' => $userId,
            'title' => SC::getLanguage()->text('base', 'authorization_user_roles')
        ));
    ...
    public function onActionToolbarAddSuspendActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $action[BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_ATTRIBUTES] = array();
        $action[BASE_CMP_ProfileActionToolbar::DATA_KEY_LABEL] = $suspended ? SC::getLanguage()->text('base', 'user_unsuspend_btn_lbl') : SC::getLanguage()->text('base', 'user_suspend_btn_lbl');

        $toggleText = !$suspended ? SC::getLanguage()->text('base', 'user_unsuspend_btn_lbl') : SC::getLanguage()->text('base', 'user_suspend_btn_lbl');
        ...
        $action[BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_CLASS] = $suspended ? "sc_mild_green" : "sc_mild_red";

        $toggleClass = !$suspended ? "sc_mild_green" : "sc_mild_red";

        $rsp = SC::getRouter()->urlFor('BASE_CTRL_SuspendedUser', 'ajaxRsp');
        $rsp = SC::getRequest()->buildUrlQueryString($rsp, array(
            "userId" => $userId
        ));

        SC::getLanguage()->addKeyForJs('base', 'suspend_floatbox_title');
        ...
        $js->jQueryEvent("#" . $uniqId, "click", ' 
            ...
            {
                SC.ajaxFloatBox("BASE_CMP_SetSuspendMessage", [e.data.userId],{width: 520, title: SC.getLanguageText(\'base\', \'suspend_floatbox_title\', {\'displayName\': e.data.displayName})}); 
            }
            else
            {
                SC.trigger("base.on_suspend_command", ["unsuspend"])
            } '
            , array("e"), array(
                "userId" => $userId,
                "toggleText" => $toggleText,
        ...
        $js->addScript( ' SC.bind("base.on_suspend_command", function( command, message ) {
                var element = $("#"+{$uniqId});

                SC.Users[command == "suspend" ? "suspendUser" : "unSuspendUser"]({$userId}, null, message);
                SC.Utils.toggleText(element, {$toggleText});
                SC.Utils.toggleAttr(element, "class", {$toggleClass});
                SC.Utils.toggleAttr(element, "data-command", {$toggleCommand});
    ...
    public function onActionToolbarAddFlagActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        if ( BOL_AuthorizationService::getInstance()->isSuperModerator($params['userId'])
            || $params['userId'] == SC::getUser()->getId() )
        {
            return;
        }
        ...
        $script = UTIL_JsGenerator::newInstance()->jQueryEvent('#' . $linkId, 'click', 'SC.flagContent(e.data.entityType, e.data.entityId);'
            , array('e'), array(
        ...
        $resultArray = array(
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LABEL => SC::getLanguage()->text('base', 'flag'),
    ...
    public function onActionToolbarAddDeleteActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $callbackUrl = SC::getRouter()->urlFor('BASE_CTRL_User', 'userDeleted');

        $linkId = 'ud' . rand(10, 1000000);
        $script = UTIL_JsGenerator::newInstance()->jQueryEvent('#' . $linkId, 'click', 'SC.Users.deleteUser(e.data.userId, e.data.callbackUrl, false);'
            , array('e'), array('userId' => $userId, 'callbackUrl' => $callbackUrl));

        SC::getDocument()->addOnloadScript($script);

        $resultArray = array(
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LABEL => SC::getLanguage()->text('base', 'profile_toolbar_user_delete_label'),
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_CLASS => 'sc_mild_red',
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_HREF => 'javascript://',
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_ID => $linkId,
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_GROUP_KEY => 'base.moderation',
            BASE_CMP_ProfileActionToolbar::DATA_KEY_LINK_GROUP_LABEL => SC::getLanguage()->text('base', 'profile_toolbar_group_moderation'),
    ...
    public function onUserRegisterWelcomeLetter( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onUserAvatarCommentFeed( SC_Event $event )
    {
        $params = $event->getParams();
        ...
        SC::getEventManager()->trigger(new SC_Event('feed.activity', array(
    ...
    public function onUserAvatarLikeFeed( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onLikeUserJoin( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onUserJoinCommentFeed( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onJoinFeed( SC_Event $event )
    {
        $params = $event->getParams();
        ...
        $event = new SC_Event('feed.action', array(
            'pluginKey' => 'base',
    ...
    public function onUserEditFeed( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function onJoinMandatoryUserApprove( SC_Event $event )
    {
        $params = $event->getParams();

        if ( !SC::getConfig()->getValue('base', 'mandatory_user_approve') )
        {
            $e = new SC_Event(SC_EventManager::ON_USER_APPROVE, array('userId' => (int) $params['userId']));
            SC::getEventManager()->trigger($e);
    ...
    {
        $event->add(array('site_name' => SC::getConfig()->getValue('base', 'site_name')));
        $event->add(array('site_url' => SC_URL_HOME));
        $event->add(array('site_email' => SC::getConfig()->getValue('base', 'site_email')));
        $event->add(array('default_currency' => BOL_BillingService::getInstance()->getActiveCurrency()));
    }

    public function onDeleteUserContent( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function sosialSharingGetUserInfo( SC_Event $event )
    {
        $params = $event->getParams();
        $data = $event->getData();
        $data['display'] = false;
    ...
    public function setAccountTypeUserRoleOnUserRegister( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function setUserRoleOnChangeAccountType( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function addFakeQuestions( SC_Event $e )
    {
        $params = $e->getParams();
    ...
    public function onAfterAvatarUpdate( SC_Event $e )
    {
        $params = $e->getParams();
    ...
    public function onProvideMetaInfoForPage( SC_Event $event )
    {
        $document = SC::getDocument();
        $language = SC::getLanguage();

        if( !$document || !$document instanceof SC_HtmlDocument )
        {
            return;
        }
        ...
        $document->addMetaInfo("og:site_name", SC::getConfig()->getValue('base', 'site_name'));
