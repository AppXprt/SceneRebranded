...
class BASE_CMP_UserViewWidget extends BASE_CLASS_Widget
{
    ...
    public function __construct( BASE_CLASS_WidgetParameter $params )
    {
        ...
        $adminMode = SC::getUser()->isAdmin() || SC::getUser()->isAuthorized('base');
        ...
        $template = SC::getPluginManager()->getPlugin('base')->getViewDir() . 'components' . DS . 'user_view_widget_table.html';

        $userViewPresntation = SC::getConfig()->getValue('base', 'user_view_presentation');

        if ( $userViewPresntation === self::USER_VIEW_PRESENTATION_TABS )
        {
            $template = SC::getPluginManager()->getPlugin('base')->getViewDir() . 'components' . DS . 'user_view_widget_tabs.html';

            SC::getDocument()->addOnloadScript(" view = new UserViewWidget(); ");

            $jsDir = SC::getPluginManager()->getPlugin("base")->getStaticJsUrl();
            SC::getDocument()->addScript($jsDir . "user_view_widget.js");

            $this->addMenu($sections);
        }

        $script = ' $(".profile_hidden_field").hover(function(){SC.showTip($(this), {timeout:150, show: "'.SC::getLanguage()->text('base', 'base_invisible_profile_field_tooltip').'"})}, function(){SC.hideTip($(this))});';
        ...
        $this->assign('profileEditUrl', SC::getRouter()->urlForRoute('base_edit'));

        if ( $adminMode && !$ownerMode )
        {
            $this->assign('profileEditUrl', SC::getRouter()->urlForRoute('base_edit_user_datails', array('userId' => $userId) ));
        }
    ...
    public static function getUserViewQuestions( $userId, $adminMode = false, $questionNames = array(), $sectionNames = null )
    {
        ...
        $event = new SC_Event(BOL_QuestionService::EVENT_ON_COLLECT_QUESTIONS_SECTIONS_LIST, array('userId' => $userId), $questions);
        ...
        if ( !empty($questions['questions']) )
        {
            ...
            foreach ( $sections as $section )
            {
                ...
                $component = SC::getClassInstance( 'BASE_CMP_UserViewSection', $section, $sectionQuestions, $data, $questions['labels'], $userViewPresntation, $isHidden, array('userId' => $userId) );
