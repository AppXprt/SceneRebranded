 ...
 * @since 1.0
 */
class BASE_CMP_LanguageValueEdit extends SC_Component
{
    public $eventBased;
	...
	public static function process( $prefix, $key )
	{
            ...
            foreach ( $list as $item )
            {
                    ...
                    if ( $dto !== null )
                    {
                        $event = new SC_Event('admin.before_save_lang_value', array('dto'=>$dto));
...
class LanguageValueEditForm extends Form
{
	...
	public function __construct( $prefix, $key, $parent )
	{
		...
		$this->setAjax(true);
		$this->setAction(SC::getRouter()->urlFor('ADMIN_CTRL_Languages', 'ajaxEditLangs')."?prefix={$prefix}&key={$key}");
		...
		else
		{
            $jsString = 'scForms[{$formName}].bind("success", function(json){
                if ( json["result"] == "success") {
                    SC.trigger("admin.language_key_edit_success", [json], this);
                }
            })';
		}
