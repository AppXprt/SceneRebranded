 ...
 * @since 1.0
 */
class BASE_CMP_GiveUserRole extends SC_Component
{
    ...
    public function __construct( $userId )
    {
        ...
        if ( !SC::getUser()->isAuthorized('base') || $user === null )
        {
        ...
        $form->setAjax(true);
        $form->setAction(SC::getRouter()->urlFor('BASE_CTRL_User', 'updateUserRoles'));
        ...
        foreach ( $roleList as $role )
        {
            ...
            $field->setLabel(SC::getLanguage()->text('base', 'authorization_role_' . $role->getName()));
        ...
        SC::getDocument()->addOnloadScript(
            "scForms['{$form->getName()}'].bind('success', function(data){
                if( data.result ){
