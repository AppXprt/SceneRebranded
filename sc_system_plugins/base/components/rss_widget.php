 ...
 * @since 1.0
 */
require_once SC_DIR_LIB . 'rss' . DS . 'rss.php';
...
class BASE_CMP_RssWidget extends BASE_CLASS_Widget
{
    ...
    public function __construct( BASE_CLASS_WidgetParameter $param )
    {
        ...
        $cachedState = SC::getCacheService()->get($cacheKey);

        if ( $cachedState === false )
        {
            try
            {
                $rssLoading = SC::getConfig()->getValue('base', 'rss_loading');
                ...
                {
                    SC::getConfig()->addConfig('base', 'rss_loading', time());
                }
                else
                {
                    SC::getConfig()->saveConfig('base', 'rss_loading', time());
                }

                $rssIterator = RssParcer::getIterator($param->customParamList['rss_url'], self::$countInterval[1]);

                SC::getConfig()->saveConfig('base', 'rss_loading', 0);
            }
            catch (Exception $e)
            {
                SC::getConfig()->saveConfig('base', 'rss_loading', 0);
            ...
            try
            {
                SC::getCacheService()->set($cacheKey, json_encode($this->rss), 60 * 60);
            }
            catch (Exception $e) {}
        }
        else
    ...
    public static function getSettingList()
    {
        ...
        $settingList['rss_url'] = array(
            ...
            'label' => SC::getLanguage()->text('base', 'rss_widget_url_label'),
            'value' => ''
        );

        $settingList['item_count'] = array(
            'presentation' => self::PRESENTATION_SELECT,
            'label' => SC::getLanguage()->text('base', 'rss_widget_count_label'),
            'value' => 5
        );
        ...
        $settingList['title_only'] = array(
            ...
            'label' => SC::getLanguage()->text('base', 'rss_widget_title_only_label'),
            'value' => false
        );
    ...
    public static function validateSettingList( $settingList )
    {
        ...
        {
            throw new WidgetSettingValidateException(SC::getLanguage()->text('base', 'rss_widget_url_invalid_msg'), 'rss_url');
        }
    }
    ...
    {
        return array(
            self::SETTING_TITLE => SC::getLanguage()->text('base', 'rss_widget_default_title'),
