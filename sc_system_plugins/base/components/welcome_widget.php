...
class BASE_CMP_WelcomeWidget extends BASE_CLASS_Widget
{
    ...
    public function __construct( BASE_CLASS_WidgetParameter $paramObject )
    {
        ...
        $text = SC::getLanguage()->text('base', 'welcome_widget_content');
        ...
        if ( SC::getPluginManager()->isPluginActive('photo') && mb_stripos($text, self::KEY_PHOTO_UPLOAD) !== false )
        {
            $label = SC::getLanguage()->text('photo', 'upload_photos');
            $js = SC::getEventManager()->call('photo.getAddPhotoURL');
        ...
        if ( mb_stripos($text, self::KEY_CHANGE_AVATAR) !== false )
        {
            ...
            $js =  ' $("#welcomeWinget_loadAvatarChangeCmp").click(function(){'
                    . 'document.avatarFloatBox = SC.ajaxFloatBox("BASE_CMP_AvatarChange", [], {width: 749, title: ' . json_encode($label) . '});'
                    . '});';
    ...
    public static function getStandardSettingValueList()
    {
        return array(
            ...
            self::SETTING_TITLE => SC::getLanguage()->text('base', 'welcome_widget_title')
        );
    }
    ...
    {
        BOL_LanguageService::getInstance()->addOrUpdateValue(SC::getLanguage()->getCurrentId(), 'base', 'welcome_widget_content', $settingList['text']);
    ...
    {
        $content = SC::getLanguage()->text('base', 'welcome_widget_content');
        $legend = nl2br(SC::getLanguage()->text('base', 'welcome_widget_legend'));
