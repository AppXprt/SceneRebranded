 ...
 * @since 1.7.5
 */
class BASE_CMP_AjaxFileUploadButton extends SC_Component
{
    ...
    public function onBeforeRender()
    {
        ...
        UTIL_JsGenerator::composeJsString(
            ';window[{$addNewFile}] = function()
                    {
                        var ajaxUploadPhotoFB = SC.ajaxFloatBox("BASE_CMP_AjaxFileUpload", [], {
                            $title: {$title},
                            addClass: "sc_admin_ajax_file_upload_form"
                        });
                    };', array(
                'addNewFile' => $id,
                'title' => SC::getLanguage()->text('base', 'upload_files'),
                'close_alert' => SC::getLanguage()->text('base', 'close_alert')
            )
        )
    );
