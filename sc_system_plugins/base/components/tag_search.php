 ...
 * @since 1.0
 */
class BASE_CMP_TagSearch extends SC_Component
{
    /**
     * @var string
     */
    ...
    public function onBeforeRender()
    {
        ...
        $urlToRedirect = ($this->routeName === null) ? SC::getRequest()->buildUrlQueryString($this->url, array('tag' => '_tag_')) : SC::getRouter()->urlForRoute($this->routeName, array('tag' => '#tag#'));
        ...
        $script = "
			...
			$('#" . $formId . "').bind( 'submit', 
				function(){
					if( !$.trim( $('#" . $elId . "').val() ) )
					{
						SC.error(".  json_encode(SC::getLanguage()->text('base', 'tag_search_empty_value_error')).");
					}
					else
					{
