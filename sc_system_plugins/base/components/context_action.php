 ...
 * @since 1.3.2
 */
class BASE_CMP_ContextAction extends SC_Component
{
    const POSITION_LEFT = 'sc_tooltip_top_left';
    const POSITION_RIGHT = 'sc_tooltip_top_right';
    ...
    public function __construct( $position = self::POSITION_RIGHT )
    {
        ...
        $script = '$(document).on("hover", ".sc_context_action",function(e) {
                        if (e.type == "mouseenter") {
                            $(this).find(".sc_tooltip").css({opacity: 0, top: 10}).show().stop(true, true).animate({top: 18, opacity: 1}, "fast"); 
                        }
                        else { // mouseleave
                            $(this).find(".sc_tooltip").hide();  
                        }     
                    }
                );';
