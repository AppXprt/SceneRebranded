 ...
 * @since 1.0
 */
abstract class BASE_CMP_DragAndDropPanel extends SC_Component
{
    ...
    public function __construct( $placeName, array $componentList, $template = null )
    {
        ...
        if ( $template !== null )
        {
            $plugin = SC::getPluginManager()->getPlugin(SC::getAutoloader()->getPluginKey(get_class($this)));
        ...
        SC_ViewRenderer::getInstance()->registerFunction('dd_component', array($this, 'tplComponent'));
    ...
    {
        $baseStaticJsUrl = SC::getPluginManager()->getPlugin('BASE')->getStaticJsUrl();

        SC::getDocument()->addScript($baseStaticJsUrl . 'jquery-ui.min.js');
        SC::getDocument()->addScript($baseStaticJsUrl . 'drag_and_drop_slider.js');
        SC::getDocument()->addScript($baseStaticJsUrl . 'ajax_utils.js');
        SC::getDocument()->addScript($baseStaticJsUrl . 'drag_and_drop_handler.js');
        SC::getDocument()->addScript($baseStaticJsUrl . 'component_drag_and_drop.js');

        SC::getLanguage()->addKeyForJs('base', 'widgets_delete_component_confirm');
        SC::getLanguage()->addKeyForJs('base', 'widgets_reset_position_confirm');
        $urlAjaxResponder = SC::getRouter()->urlFor($responderController, 'processQueue');
        ...
        $js->newObject('handler', 'SC_Components_DragAndDropAjaxHandler', array($urlAjaxResponder, $sharedData));
