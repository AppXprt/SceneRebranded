 ...
 * @since 1.0
 */
class BASE_CMP_Rate extends SC_Component
{
    ...
    public function __construct( $pluginKey, $entityType, $entityId, $ownerId )
    {
        ...
        {
            $userRateItem = $service->findRate($entityId, $entityType, SC::getUser()->getId());
        ...
        $jsParamsArray = array(
            ...
            'itemsCount' => $maxRate,
            'respondUrl' => SC::getRouter()->urlFor('BASE_CTRL_Rate', 'updateRate'),
            'ownerId' => $ownerId
        );

        SC::getDocument()->addOnloadScript("var rate$cmpId = new OwRate(" . json_encode($jsParamsArray) . "); rate$cmpId.init();");
    }
}