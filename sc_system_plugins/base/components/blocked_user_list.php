...
class BASE_CMP_BlockedUserList extends BASE_CMP_Users
{
    ...
    public function getContextMenu($userId)
    {
        ...
        $contextAction->setLabel(SC::getLanguage()->text('base', 'user_unblock_btn_lbl'));

        $url = SC::getRouter()->urlForRoute('users-blocked');
        $contextAction->setUrl('javascript://');
        $contextAction->addAttribute('onclick', "SC.postRequest('{$url}', {userId: {$userId}})");
