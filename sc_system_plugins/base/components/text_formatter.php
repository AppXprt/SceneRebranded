 ...
 * @since 1.0
 */
class BASE_CMP_TextFormatter extends SC_Component
{
    ...
    function __construct( $plugin, $elId, $dl, $dr, $controls, $template = null )
    {
        ...
        if ( $template == null )
        {
            $this->setTemplate(SC::getPluginManager()->getPlugin('base')->getCmpViewDir() . 'text_formatter.html');
        }

        $this->assign('plugin', $plugin);

        SC::getDocument()->addScript(SC::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'jquery-fieldselection.js');
	...
	$('#{$ctrl['id']}').click(function(){
        ...
        SC::getDocument()->addOnloadScript($js, 100);

        $this->assign('controls', $controls);
        $this->assign('tid', $tid);


        SC::getDocument()->addScriptDeclaration("
