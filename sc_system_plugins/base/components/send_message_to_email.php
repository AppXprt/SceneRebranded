 ...
 * @since 1.8.0
 */
class BASE_CMP_SendMessageToEmail extends SC_Component
{
    public function __construct($userId)
    {
        ...
        $form->setAction(SC::getRouter()->urlFor('BASE_CTRL_AjaxSendMessageToEmail', 'sendMessage'));
        ...
        $subject->setInvitation(SC::getLanguage()->text('base', 'subject'));
        $subject->setRequired(true);
        $form->addElement($subject);

        $textarea = new WysiwygTextarea("message");
        $textarea->setInvitation(SC::getLanguage()->text('base', 'message_invitation'));
        $requiredValidator = new WyswygRequiredValidator();
        $requiredValidator->setErrorMessage(SC::getLanguage()->text('base', 'message_empty'));
        $textarea->addValidator($requiredValidator);

        $form->addElement($textarea);

        $submit = new Submit('send');
        $submit->setLabel(SC::getLanguage()->text('base', 'send'));
        ...
        $form->bindJsFunction(Form::BIND_SUCCESS, ' function ( data ) {
            ...
            if ( SC.getActiveFloatBox() )
            {
                SC.getActiveFloatBox().close();
            }
