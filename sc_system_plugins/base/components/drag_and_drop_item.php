 ...
 * @since 1.0
 */
class BASE_CMP_DragAndDropItem extends SC_Component
{
    ...
    public function __construct( $componentUniqName, $isClone = false, $template = null, $sharedData = array() )
    {
        ...
        if ( $template !== null )
        {
            $this->setTemplate(SC::getPluginManager()->getPlugin('base')->getCmpViewDir() . $template . '.html');
        }
    ...
    {
        $settingList['shsc_title'] = $this->componentParamObject->standartParamList->showTitle;
    ...
    {
        $isUserAuthenticated = SC::getUser()->isAuthenticated();
        ...
        $userRoles = BOL_AuthorizationService::getInstance()->findUserRoleList(SC::getUser()->getId());
    ...
    public function renderView()
    {
        ...
        $contentComponent = SC::getClassInstance($className, $paramsObject);
