...
class BASE_CMP_ConsoleList extends SC_Component
{
    ...
    public function __construct( $consoleItemKey )
    {
        ...
        $this->listRsp = SC::getRouter()->urlFor('BASE_CTRL_Console', 'listRsp');
    }

    public function initJs()
    {
        $js = UTIL_JsGenerator::composeJsString('$.extend(SC.Console.getItem({$key}), SC_ConsoleList).construct({$params});', array(
            'key' => $this->itemKey,
            'params' => array(
                'rsp' => $this->listRsp,
                'key' => $this->itemKey
