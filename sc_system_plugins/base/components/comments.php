 ...
 * @package ow.sc_system_plugins.base.comments
 * @since 1.0
 */
class BASE_CMP_Comments extends SC_Component
{
    /**
    ...
    public function __construct( BASE_CommentsParams $params )
    {
        ...
        $this->isAuthorized = SC::getUser()->isAuthorized($params->getPluginKey(), 'add_comment') && $params->getAddComment();
        ...
        if ( !$this->isAuthorized )
        {
            ...
            if ( empty($errorMessage) )
            {
                ...
                $errorMessage = SC::getUser()->isAuthenticated() ? $status['msg'] : SC::getLanguage()->text('base', 'comments_add_login_message');
            }
    ...
    public function initForm()
    {
        ...
        if ( $this->isAuthorized )
        {
            SC::getDocument()->addScript(SC::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'jquery.autosize.js');
            $taId = 'cta' . $this->id;
            ...
            $jsParams['addUrl'] = SC::getRouter()->urlFor('BASE_CTRL_Comments', 'addComment');
            ...
            $jsParams['labels'] = array(
                'emptyCommentMsg' => SC::getLanguage()->text('base', 'empty_comment_error_msg'),
                'disabledSubmit' => SC::getLanguage()->text('base', 'submit_disabled_error_msg'),
                'attachmentLoading' => SC::getLanguage()->text('base', 'submit_attachment_not_loaded'),
            );
            ...
            {
                $currentUserInfo = BOL_AvatarService::getInstance()->getDataForUserAvatars(array(SC::getUser()->getId()));
                $userInfoToAssign = $currentUserInfo[SC::getUser()->getId()];
            }
        ...
        SC::getDocument()->addOnloadScript("new ScComments(" . json_encode($jsParams) . ");");
