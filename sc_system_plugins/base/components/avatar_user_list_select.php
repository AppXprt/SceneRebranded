 ...
 * @since 1.0
 */
class BASE_CMP_AvatarUserListSelect extends SC_Component
{
    /**
     * @var string
     */
    ...
    public function __construct( array $idList, $langs = array() )
    {
        ...
        $this->countLabel = SC::getLanguage()->text('base', 'avatar_user_list_select_count_label');
        $this->buttonLabel = SC::getLanguage()->text('base', 'avatar_user_list_select_button_label');
        $this->idList = $idList;
    ...
    public function onBeforeRender()
    {
        ...
        SC::getDocument()->addScript(SC::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'avatar_user_select.js');
        SC::getDocument()->addOnloadScript("
            var cmp = new AvatarUserSelect(" . json_encode($jsArray) . ", '" . $contexId . "');
            cmp.init();  ");

        SC::getLanguage()->addKeyForJs('base', 'avatar_user_select_empty_list_message');
