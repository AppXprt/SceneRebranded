...
class BASE_CMP_ConsoleDropdownClick extends BASE_CMP_ConsoleDropdown
{
    public function __construct($label, $key = null)
    {
        ...
        $template = SC::getPluginManager()->getPlugin('BASE')->getCmpViewDir() . 'console_dropdown_click.html';
        $this->setTemplate($template);

        $this->addClass('sc_console_dropdown_click');
    }

    protected function initJs()
    {
        $js = UTIL_JsGenerator::newInstance();
        $js->addScript('SC.Console.addItem(new SC_ConsoleDropdownClick({$uniqId}, {$contentIniqId}), {$key});', array(
