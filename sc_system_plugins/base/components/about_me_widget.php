...
class BASE_CMP_AboutMeWidget extends BASE_CLASS_Widget
{
    ...
    public function __construct( BASE_CLASS_WidgetParameter $param )
    {
        ...
        if ( $param->additionalParamList['entityId'] == SC::getUser()->getId() )
        {
    ...
    {
        return array(
            self::SETTING_TITLE => SC::getLanguage()->text('base', 'about_me_widget_default_title'),
...
class AboutMeForm extends Form
{
    ...
    public function __construct( $widgetUniqName = null, $content = null )
    {
        ...
        $this->setAjax(true);
        $this->setAction(SC::getRouter()->urlFor('BASE_CTRL_ComponentPanel', 'ajaxSaveAboutMe'));

        $input = new Textarea('about_me');
        $input->addAttribute('style', 'width: 93%');
        $input->setId('about_me_widget_input');
        $input->setHasInvitation(true);
        $input->setInvitation(SC::getLanguage()->text('base', 'about_me_widget_inv_text'));
        ...
        //$submit->setLabel(SC::getLanguage()->text('base', 'widget_about_me_save_btn'));

        $this->addElement($submit);

        SC::getDocument()->addOnloadScript('
           window.scForms["about_me_form"].bind("success", function(data){
                SC.info(data.message);
           });
           window.scForms["about_me_form"].reset = false;
        ');
    }
    ...
    public function process( $data )
    {
        ...
        return array('message' => SC::getLanguage()->text('base', 'about_me_widget_content_saved'));
    }
}
