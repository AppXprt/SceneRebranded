 ...
 * @since 1.0
 */
//class BASE_CMP_AjaxSignIn extends SC_Component
//{
...
//        $form->setAjax();
//        $form->setAction(SC::getRouter()->urlFor('BASE_CTRL_User', 'ajaxSignIn'));
//        $form->bindJsFunction(Form::BIND_SUCCESS, 'function(data){if( data.result ){SC.info(data.message);setTimeout(function(){window.location.reload();}, 1000);}else{SC.error(data.message);}}');
//        $this->addForm($form);
//        $this->assign('forgot_url', SC::getRouter()->urlForRoute('base_forgot_password'));
//        $this->assign('buttonList', implode('', $this->getRemoteAuthButtonList()));
//    }
//
//    private function getRemoteAuthButtonList()
//    {
//        $items = SC::getRegistry()->getArray(self::HOOK_REMOTE_AUTH_BUTTON_LIST);
//
//        if ( empty($items) )
//        {
//            return array();
