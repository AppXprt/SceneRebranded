 ...
 * @since 1.0
 */
class BASE_CMP_Console extends SC_Component
{
    ...
    public function onBeforeRender()
    {
        ...
        foreach ( $items as $item )
        {
            ...
            $renderEvent = new SC_Event(self::RENDER_EVENT_NAME, $resultItem, $resultItem);
            SC::getEventManager()->trigger($renderEvent);
            $resultItem = $renderEvent->getData();

            $itemCmp = $resultItem['item'];
            $order = $resultItem['order'];

            if ( is_subclass_of($itemCmp, 'SC_Renderable') && $itemCmp->isVisible() )
            {
                $resultItems[] = array(
                    'item' => $itemCmp->render(),
                    'order' => $order
        ...
        $jsUrl = SC::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'console.js';
        SC::getDocument()->addScript($jsUrl);

        $event = new SC_Event(BASE_CTRL_Ping::PING_EVENT . '.consoleUpdate');
        SC::getEventManager()->trigger($event);
        ...
        $js->newObject(array('SC', 'Console'), 'SC_Console', array($params, $event->getData()));

        SC::getDocument()->addOnloadScript($js, 900);
    }
    ...
    const VALUE_BLOCK_CLASS_GREEN = 'sc_mild_green';
    const VALUE_BLOCK_CLASS_RED = 'sc_mild_red';
