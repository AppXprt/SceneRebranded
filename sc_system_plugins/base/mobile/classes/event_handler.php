...
class BASE_MCLASS_EventHandler extends BASE_CLASS_EventHandler
{
    ...
    public function init()
    {
        ...
        $eventManager->bind(SC_EventManager::ON_BEFORE_DOCUMENT_RENDER, array($this, 'onDocRenderAddJsDeclarations'));
        ...
        $eventManager->bind(SC_EventManager::ON_PLUGINS_INIT, array($this, 'onPluginsInitCheckUserStatus'));
    ...
    public function onCollectButtonList( BASE_CLASS_EventCollector $e )
    {
        ...
        $e->add(array('iconClass' => 'sc_ico_signin_f', 'markup' => $button->render()));
    }
    ...
    public function onMobileProfileContentMenu( BASE_CLASS_EventCollector $event )
    {
        ...
        $url = SC::getRouter()->urlForRoute('base_about_profile', array('username' => $userName));
        $resultArray = array(
    ...
    public function onDocRenderAddJsDeclarations( $e )
    {
        ...
        $scriptGen = UTIL_JsGenerator::newInstance()->setVariable(
                array('SCM', 'ajaxComponentLoaderRsp'), SC::getRouter()->urlFor('BASE_MCTRL_AjaxLoader', 'component')
        );

        //UsersApi
        $scriptGen->newObject(array('SC', 'Users'), 'SCM_UsersApi', array(array(
        ...
        {
            SC::getLanguage()->addKeyForJs('base', 'mobile_disabled_item_message');
            $params = array(
                'pages' => MBOL_ConsoleService::getInstance()->getPages(),
                'rspUrl' => SC::getRouter()->urlFor('BASE_MCTRL_Ping', 'index'),
                'lastFetchTime' => time(),
                'pingInterval' => 10,
                'desktopUrl' => SC::getRouter()->urlForRoute('base.desktop_version')
            );

            $scriptGen->addScript('
            var mconsole = new SCM_Console(' . json_encode($params) . ');
            mconsole.init();
        ');
        }
    ...
    public function onActionToolbarAddUserBlockActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $resultArray["label"] = $isBlocked ? SC::getLanguage()->text('base', 'user_unblock_btn_lbl') : SC::getLanguage()->text('base', 'user_block_btn_lbl');
        ...
        $js->jQueryEvent("#" . $uniqId, "click",
            'var toggle = false; if ( $(this).attr("data-command") == "block" && confirm(e.data.msg) ) { SCM.Users.blockUser(e.data.userId); toggle = true; };
            if ( $(this).attr("data-command") != "block") { SCM.Users.unBlockUser(e.data.userId); toggle =true;}
            toggle && SCM.Utils.toggleText($("span:eq(0)", this), e.data.toggleText);
            toggle && SCM.Utils.toggleAttr(this, "class", e.data.toggleClass);
            toggle && SCM.Utils.toggleAttr(this, "data-command", e.data.toggleCommand);',
            array("e"), array(
            "userId" => $userId,
            "toggleText" => $toggleText,
            "toggleCommand" => $toggleCommand,
            "toggleClass" => $toggleClass,
            "msg" => strip_tags(SC::getLanguage()->text("base", "user_block_confirm_message"))
        ));
    ...
    public function onActionToolbarAddUserFeatureActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $action = array(
            "group" => 'addition',
            "label" => SC::getLanguage()->text('base', 'profile_toolbar_group_moderation'),
            "order" => 2
        );
        ...
        $action["label"] = $isFeatured ? SC::getLanguage()->text('base', 'user_action_unmark_as_featured') : SC::getLanguage()->text('base', 'user_action_mark_as_featured');
        ...
        $js = UTIL_JsGenerator::newInstance();
        $js->jQueryEvent("#" . $uniqId, "click",
            'SCM.Users[$(this).attr("data-command") == "feature" ? "featureUser" : "unFeatureUser"](e.data.userId);
            SCM.Utils.toggleText($("span:eq(0)", this), e.data.toggleText);
            SCM.Utils.toggleAttr(this, "data-command", e.data.toggleCommand);'
            , array("e"), array(
            "userId" => $userId,
            "toggleText" => $toggleText,
            "toggleCommand" => $toggleCommand
    ...
    public function onActionToolbarAddUserApproveActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $action = array(
            "group" => 'addition',
            "label" => SC::getLanguage()->text('base', 'profile_toolbar_group_moderation'),
            "href" => SC::getRouter()->urlFor('BASE_CTRL_User', 'approve', array('userId' => $userId)),
            "label" => SC::getLanguage()->text('base', 'profile_toolbar_user_approve_label'),
            "class" => '',
    ...
    public function onActionToolbarAddSuspendActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $action["attributes"] = array();
        $action["label"] = $suspended ? SC::getLanguage()->text('base', 'user_unsuspend_btn_lbl') : SC::getLanguage()->text('base', 'user_suspend_btn_lbl');
        ...
        $js = UTIL_JsGenerator::newInstance();
        $js->jQueryEvent("#" . $uniqId, "click",
            'SCM.Users[$(this).attr("data-command") == "suspend" ? "suspendUser" : "unSuspendUser"](e.data.userId);
            SCM.Utils.toggleText($("span:eq(0)", this), e.data.toggleText);
            SCM.Utils.toggleAttr(this, "class", e.data.toggleClass);
            SCM.Utils.toggleAttr(this, "data-command", e.data.toggleCommand);'
            , array("e"), array(
            "userId" => $userId,
            "toggleText" => $toggleText,
    ...
    public function onActionToolbarAddEditUserActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        if ( SC::getUser()->getId() == $params['userId']
            || (SC::getUser()->isAuthorized('base') && !BOL_AuthorizationService::getInstance()->isSuperModerator($params['userId'])) )
        {
            $resultArray = array(
                "label" => SC::getUser()->getId() == $params['userId']
                    ? SC::getLanguage()->text('mobile', 'edit_profile')
                    : SC::getLanguage()->text('mobile', 'edit_other_profile'),
                "href" => SC::getUser()->getId() == $params['userId']
                    ? SC::getRouter()->urlForRoute('base_edit')
                    : SC::getRouter()->urlForRoute('base_edit_user_datails', array('userId' => $params['userId']))
            );
    ...
    public function onActionToolbarAddDeleteActionTool( BASE_CLASS_EventCollector $event )
    {
        ...
        $confirmMsg = SC::getLanguage()->text('base', 'are_you_sure');
        $callbackUrl = SC::getRouter()->urlFor('BASE_MCTRL_User', 'userDeleted');

        $linkId = 'ud' . rand(10, 1000000);
        $script = UTIL_JsGenerator::newInstance()->jQueryEvent('#' . $linkId, 'click',
                'if (confirm(e.data.confirmMsg)) SCM.Users.deleteUser(e.data.userId, e.data.callbackUrl);'
                , array('e'), array('userId' => $userId, "confirmMsg" => $confirmMsg, 'callbackUrl' => $callbackUrl));

        SC::getDocument()->addOnloadScript($script);

        $resultArray = array(
            "label" => SC::getLanguage()->text('base', 'profile_toolbar_user_delete_label'),
            "class" => 'owm_red_btn',
            "href" => 'javascript://',
            "id" => $linkId,
            "group" => 'addition',
    ...
    public function onPluginsInitCheckUserStatus()
    {
        if ( SC::getUser()->isAuthenticated() )
        {
            ...
            $signOutDispatchAttrs = SC::getRouter()->getRoute('base_sign_out')->getDispatchAttrs();
            ...
            if ( SC::getConfig()->getValue('base', 'mandatory_user_approve') && !BOL_UserService::getInstance()->isApproved() )
            {
                SC::getRequestHandler()->setCatchAllRequestsAttributes('base.wait_for_approval', array('controller' => 'BASE_MCTRL_WaitForApproval', 'action' => 'index'));
                SC::getRequestHandler()->addCatchAllRequestsExclude('base.wait_for_approval', $signOutDispatchAttrs['controller'], $signOutDispatchAttrs['action']);
                SC::getRequestHandler()->addCatchAllRequestsExclude('base.wait_for_approval', 'BASE_MCTRL_AjaxLoader', 'component');
                SC::getRequestHandler()->addCatchAllRequestsExclude('base.wait_for_approval', 'BASE_MCTRL_Invitations', 'command');
                SC::getRequestHandler()->addCatchAllRequestsExclude('base.wait_for_approval', 'BASE_MCTRL_Ping', 'index');
            }
            ...
            if ( $user !== null )
            {
                ...
                {
                    SC::getRequestHandler()->setCatchAllRequestsAttributes('base.suspended_user', array('controller' => 'BASE_MCTRL_SuspendedUser', 'action' => 'index'));
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', $signOutDispatchAttrs['controller'], $signOutDispatchAttrs['action']);
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_MCTRL_AjaxLoader');
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_MCTRL_Invitations', 'command');
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.suspended_user', 'BASE_MCTRL_Ping', 'index');
                }

                if ( (int) $user->emailVerify === 0 && SC::getConfig()->getValue('base', 'confirm_email') )
                {
                    SC::getRequestHandler()->setCatchAllRequestsAttributes('base.email_verify', array(SC_RequestHandler::CATCH_ALL_REQUEST_KEY_CTRL => 'BASE_MCTRL_EmailVerify', SC_RequestHandler::CATCH_ALL_REQUEST_KEY_ACTION => 'index'));

                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.email_verify', $signOutDispatchAttrs['controller'], $signOutDispatchAttrs['action']);
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.email_verify', 'BASE_MCTRL_EmailVerify');
                }
                ...
                {
                    SC::getRequestHandler()->setCatchAllRequestsAttributes('base.complete_profile.account_type', array('controller' => 'BASE_MCTRL_CompleteProfile', 'action' => 'fillAccountType'));
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', $signOutDispatchAttrs['controller'], $signOutDispatchAttrs['action']);
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'BASE_MCTRL_AjaxLoader');
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'BASE_MCTRL_Invitations');
                    SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile.account_type', 'BASE_MCTRL_Ping');
                ...
                else
                {
                    ...
                    if ( $questionsEditStamp >= (int) $updateDetailsStamp )
                    {
                        ...
                        {
                            SC::getRequestHandler()->setCatchAllRequestsAttributes('base.complete_profile', array('controller' => 'BASE_MCTRL_CompleteProfile', 'action' => 'fillRequiredQuestions'));
                            SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', $signOutDispatchAttrs['controller'], $signOutDispatchAttrs['action']);
                            SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'BASE_MCTRL_AjaxLoader');
                            SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'BASE_MCTRL_Invitations');
                            SC::getRequestHandler()->addCatchAllRequestsExclude('base.complete_profile', 'BASE_MCTRL_Ping');
                        }
                        else
                        {
    ...
    public function onNotificationRender( SC_Event $event )
    {
        $params = $event->getParams();
    ...
    public function getUserListFields( SC_Event $e )
    {
        $params = $e->getParams();
            ...
            switch( $list )
            {
                case "birthdays":
                    if ( !empty($questionsData['birthdate']) && $list == 'birthdays'  )
                    {
                        ...
                        {
                            $birthday= SC::getLanguage()->text('base', 'date_time_tomorrow');
                            $birthdate = '<a href="#" class="sc_lbutton sc_green">' . $birthday . '</a>';
                        }
                        else if ( intval(date('d')) == intval($dinfo['day']) )
                        {
                            $birthday = SC::getLanguage()->text('base', 'date_time_today');
                            $birthdate = '<a href="#" class="sc_lbutton sc_green">' . $birthday . '</a>';
                        }
                        else
                        {
                        ...
                        $data[$userId][] = SC::getLanguage()->text('birthdays', 'birthday') . ":" . $birthdate;
                    }
                break;
            }
        }
