...
class BASE_MCTRL_Join extends BASE_CTRL_Join
{
    ...
    {        
        ...
        $this->responderUrl = SC::getRouter()->urlFor("BASE_MCTRL_Join", "ajaxResponder");
    }
    ...
    public function index( $params )
    {
        ...
        {
            $this->redirect(SC::getRouter()->urlForRoute('base_index'));
        }
        
        parent::index($params);
        $this->setTemplate(SC::getPluginManager()->getPlugin('base')->getMobileCtrlViewDir() . 'join_index.html');
        ...
        if( !empty($form) )
        {
            $this->joinForm->setAction(SC::getRouter()->urlFor('BASE_MCTRL_Join', 'joinFormSubmit', $urlParams));
