 ...
 * @since 1.0
 */
class BASE_MCMP_SignIn extends SC_MobileComponent
{
    ...
    public function __construct( $ajax = true )
    {
        ...
        if ( $ajax )
        {
            $form->setAction(SC::getRouter()->urlFor('BASE_MCTRL_User', 'signIn'));
            $form->setAjax();
            $form->bindJsFunction(Form::BIND_SUCCESS, 'function(data){if( data.result ){SCM.info(data.message);setTimeout(function(){window.location.reload();}, 1000);}else{SCM.error(data.message);}}');
        }
        $this->addForm($form);
