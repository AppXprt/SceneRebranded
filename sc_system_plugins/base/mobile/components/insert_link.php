 ...
 * @since 1.0
 */
class BASE_MCMP_InsertLink extends SC_MobileComponent
{
    /**
     * Class constructor
     * 
...
class InsertLinkForm extends Form
{
    ...
    public function __construct()
    {
        ...
        $titleField->setRequired(true)->setHasInvitation(true)->setInvitation(SC::getLanguage()->text('base', 'ws_link_text_label'));

        $sValidator = new StringValidator(self::MIN_TITLE_LENGTH, self::MAX_TITLE_LENGTH);
        $sValidator->setErrorMessage(SC::getLanguage()->
        ...
        $linkField->setRequired(true)->setHasInvitation(true)->setInvitation(SC::getLanguage()->text('base', 'ws_link_url_label'));
        ...
        $submit->setValue(SC::getLanguage()->text('base', 'ws_insert_label'));
