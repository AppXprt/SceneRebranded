 ...
 * @package ow.sc_system_plugins.base.mobile.components
 * @since 1.6.0
 */
class BASE_MCMP_ConsoleInvitationsSection extends SC_MobileComponent
{
    /**
    ...
    public function onBeforeRender()
    {
        ...
        $this->assign('loadMore', $this->service->findInvitationCount(SC::getUser()->getId()) > $limit);

        $params = array(
            'cmdUrl' => SC::getRouter()->urlFor('BASE_MCTRL_Invitations', 'command')
        );

        $script = 'var invitationsConsole = new SCM_InvitationsConsole(' . json_encode($params) . ');';
