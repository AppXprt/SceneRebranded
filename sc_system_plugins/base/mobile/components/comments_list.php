...
class BASE_MCMP_CommentsList extends BASE_CMP_CommentsList
{
    ...
    public function __construct( BASE_CommentsParams $params, $id )
    {
        ...
        $this->setTemplate(SC::getPluginManager()->getPlugin('base')->getMobileCmpViewDir() . 'comments_list.html');
    }
    ...
    protected function init()
    {
        ...
        if ( !$dataInit )
        {
            $staticDataArray = array(
                'respondUrl' => SC::getRouter()->urlFor('BASE_CTRL_Comments', 'getMobileCommentList'),
        ...
        SC::getDocument()->addOnloadScript(
            "window.owCommentListCmps.items['$this->id'] = new ScMobileCommentsList($jsParams);
