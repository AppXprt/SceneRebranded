...
class BASE_MCMP_ProfileInfo extends SC_MobileComponent
{
    /**
     *
     * @var BOL_User
    ...
    public function onBeforeRender() 
    {
        ...
        if ( $this->previewMode )
        {
            ...
            {
                if ( $question["name"] == SC::getConfig()->getValue('base', 'display_name_question') )
                {
                    continue;
                }
        ...
        $questions = BASE_CMP_UserViewWidget::getUserViewQuestions($this->user->id, SC::getUser()->isAdmin(), reset($questionNames));
        ...
        $event = new SC_Event('mbase.profile_on_collect_question_data', array('profileUserId' => $this->user->getId()), $questions);
