 ...
 * @since 1.6.0
 */
class BASE_MCMP_ConsoleInvitations extends SC_MobileComponent
{
    /**
     * Constructor.
     */
    public function __construct(  $limit, $exclude = null )
    {
        ...
        if ( !$loadMore )
        {
            $script = "SCM.trigger('mobile.console_hide_invitations_load_more', {});";
    ...
    public static function prepareData( $invitations )
    {
        ...
        foreach ( $invitations as $invitation )
        {
            ...
            $itemEvent = new SC_Event('mobile.invitations.on_item_render', array(
            ...
            if ( !empty($item['string']) && is_array($item['string']) )
            {
                ...
                $item['string'] = SC::getLanguage()->text($key[0], $key[1], $vars);
                if ( $disabled )
                {
