 ...
 * @since 1.0
 */
class BASE_CTRL_Avatar extends SC_ActionController
{
    ...
    public function ajaxResponder()
    {
        ...
        if ( isset($request['ajaxFunc']) && SC::getRequest()->isAjax() )
        {
    ...
    public function ajaxCropPhoto( $params )
    {
        ...
        if ( !empty($params['entityType']) && !empty($params['id']) )
        {
            ...
            if ( !$item || empty($item['path']) || !SC::getStorage()->fileExists($item['path']) )
            {
        ...
        else if ( isset($params['url']) ) 
        {
            ...
            if ( !SC::getStorage()->fileExists($path)  )
            {
                if ( !file_exists($path) )
                {
        ...
        $userId = SC_Auth::getInstance()->getUserId();
    ...
    {
        if ( isset($params['avatarId']) && SC::getUser()->isAuthorized('base') )
        {
            $entityId = $params['avatarId'];
            $entityType = BASE_CLASS_ContentProvider::ENTITY_TYPE_AVATAR;

            $event = new SC_Event("moderation.approve", array(
