 ...
 * @since 1.0
 */
class BASE_CTRL_Preference extends SC_ActionController
{
    ...
    public function index( $params )
    {
        ...
        $preferenceSubmit->addAttribute('class', 'sc_button sc_ic_save');
        ...
        SC::getEventManager()->trigger($formElementEvent);
        ...
        if ( SC::getRequest()->isPost() )
        {
            if( $preferenceForm->isValid($_POST) )
            {
                ...
                if ( $restul )
                {
                    SC::getFeedback()->info($language->text('base', 'preference_preference_data_was_saved'));
                }
                else
                {
                    SC::getFeedback()->warning($language->text('base', 'preference_preference_data_not_changed'));
                }
        ...
        SC::getEventManager()->trigger($sectionLabelEvent);
