 ...
 * @since 1.0
 */
class BASE_CTRL_UserList extends SC_ActionController
{
    ...
    public function __construct()
    {
        ...
        $this->setPageHeading(SC::getLanguage()->text('base', 'users_browse_page_heading'));
        $this->setPageHeadingIconClass('sc_ic_user');
        $this->usersPerPage = (int)SC::getConfig()->getValue('base', 'users_count_on_page');
    ...
    public function index( $params )
    {
        ...
        $cmp = SC::getClassInstance("BASE_Members", $list, $itemCount, $this->usersPerPage, true, $listType);
    ...
    public function blocked()
    {
        ...
        // unblock action
        if ( SC::getRequest()->isPost() && !empty($_POST['userId']) )
        {
        ...
        // set page settings
        $this->setPageHeading(SC::getLanguage()->text('base', 'blocked_users_browse_page_heading'));
        $this->setPageTitle(SC::getLanguage()->text('base', 'blocked_users_browse_page_heading'));
    }

    public function forApproval()
    {
        $this->setTemplate(SC::getPluginManager()->getPlugin('base')->getCtrlViewDir() . 'user_list_index.html');
        ...
        $cmp = SC::getClassInstance("BASE_Members", $list, $itemCount, $this->usersPerPage, false, 'waiting-for-approval');
    ...
    public static function getMenu( $activeListType )
    {
        ...
        $menuArray = array(
            array(
                ...
                'url' => SC::getRouter()->urlForRoute('base_user_lists', array('list' => 'latest')),
            ...
            array(
                ...
                'url' => SC::getRouter()->urlForRoute('base_user_lists', array('list' => 'online')),
                'iconClass' => 'sc_ic_push_pin',
                'key' => 'online',
                'order' => 3
            ),
            array(
                'label' => $language->text('base', 'user_search_menu_item_label'),
                'url' => SC::getRouter()->urlForRoute('users-search'),
        ...
        if ( BOL_UserService::getInstance()->countFeatured() > 0 )
        {
            $menuArray[] =  array(
                ...
                'url' => SC::getRouter()->urlForRoute('base_user_lists', array('list' => 'featured')),
                'iconClass' => 'sc_ic_push_pin',
                'key' => 'featured',
                'order' => 2
            );
        ...
        foreach ( $questionList as $uid => $question )
        {
            ...
            if ( !empty($question['birthdate']) )
            {
                ...
                if ( $this->listKey == 'birthdays' )
                {
                    ...
                    {
                        $questionList[$uid]['birthday'] = SC::getLanguage()->text('base', 'date_time_tomorrow');

                        $birthdate = '<span class="sc_green" style="font-weight: bold; text-transform: uppercase;">' . $questionList[$uid]['birthday'] . '</a>';
                    }
                    else if ( intval(date('d')) == intval($dinfo['day']) )
                    {
                        $questionList[$uid]['birthday'] = SC::getLanguage()->text('base', 'date_time_today');

                        $birthdate = '<span class="sc_green" style="font-weight: bold; text-transform: uppercase;">' . $questionList[$uid]['birthday'] . '</span>';
                    }
                    else
                    {
                    ...
                    $fields[$uid][] = array(
                        'label' => SC::getLanguage()->text('birthdays', 'birthday'),
                        'value' => $birthdate
                    );
                }
            }
