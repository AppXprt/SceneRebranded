 ...
 * @since 1.0
 */
class BASE_CTRL_Comments extends SC_ActionController
{
    /**
    ...
    public function addComment()
    {
        ...
        {
            $errorMessage = SC::getLanguage()->text('base', 'comment_required_validator_message');
        }
        else if ( !SC::getUser()->isAuthorized($params->getPluginKey(), 'add_comment') )
        {
            $status = BOL_AuthorizationService::getInstance()->getActionStatus($params->getPluginKey(), 'add_comment');
            $errorMessage = $status['msg'];
        }
        else if ( BOL_UserService::getInstance()->isBlocked(SC::getUser()->getId(), $params->getOwnerId()) )
        {
            $errorMessage = SC::getLanguage()->text('base', 'user_block_message');
        }
        ...
        if ( BOL_TextFormatService::getInstance()->isCommentsRichMediaAllowed() && !$isMobile )
        {
            if ( !empty($_POST['attachmentInfo']) )
            {
                ...
                SC::getEventManager()->call('base.attachment_save_image', array('uid' => $tempArr['uid'], 'pluginKey' => $tempArr['pluginKey']));
        ...
        $comment = $this->commentService->addComment($params->getEntityType(), $params->getEntityId(), $params->getPluginKey(), SC::getUser()->getId(), $commentText, $attachment);

        // trigger event comment add
        $event = new SC_Event('base_add_comment', array(
        ...
        exit(json_encode(array(
            ...
            'onloadScript' => SC::getDocument()->getOnloadScript(),
    ...
    public function getMobileCommentList()
    {
        ...
        exit(json_encode(array('onloadScript' => SC::getDocument()->getOnloadScript(), 'commentList' => $commentsList->render())));
    }
    ...
    public function deleteComment()
    {
        ...
        $event = new SC_Event('base_delete_comment', array(
    ...
    private function getCommentInfoForDelete()
    {
        ...
        {
            echo json_encode(array('error' => SC::getLanguage()->text('base', 'comment_ajax_error')));
            exit();
        }
        ...
        $isModerator = SC::getUser()->isAuthorized($params->getPluginKey());
        $isOwnerAuthorized = (SC::getUser()->isAuthenticated() && $params->getOwnerId() !== null && (int) $params->getOwnerId() === (int) SC::getUser()->getId());
        $commentOwner = ( (int) SC::getUser()->getId() === (int) $comment->getUserId() );

        if ( !$isModerator && !$isOwnerAuthorized && !$commentOwner )
        {
            echo json_encode(array('error' => SC::getLanguage()->text('base', 'auth_ajax_error')));
            exit();
        }
    ...
    private function getParamsObject()
    {
        ...
        {
            $errorMessage = SC::getLanguage()->text('base', 'comment_ajax_error');
        }
