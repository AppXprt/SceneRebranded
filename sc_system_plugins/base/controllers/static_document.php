 ...
 * @since 1.0
 */
class BASE_CTRL_StaticDocument extends SC_ActionController
{
    /**
    ...
    {
        SC::getDocument()->setDescription(null);

        if ( SC::getLanguage()->valueExist('base', "local_page_meta_desc_{$this->getDocumentKey()}") )
        {
            SC::getDocument()->setDescription(SC::getLanguage()->text('base', "local_page_meta_desc_{$this->getDocumentKey()}"));
        }

        if ( SC::getLanguage()->valueExist('base', "local_page_meta_keywords_{$this->getDocumentKey()}") )
        {
            SC::getDocument()->setKeywords(SC::getLanguage()->text('base', "local_page_meta_keywords_{$this->getDocumentKey()}"));
        }
    }
}
