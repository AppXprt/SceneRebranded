...
class BASE_CTRL_Moderation extends SC_ActionController
{
    ...
    {
        $this->setPageTitle(SC::getLanguage()->text("base", "moderation_tools"));
        $this->setPageHeading(SC::getLanguage()->text("base", "moderation_tools"));
    ...
    public function flags( $params )
    {
        ...
        if ( $menu === null )
        {
            $this->redirect(SC::getRouter()->urlForRoute("base.moderation_tools"));
        }
        ...
        {
            $this->redirect(SC::getRouter()->urlForRoute("base.moderation_flags_index"));
        }
        ...
        foreach ( $flags as $flag )
        {
            ...
            $tplFlags[] = array(
                ...
                "string" => SC::getLanguage()->text("base", "moderation_flags_item_string", array(
                    "userName" => $userName,
                    "userUrl" => $userUrl,
        ...
        $this->assign("responderUrl", SC::getRouter()->urlFor(__CLASS__, "flagsResponder", array(
            "group" => $currentGroup["name"]
        )));
        
        SC::getLanguage()->addKeyForJs("base", "are_you_sure");
        SC::getLanguage()->addKeyForJs("base", "moderation_delete_confirmation");
        SC::getLanguage()->addKeyForJs("base", "moderation_delete_multiple_confirmation");
        SC::getLanguage()->addKeyForJs("base", "moderation_no_items_warning");
    ...
    public function flagsResponder( $params )
    {
        if ( !SC::getRequest()->isPost() ||
                !(SC::getUser()->isAdmin() || BOL_AuthorizationService::getInstance()->isModerator()) )
        {
            throw new Redirect403Exception;
        }
        ...
        $backUrl = SC::getRouter()->urlForRoute("base.moderation_flags", array(
            "group" => $params["group"]
        ));
        ...
        if ( empty($itemKeys) )
        {
            SC::getFeedback()->warning(SC::getLanguage()->text("base", "moderation_no_items_warning"));
        ...
        SC::getFeedback()->info(SC::getLanguage()
