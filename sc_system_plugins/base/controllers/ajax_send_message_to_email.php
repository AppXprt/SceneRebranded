...
class BASE_CTRL_AjaxSendMessageToEmail extends SC_ActionController
{
    public function init()
    {
        ...
        if ( !( SC::getUser()->isAuthenticated() && ( SC::getUser()->isAdmin() || SC::getUser()->isAuthorized('base') ) ) ) {
    ...
    public function sendMessage()
    {
        ...
        if ( empty($user) )
        {
            exit(json_encode(array('result' => false, 'message' => SC::getLanguage()->text('base', 'invalid_user'))));
        }

        if ( empty($subject) )
        {
            exit(json_encode(array('result' => false, 'message' => SC::getLanguage()->text('base', 'empty_subject'))));
        }

        if ( empty($message) )
        {
            exit(json_encode(array('result' => false, 'message' => SC::getLanguage()->text('base', 'empty_message'))));
        }
        ...
        exit(json_encode(array('result' => true, 'message' => SC::getLanguage()->text('base', 'message_send'))));
    }
}
