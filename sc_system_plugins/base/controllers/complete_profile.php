...
require_once SC_DIR_SYSTEM_PLUGIN . 'base' . DS . 'controllers' . DS . 'edit.php';

class BASE_CTRL_CompleteProfile extends SC_ActionController
{
    ...
    public function __construct()
    {
        ...
        $this->setPageHeading(SC::getLanguage()->text('base', 'complete_your_profile_page_heading'));
        $this->setPageHeadingIconClass('sc_ic_user');

        $item = new BASE_MenuItem();
        $item->setLabel(SC::getLanguage()->text('base', 'complete_profile'));
        $item->setUrl(SC::getRouter()->urlForRoute("base.complete_required_questions"));
        $item->setKey('complete_profile');
        $item->setOrder(1);
        
        $masterpage = SC::getDocument()->getMasterPage();
    ...
    public function fillAccountType( $params )
    {
        ...
        $event = new SC_Event( SC_EventManager::ON_BEFORE_USER_COMPLETE_ACCOUNT_TYPE, array( 'user' => $user ) );
        ...
        if ( count($accounts) == 1 )
        {
            ...
            if ( $accountType )
            {
                ...
                $this->redirect(SC::getRouter()->urlForRoute('base_default_index'));
            }
        }
        ...
        $submit->addAttribute('class', 'sc_button sc_ic_save');
        $submit->setValue(SC::getLanguage()->text('base', 'continue_button'));
        ...
        {
            SC::getDocument()->addOnloadScript(" SC.info(".  json_encode(SC::getLanguage()->text('base', 'complete_profile_info')).") ");
        }
    ...
    public function fillRequiredQuestions( $params )
    {
        ...
        $event = new SC_Event( SC_EventManager::ON_BEFORE_USER_COMPLETE_PROFILE, array( 'user' => $user ) );
        SC::getEventManager()->trigger($event);
    ...
    protected function saveRequiredQuestionsData($data, $userId)
    {
        ...
        if ( !empty($userId) )
        {
            ...
            {
                SC::getFeedback()->info(SC::getLanguage()->text('base', 'edit_successfull_edit'));
                
                $event = new SC_Event(SC_EventManager::ON_AFTER_USER_COMPLETE_PROFILE, array( 'userId' => $userId ));
            ...
            {
                SC::getFeedback()->info(SC::getLanguage()->text('base', 'edit_edit_error'));
            }
        }
        else
        {
