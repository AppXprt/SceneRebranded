 ...
 * @since 1.0
 */
class BASE_CTRL_Attachment extends SC_ActionController
{
    /**
    ...
    public function addPhoto( $params )
    {
        ...
        if ( SC::getUser()->isAuthenticated() && !empty($_POST['flUid']) && !empty($_POST['pluginKey']) && !empty($_FILES['attachment']) )
        {
    ...
    public function addFile()
    {
        $respondArr = array();
        $bundle = $_GET['flUid'];
        if ( SC::getUser()->isAuthenticated() && !empty($_POST['flData']) && !empty($_POST['pluginKey']) && !empty($_FILES['sc_file_attachment']) )
        {
            ...
            foreach ( $_FILES['sc_file_attachment'] as $key => $items )
            {
            ...
            SC::getEventManager()->trigger(new SC_Event('base.attachment_uploaded', array('pluginKey' => $pluginKey, 'uid' => $bundle, 'files' => $items)));
        }
        else
        {
    ...
    public function deleteFile()
    {
        ...
        $this->service->deleteAttachment(SC::getUser()->getId(), $fileId);
