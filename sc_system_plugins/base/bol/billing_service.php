...
final class BOL_BillingService
{
    ...
    public function initSale( BOL_BillingSale $sale, $gatewayKey )
    {
        ...
        $event = new SC_Event(self::EVENT_ON_AFTER_INIT_SALE, array("saleDbo" => $sale, "gateway" => $gateway));
     ...
     * @return bool
     */
    public function prepareSale( SC_BillingAdapter $adapter, BOL_BillingSale $sale )
    {
     ...
     * 
     * @param SC_BillingAdapter $adapter
     * @param BOL_BillingSale $sale
     * @return boolean
     */
    public function verifySale( SC_BillingAdapter $adapter, BOL_BillingSale $sale )
    {
        if ( !$this->getSaleById($sale->id) )
        {
            return false;
     ...
     * 
     * @param SC_BillingProductAdapter $adapter
     * @param BOL_BillingSale $sale
     * @return int
     */
    public function deliverSale( SC_BillingProductAdapter $adapter, BOL_BillingSale $sale )
    {
        if ( !$this->getSaleById($sale->id) )
        {
            return false;
        ...
        if ( $adapter->deliverSale($sale) )
        {
            ...
            $this->saveSale($sale);
            $event = new SC_Event(self::EVENT_ON_AFTER_DELIVER_SALE, array("saleDbo" => $sale, "adapter" => $adapter));
            SC::getEventManager()->trigger($event);
            return true;
        }
    ...
    public function registerRebillSale( SC_BillingAdapter $adapter, BOL_BillingSale $parentSale, $rebillTransId )
    {
        $parentSale->id = null;
     ...
     * 
     * @param string $productKey
     * @return SC_BillingProductAdapter
     */
    ...
    {
        return SC::getConfig()->getValue('base', 'billing_currency');
    }
    ...
    public function getOrderCompletedPageUrl( $hash = null )
    {
        ...
        {
            return SC::getRouter()->urlForRoute('base_billing_completed', array('hash' => $hash));
        }
        else
        {
            return SC::getRouter()->urlForRoute('base_billing_completed_st');
        }
    }
    ...
    public function getOrderCancelledPageUrl( $hash = null)
    {
        if ( isset($hash) && $sale = $this->getSaleByHash($hash) )
        {
            return SC::getRouter()->urlForRoute('base_billing_canceled', array('hash' => $hash));
        }
        else
        {
            return SC::getRouter()->urlForRoute('base_billing_canceled_st');
        }
    }
    ...
    {
        return SC::getRouter()->urlForRoute('base_billing_error');
    }
    ...
    public function  getBillingGatewayExtraInfo( $billingGatewayKey, $langKey )
    {
        ...
        return SC::getLanguage()->text($billingGatewayKey, $langKey);
    }
    ...
    public function findGatewayById( $gatewayId = null )
    {
        if( empty($gatewayId) )
        {
            $logger = SC::getLogger('mobile_billing');
