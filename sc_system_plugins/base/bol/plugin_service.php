...
class BOL_PluginService
{
    ...
    public function getPluginsXmlInfo()
    {
        ...
        $xmlFiles = UTIL_File::findFiles(SC_DIR_PLUGIN, array("xml"), 1);
    ...
    {
        $files = UTIL_File::findFiles(SC_DIR_SYSTEM_PLUGIN, array("xml"), 1);
        $pluginData = array();
...
// install plugins list
        foreach ( $pluginData as $pluginInfo )
        {
            ...
            $plugin = new SC_Plugin($pluginDto);
    ...
    public function install( $key, $generateCache = true )
    {
        ...
        // trigger event
        SC::getEventManager()->trigger(new SC_Event(SC_EventManager::ON_AFTER_PLUGIN_INSTALL,
    ...
    public function addPluginDirs( BOL_Plugin $pluginDto )
    {
        ...
        {
            SC::getStorage()->copyDir($plugin->getInnerUserFilesDir(), $plugin->getUserFilesDir());
        }
        else if ( !file_exists($plugin->getUserFilesDir()) )
        {
            SC::getStorage()->mkdir($plugin->getUserFilesDir());
        }
    }
    ...
    public function uninstall( $pluginKey )
    {
        ...
        // trigger event
        SC::getEventManager()->trigger(new SC_Event(SC_EventManager::ON_BEFORE_PLUGIN_UNINSTALL,
            array("pluginKey" => $pluginDto->getKey())));
        ...
        SC::getConfig()->deletePluginConfigs($pluginDto->getKey());
        ...
        // drop plugin tables
        $tables = SC::getDbo()->queryForColumnList("SHOW TABLES LIKE '" . str_replace('_', '\_', SC_DB_PREFIX) . $pluginDto->getKey() . "\_%'");
        ...
        // trigger event
        SC::getEventManager()->trigger(new SC_Event(SC_EventManager::ON_AFTER_PLUGIN_UNINSTALL,
            array("pluginKey" => $pluginDto->getKey())));
    }
    ...
    public function activate( $key )
    {
        ...
        $pluginDto->setIsActive(true);
        $this->pluginDao->save($pluginDto);
        SC::getPluginManager()->addPackagePointers($pluginDto);

        $this->includeScript(SC_DIR_PLUGIN . $pluginDto->getModule() . DS . self::SCRIPT_ACTIVATE);

        $this->updatePluginListCache();

        // trigger event
        $event = new SC_Event(SC_EventManager::ON_AFTER_PLUGIN_ACTIVATE, array("pluginKey" => $pluginDto->getKey()));
    ...
    public function deactivate( $key )
    {
        ...
        // trigger event
        $event = new SC_Event(SC_EventManager::ON_BEFORE_PLUGIN_DEACTIVATE, array("pluginKey" => $pluginDto->getKey()));
        SC::getEventManager()->trigger($event);

        $pluginDto->setIsActive(false);
        $this->pluginDao->save($pluginDto);

        $this->includeScript(SC::getPluginManager()->getPlugin($pluginDto->getKey())->getRootDir() . self::SCRIPT_DEACTIVATE);

        $this->updatePluginListCache();

        $event = new SC_Event(SC_EventManager::ON_AFTER_PLUGIN_DEACTIVATE, array("pluginKey" => $pluginDto->getKey()));
        SC::getEventManager()->trigger($event);
    }
    ...
    {
        $timestamp = SC::getConfig()->getValue("base", "check_mupdates_ts");
        ...
        SC::getConfig()->saveConfig("base", "check_mupdates_ts", time());
    }
