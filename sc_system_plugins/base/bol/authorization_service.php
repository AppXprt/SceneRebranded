...
class BOL_AuthorizationService
{
    ...
    public function isActionAuthorized( $groupName, $actionName = null, $extra = null )
    {
        if ( $extra !== null && !is_array($extra) )
        {
            trigger_error("`ownerId` parameter has been deprecated, pass `extra` parameter instead\n"
                . SC_ErrorManager::getInstance()->debugBacktrace(), E_USER_WARNING);
        }
        ...
        {
            $userId = SC::getUser()->isAuthenticated() ? SC::getUser()->getId() : 0;
        }
        ...
        {
            SC::getLogger()->addEntry($ex->getMessage() . "\n" . print_r($ex->getTrace(), true) );
        }
    ...
    public function trackActionForUser( $userId, $groupName, $actionName = null, $extra = null )
    {
        $userId = !empty($userId) ? $userId : (SC::getUser()->isAuthenticated() ? SC::getUser()->getId() : 0);
    ...
    public function isActionAuthorizedForUser( $userId, $groupName, $actionName = null )
    {
        ...
        if ( $userId > 0 || SC::getUser()->isAuthenticated() )
        {
            $userId = ( $userId > 0 ) ? $userId : SC::getUser()->getId();
...
//                        || isset($this->moderatorPermissionCache[$moderatorId][$adminGroupId])
    ...
    public function generateCaches() {
        ...
        {
            $this->userRolesCache[SC::getUser()->getId()] = $this->userRoleDao->getRoleIdList(SC::getUser()->getId());
        }
    ...
    {
        if ( $userId == null && ( $userId = SC::getUser()->getId() ) === null )
        {
            return false;
        }
    ...
    {
        return SC::getLanguage()->text('base', "authorization_role_{$roleName}");
    }
    ...
    public function saveUserRole( $userId, $roleId )
    {
        ...
        $event = new SC_Event('base.after_save_user_role', array('userId' => $userId, 'roleId' => $roleId));
        SC::getEventManager()->trigger($event);
    }
    ...
    public function deleteRoleById( $id )
    {
        ...
        $eventBefore = new SC_Event(self::ON_BEFORE_ROLE_DELETE, array('roleId' => $role->getId()));
        SC::getEventManager()->trigger($eventBefore);
        ...
        $eventAfter = new SC_Event(self::ON_AFTER_ROLE_DELETE, array('roleId' => $role->getId()));
        SC::getEventManager()->trigger($eventAfter);
    }
    ...
    public function deleteUserRole( $userId, $roleId )
    {
        ...
        $event = new SC_Event('base.after_delete_user_role', array('userId' => $userId, 'roleId' => $roleId));
        SC::getEventManager()->trigger($event);
    }
