 ...
 * @since 1.0
 */
class BOL_MenuItemDao extends SC_BaseDao
{
    const PREFIX = 'prefix';
    const KEY = 'key';
    ...
    {
        return SC_DB_PREFIX . 'base_menu_item';
    }
    ...
    public function findMenuItems( $menuType )
    {
        return $this->dbo->queryForList("
			...
			WHERE `mi`.`" . self::TYPE . "` = :menuType ORDER BY `mi`.`order` ASC", array('menuType' => $menuType), 24 * 3600, array(self::CACHE_TAG_MENU_TYPE_LIST, SC_CacheManager::TAG_OPTION_INSTANT_LOAD));
    }
    ...
    {
        SC::getCacheManager()->clean(array(BOL_MenuItemDao::CACHE_TAG_MENU_TYPE_LIST));
    }
}
