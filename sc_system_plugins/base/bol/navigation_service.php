...
class BOL_NavigationService
{
    ...
    public function getMenuItems( array $menuItems )
    {
        ...
        foreach ( $menuItems as $value )
        {
            ...
            $auth = SC::getUser()->isAuthenticated();
            ...
            {
                $url = SC::getRouter()->getBaseUrl() . $value['uri'];
            }
            else if ( !empty($value['routePath']) )
            {
                $url = SC::getRouter()->urlForRoute($value['routePath']);
            }
            else if ( !empty($value['class']) && !empty($value['action']) )
            {
                $url = SC::getRouter()->urlFor($value['class'], $value['action']);
            }
            else
            {
                $url = '_INVALID_URL_';
            ...
            $menuItem->setLabel(SC::getLanguage()->text($value['prefix'], $value['menu_key']));
        ...
        $event = new SC_Event('navigation.onGetMenuItems', ['menuItems' => $menuItems], $resultArray);
