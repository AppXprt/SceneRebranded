...
final class BOL_UserService
{
    ...
    public function getDisplayName( $userId )
    {
        ...
        $displayName = isset($questionValue[$userId]) ? ( isset($questionValue[$userId][$questionName]) ? $questionValue[$userId][$questionName] : '' ) : SC::getLanguage()->text('base', 'deleted_user');
    ...
    public function getDisplayNamesForList( array $userIdList )
    {
        ...
        {
            $resultArray[$value] = SC::getLanguage()->text('base', 'deleted_user');
    ...
    public function getUserUrlForUsername( $username )
    {
        ...
        return SC::getRouter()->urlForRoute('base_user_profile', array('username' => $username));
    }
    ...
    public function findOnlineList( $first, $count )
    {
        ...
        $permission = SC::getEventManager()->getInstance()->call('privacy_check_permission_for_user_list', $eventParams);
    ...
    {
        $event = new SC_Event('base.before_save_user', array('dto' => $user));
        SC::getEventManager()->trigger($event);
    ...
    public function markAsFeatured( $userId )
    {
        ...
        $event = new SC_Event(SC_EventManager::ON_USER_MARK_FEATURED, array('userId' => $userId));
        SC::getEventManager()->trigger($event);
    ...
    public function cancelFeatured( $userId )
    {
        ...
        $event = new SC_Event(SC_EventManager::ON_USER_UNMARK_FEATURED, array('userId' => $userId));
        SC::getEventManager()->trigger($event);
    }
    ...
    public function updateActivityStamp( $userId, $context )
    {
        ...
        $event = new SC_Event(self::BEFORE_USER_ONLINE, array('userId' => $userId, 'context' => $context), $userOnline);
    ...
    {
        $event = new SC_Event(SC_EventManager::ON_USER_UNREGISTER, array('userId' => $userId, 'deleteContent' => $deleteContent));
        SC::getEventManager()->trigger($event);
    ...
    public function suspend( $userId, $message )
    {
        ...
        $event = new SC_Event(SC_EventManager::ON_USER_SUSPEND, array('userId' => $userId, 'message' => $message));
        SC::getEventManager()->trigger($event);
    }
    ...
    public function unsuspend( $userId )
    {
        ...
        $event = new SC_Event(SC_EventManager::ON_USER_UNSUSPEND, array('userId' => $userId));
        SC::getEventManager()->trigger($event);
    }
    ...
    public function block( $userId )
    {
        ...
        $event = new SC_Event(SC_EventManager::ON_USER_BLOCK, array('userId' => SC::getUser()->getId(), 'blockedUserId' => $userId));
        SC::getEventManager()->trigger($event);
    }
    ...
    public function unblock( $userId )
    {
        ...
        $dto = $this->userBlockDao->findBlockedUser(SC::getUser()->getId(), $userId);

        $this->userBlockDao->delete($dto);

        $event = new SC_Event(SC_EventManager::ON_USER_UNBLOCK, array('userId' => SC::getUser()->getId(), 'blockedUserId' => $userId));
        SC::getEventManager()->trigger($event);
    }
    ...
    {
        return hash('sha256', SC_PASSWORD_SALT . $password);
    }
    ...
    {
        $timestamp = SC::getConfig()->configExists('base', 'user_expired_time') ? time() - SC::getConfig()->getValue('base', 'user_expired_time') * 60 : time() - 30 * 60;
    ...
    public function createUser( $username, $password, $email, $accountType = null, $emailVerify = false, $ip = null )
    {
        ...
        $user->joinIp = $ip ? $ip : ip2long(SC::getRequest()->getRemoteAddress());
    ...
    public function sendAdminInvitation( $email )
    {
        ...
        $inviteUrl = SC::getRequest()->buildUrlQueryString(SC::getRouter()->urlForRoute('base_join'), array('code' => $inviteCodeDto->getCode()));

        $mail = SC::getMailer()->createMail();
        $mail->addRecipientEmail($email);
        $mail->setSubject(SC::getLanguage()->text('base', 'mail_template_admin_invite_user_subject'));
        $mail->setHtmlContent(SC::getLanguage()->text('base', 'mail_template_admin_invite_user_content_html', array('url' => $inviteUrl)));
        $mail->setTextContent(SC::getLanguage()->text('base', 'mail_template_admin_invite_user_content_text', array('url' => $inviteUrl)));

        SC::getMailer()->addToQueue($mail);
    }
    ...
    public function disapprove( $userId )
    {
        ...
        $event = new SC_Event(SC_EventManager::ON_USER_DISAPPROVE, array('userId' => $userId));
        SC::getEventManager()->trigger($event);
    }
    ...
    public function approve( $userId )
    {
        ...
        $event = new SC_Event(SC_EventManager::ON_USER_APPROVE, array('userId' => $userId));
        SC::getEventManager()->trigger($event);
    }
    ...
    public function findUserIdListByQuestionValues( $questionValues, $first, $count, $isAdmin = false, $aditionalParams = array() )
    {
        ...
        $event = new SC_Event("base.question.before_user_search", $data, $data);
    ...
    public function sendWellcomeLetter( BOL_User $user )
    {
        ...
        if ( SC::getConfig()->getValue('base', 'confirm_email') && $user->emailVerify != true )
        {
            return;
        }
    ...
    public function getSignInForm( $formName = 'sign-in', $submitDecorator = 'button' )
    {
        ...
        $password->setInvitation(SC::getLanguage()->text('base', 'component_sign_in_password_invitation'));
    ...
    public function processSignIn( $identity, $password, $rememberMe = false )
    {
        ...
        $result = SC::getUser()->authenticate(new BASE_CLASS_StandardAuth($identity, $password));

        if ( $result->isValid() )
        {
            if ( $rememberMe )
            {
                $loginCookie = $this->saveLoginCookie(SC::getUser()->getId());

                setcookie('sc_login', $loginCookie->getCookie(), (time() + 86400 * 7), '/', null, null, true);
            }
        }
    ...
    public function processResetForm( $data )
    {
        ...
        $vars = array('code' => $resetPassword->getCode(), 'username' => $user->getUsername(), 'requestUrl' => SC::getRouter()->urlForRoute('base.reset_user_password_request'),
            'resetUrl' => SC::getRouter()->urlForRoute('base.reset_user_password', array('code' => $resetPassword->getCode())));
    ...
    public function processResetPasswordForm( $data, BOL_User $user, $resetCode )
    {
        ...
        {
            throw new LogicException(SC::getLanguage()->text('base', 'reset_password_length_error_message', array('min' => UTIL_Validator::PASSWORD_MIN_LENGTH, 'max' => UTIL_Validator::PASSWORD_MAX_LENGTH)));
        }
    ...
    public function getUserViewQuestions( $userId, $adminMode = false, $questionNames = array(), $sectionNames = null )
    {
        ...
        $event = new SC_Event(self::EVENT_GET_USER_VIEW_QUESTIONS, array(
            'userId' => $userId,
            'adminMode' => $adminMode,
        ...
        foreach ( $questionArray as $sectionKey => $section )
        {
            foreach ( $section as $questionKey => $question )
            {
                ...
                $event = new SC_Event('base.questions_field_get_value', array(
                    'presentation' => $question['presentation'],
                    'fieldName' => $question['name'],
                ...
                if ( !empty($questionData[$userId][$question['name']]) )
                {
                    switch ( $question['presentation'] )
                    {
                        case BOL_QuestionService::QUESTION_PRESENTATION_CHECKBOX:
                            ...
                            {
                                $questionData[$userId][$question['name']] = SC::getLanguage()->text('base', 'yes');
                            }
                            else
                            {
    ...
    public function sendReActivationNotification( array $userIds )
    {
        ...
        if ( is_array($userIds) )
        {
            foreach ( $userIds as $item => $userId )
            {
                ...
                {
                    SC::getLogger()->addEntry(json_encode($e));
                }
            }
        }
    }
