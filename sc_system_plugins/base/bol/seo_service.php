...
class BOL_SeoService
{
    ...
    {
        $url =  SC::getRouter()->urlForRoute('base.sitemap');
    ...
    {
        $sitemapBuild = (int) SC::getConfig()->getValue('base', 'seo_sitemap_last_build');
    ...
    {
        $path = SC::getPluginManager()->getPlugin('base')->getUserFilesDir() . self::SITEMAP_DIR_NAME . '/';
    ...
    public function generateSitemap()
    {
        ...
        if ( !(int) SC::getConfig()->getValue('base', 'seo_sitemap_build_in_progress') )
        {
            SC::getConfig()->saveConfig('base', 'seo_sitemap_build_finished', 0);

            // get sitemap entities
            $entities = $this->getSitemapEntities();
            $maxCount = (int) SC::getConfig()->getValue('base', 'seo_sitemap_entitites_max_count');
            $limit = (int) SC::getConfig()->getValue('base', 'seo_sitemap_entitites_limit');
            ...
            if ( $entities )
            {
                ...
                foreach ( $entities as $entityType => $entityData )
                {
                    ...
                    foreach ( $entityData['items'] as $item )
                    {
                        ...
                        // get urls
                        $event = new SC_Event('base.sitemap.get_urls', array(
                            'entity' => $item['name'],
                            'limit' => $limit,
    ...
    {
        SC::getConfig()->saveConfig('base', 'seo_sitemap_build_in_progress', 1);

        $urls = $this->sitemapDao->findUrlList( (int) SC::getConfig()->getValue('base', 'seo_sitemap_max_urls_in_file') );
        $newSitemapBuild = (int) SC::getConfig()->getValue('base', 'seo_sitemap_last_build') + 1;
        $entities = $this->getSitemapEntities();
        $sitemapIndex = (int) SC::getConfig()->getValue('base', 'seo_sitemap_index');
        ...
        if ( $urls )
        {
            ...
            // render data
            $view = new SC_View();
            $view->setTemplate(SC::getPluginManager()->getPlugin('base')->getViewDir() . 'sitemap_part.xml');
            $view->assign('urls', $processedUrls);

            // save data in a file
            file_put_contents($newSitemapPath . sprintf(self::SITEMAP_FILE_NAME, $sitemapIndex + 1), $view->render());

            SC::getConfig()->saveConfig('base', 'seo_sitemap_index', $sitemapIndex + 1);
        ...
        if ( $sitemapIndex )
        {
            ...
            // render data
            $view = new SC_View();
            $view->setTemplate(SC::getPluginManager()->getPlugin('base')->getViewDir() . 'sitemap.xml');
            ...
            // update configs
            SC::getConfig()->saveConfig('base', 'seo_sitemap_index', 0);
            SC::getConfig()->saveConfig('base', 'seo_sitemap_last_start', time());
            SC::getConfig()->saveConfig('base', 'seo_sitemap_last_build', $newSitemapBuild);
        ...
        SC::getConfig()->saveConfig('base', 'seo_sitemap_build_in_progress', 0);
        SC::getConfig()->saveConfig('base', 'seo_sitemap_build_finished', 1);
    }
    ...
    {
        $lastStart  = (int) SC::getConfig()->getValue('base', 'seo_sitemap_last_start');
        $scheduleUpdate = SC::getConfig()->getValue('base', 'seo_sitemap_schedule_update');
    ...
    {
        return json_decode(SC::getConfig()->getValue('base', 'seo_sitemap_entities'), true);
    }
    ...
    public function addSitemapEntity($langPrefix, $label, $entityType, array $items, $description = null, $priority = 0.5, $changeFreq = self::SITEMAP_ITEM_UPDATE_WEEKLY)
    {
        ...
        if ( !array_key_exists($entityType, $entities) )
        {
            ...
            SC::getConfig()->saveConfig('base', 'seo_sitemap_entities', json_encode($entities));
        }
    }
    ...
    public function getMetaData()
    {
        ...
        {
            $this->metaData = json_decode(SC::getConfig()->getValue("base", "seo_meta_info"), true);
        }
    ...
    {
        $this->metaData = $data;
        SC::getConfig()->saveConfig("base", "seo_meta_info", json_encode($data));
    }
    ...
    {
        SC::getStorage()->copyFile($path, SC::getPluginManager()->getPlugin("base")->getUserFilesDir().$name);
        SC::getConfig()->saveConfig("base", "seo_social_meta_logo_name", $name);
    }
    ...
    {
        $fileName = SC::getConfig()->getValue("base", "seo_social_meta_logo_name");
        ...
        return SC::getStorage()->getFileUrl(SC::getPluginManager()->getPlugin("base")->getUserFilesDir().$fileName);
    }
