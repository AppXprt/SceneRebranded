...
class BOL_LanguageService
{
     ...
     * @var array
     */
    private $exceptionPrefixes = array( "mobile", "nav", "sc_custom" ); // section which importing without checking plugin key
    ...
    public static function getInstance( $includeCache = true )
    {
        if ( !isset(self::$classInstance) )
        {
            try
            {
                self::$classInstance = SC::getClassInstance(self::class, $includeCache);
            }
    ...
    {
        SC::getEventManager()->trigger( new SC_Event('servicelangtools.lang_used_log', array( 'prefix' => $prefix, 'key' => $key)) );
    ...
    public function getText( $languageId, $prefix, $key, $vars = array() )
    {
        ...
        $event = new SC_Event("core.get_text", array("prefix" => $prefix, "key" => $key, "vars" => $vars));
        SC::getEventManager()->trigger($event);
    ...
    {
        return SC::getPluginManager()->getPlugin('admin')->getPluginFilesDir() . 'languages' . DS . 'import' . DS;
    }

    public static function getExportDirPath()
    {
        return SC::getPluginManager()->getPlugin('admin')->getPluginFilesDir() . 'languages' . DS . 'export' . DS;
    }

    public static function getTmpDirPath()
    {
        return SC::getPluginManager()->getPlugin('admin')->getPluginFilesDir() . 'languages' . DS . 'tmp' . DS;
    }
    ...
    {
        return SC::getPluginManager()->getPlugin('base')->getPluginFilesDir();
    }
