...
class BOL_ThemeService
{
    ...
    {
        return SC::getConfig()->getValue("base", "selectedTheme");
    }
    ...
    {
        SC::getConfig()->saveConfig("base", "selectedTheme", trim($name));
    }
    ...
    {
        return SC_DIR_USERFILES . "themes" . DS;
    }
    ...
    {
        return SC_URL_USERFILES . "themes/";
    }
    ...
    public function updateThemeList()
    {
        ...
        $xmlFiles = UTIL_File::findFiles(SC_DIR_THEME, array("xml"), 1);
        ...
        foreach ( $xmlFiles as $themeXml )
        {
            ...
            $result = SC::getEventManager()->call("admin.themes_list_theme_avail", array("name" => $themeKey));
    ...
    public function deleteTheme( $themeId, $deleteControlValues = false )
    {
        ...
        if ( $deleteControlValues )
        {
            ...
            $curentValue = json_decode(SC::getConfig()->getValue("base", "master_page_theme_info"), true);
            unset($curentValue[$themeId]);
            SC::getConfig()->getValue("base", "master_page_theme_info", json_encode($curentValue));
        }
    ...
    /**
     * Generates theme object for theme manager (SC_Theme).
     *
    ...
    public function saveThemeControls( $themeId, $values )
    {
        ...
        foreach ( $values as $key => $value )
        {
            ...
            if ( $namedControls[$key]["type"] == "image" )
            {
                ...
                $value = "url(" . SC::getStorage()->getFileUrl($this->getUserfileImagesDir() . $image->getFilename()) . ")";

                //TODO remove hotfix temp solution for assigning theme img data in master pages
                $curentValue = json_decode(SC::getConfig()->getValue("base", "master_page_theme_info"), true);
                $curentValue[$themeId][$namedControls[$key]["key"]] = array("src" => SC::getStorage()->getFileUrl($this->getUserfileImagesDir() . $image->getFilename()),
                    "width" => $width, "height" => $height);
                SC::getConfig()->saveConfig("base", "master_page_theme_info", json_encode($curentValue));
            }
    ...
    public function addImage( $fileArr )
    {
        ...
        SC::getStorage()->copyFile($newTempName, $this->getUserfileImagesDir() . $imageName);
    ...
    public function moveTemporaryFile( $tmpId, $title = '' )
    {
        ...
        SC::getStorage()->copyFile($tmpPath, $this->getUserfileImagesDir() . $imageName);
        if ( file_exists($newTempName) )
        {
            unlink($newTempName);
        }
    ...
    public function deleteImage( $id )
    {
        ...
        if ( $image !== null )
        {
            if ( SC::getStorage()->fileExists($this->getUserfileImagesDir() . $image->getFilename()) )
            {
                SC::getStorage()->removeFile($this->getUserfileImagesDir() . $image->getFilename());
            }
    ...
    public function updateCustomCssFile( $themeId )
    {
        ...
        if ( $theme->getCustomCssFileName() !== null )
        {
            ...
            if ( SC::getStorage()->fileExists($oldCssFilePath) )
            {
                SC::getStorage()->removeFile($oldCssFilePath);
            }

            if ( SC::getStorage()->fileExists($oldMobileCssFilePath) )
            {
                SC::getStorage()->removeFile($oldMobileCssFilePath);
            }
        }
        ...
        SC::getStorage()->copyFile($tempCssFilePath, $newCssFilePath);
        ...
        SC::getEventManager()->trigger(new SC_Event("base.update_custom_css_file", array("name" => $theme->getName())));
    }
    ...
    public function getCustomCssFileUrl( $themeName, $mobile = false )
    {
        ...
        return SC::getStorage()->getFileUrl($this->getUserfileImagesDir() . ( $mobile ? "mobile_" : '' ) . $theme->getCustomCssFileName());
    }
    ...
    {
        return SC_DIR_STATIC_THEME . $themeName . ($mobile ? DS . self::DIR_NAME_MOBILE : '') . DS;
    }
    ...
    {
        return SC_URL_STATIC_THEMES . $themeName . ($mobile ? '/' . self::DIR_NAME_MOBILE : '') . '/';
    }
    ...
    {
        return SC_DIR_THEME . $themeName . ($mobile ? DS . self::DIR_NAME_MOBILE : '') . DS;
    }
    ...
    /**
     * Update theme info in the [SC_DB_PREFIX]_base_theme table according to the theme.xml file and force theme rebuilding if necessary
     * @param $name
    ...
    private function getThemeXmlInfo( $themeXmlPath )
    {
        ...
        {
            SC::getLogger()->addEntry(__CLASS__ . "::" . __FUNCTION__ . " - `" . $themeXmlPath . "` not found");
            return null;
        }
        ...
        if ( !$xmlInfo )
        {
            SC::getLogger()->addEntry(__CLASS__ . "::" . __FUNCTION__ . " - invalid `" . $themeXmlPath . "`");
            return null;
        }
        ...
        foreach ( $propList as $prop )
        {
            ...
            {
                SC::getLogger()->addEntry(__CLASS__ . "::" . __FUNCTION__ . " - in `" . $themeXmlPath . "` property `" . $prop . "` not found");
                return null;
            }
        }
    ...
    private function unlinkControlValueImage( $controlValue )
    {
        ...
        if ( SC::getStorage()->fileExists($this->getUserfileImagesDir() . $fileName) )
        {
            SC::getStorage()->removeFile($this->getUserfileImagesDir() . $fileName);
        }
    }
}
