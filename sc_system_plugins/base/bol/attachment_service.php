...
final class BOL_AttachmentService
{
    ...
    public function deleteExpiredTempImages()
    {
        ...
        foreach ( $attachList as $item )
        {
            ...
            if ( SC::getStorage()->fileExists($filePath) )
            {
                SC::getStorage()->removeFile($filePath);
            }
...
//        if ( $attch != NULL )
//        {
//            if ( SC::getStorage()->fileExists($this->getAttachmentsDir() . $attch->getFileName()) )
//            {
//                SC::getStorage()->removeFile($this->getAttachmentsDir() . $attch->getFileName());
//            }
//
...
//        else
//        {
//            if ( SC::getStorage()->fileExists($this->getAttachmentsDir() . basename($url)) )
//            {
//                SC::getStorage()->removeFile($this->getAttachmentsDir() . basename($url));
//            }
//        }
//    }
...
//
//        if ( SC::getUser()->isAuthenticated() && file_exists($filePath) )
//        {
//            SC::getStorage()->copyFile($filePath, $this->getAttachmentsDir() . $attch->getFileName());
//            unlink($filePath);
//        }
//
...
//
//        return SC::getStorage()->getFileUrl($this->getAttachmentsDir() . $attch->getFileName());
//    }
    /*
    ...
    {
        return $this->processUploadedFile($pluginKey, $fileInfo, $bundle, array('jpeg', 'jpg', 'png', 'gif'), (float) SC::getConfig()->getValue('base', 'tf_max_pic_size') * 1024);
    }
    /* attachments 1.6.1 update */

//    public function getAttachmentsTempUrl()
//    {
//        return SC::getPluginManager()->getPlugin('base')->getUserFilesUrl() . 'attachments/temp/';
//    }
//
//    public function getAttachmentsTempDir()
//    {
//        return SC::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'attachments' . DS . 'temp' . DS;
//    }

    public function getAttachmentsUrl()
    {
        return SC::getStorage()->getFileUrl(SC::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'attachments') . '/';
    }

    public function getAttachmentsDir()
    {
        return SC::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'attachments' . DS;
    }
    ...
    public function processUploadedFile( $pluginKey, array $fileInfo, $bundle = null, $validFileExtensions = array(), $maxUploadSize = null, $dimensions = null )
    {
        ...
        {
            $validFileExtensions = json_decode(SC::getConfig()->getValue('base', 'attch_ext_list'), true);
        }

        if ( $maxUploadSize === null )
        {
            $maxUploadSize = SC::getConfig()->getValue('base', 'attch_file_max_size_mb');
        }
        ...
        $attachDto->setUserId(SC::getUser()->getId());
        ...
        SC::getStorage()->copyFile($tempPath, $uploadPath);
        SC::getStorage()->chmod($uploadPath, 0666);
        unlink($tempPath);
    ...
    public function deleteAttachment( $userId, $attachId )
    {
        ...
        if ( $attach !== null && SC::getUser()->isAuthenticated() && SC::getUser()->getId() == $attach->getUserId() )
        {
            SC::getStorage()->removeFile($this->getAttachmentsDir() . $attach->getFileName());
