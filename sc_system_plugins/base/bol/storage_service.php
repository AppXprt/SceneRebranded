...
{
    const UPDATE_SERVER = "https://storage.scene.org/";
    /* ---------------------------------------------------------------------- */
    ...
    const URI_DSCNLOAD_PLATFORM_ARCHIVE = "download-platform";
    const URI_DSCNLOAD_ITEM = "get-item";
    ...
    const SCENE_STORE_DEV_KEY = "e547ebcf734341ec11911209d93a1054";
    ...
    {
        $requestArray = array("platform" => array(self::URI_VAR_BUILD => SC::getConfig()->getValue("base", "soft_build")),
            "items" => array());
        ...
        {
            SC::getLogger()->addEntry(__CLASS__ . "::" . __METHOD__ . "#" . __LINE__ . " storage request status is not OK",
                "core.update");
        ...
        {
            SC::getLogger()->addEntry(__CLASS__ . "::" . __METHOD__ . "#" . __LINE__ . " remote request returned empty result",
                "core.update");
        ...
        if ( !empty($resultArray["update"]) )
        {
            ...
            {
                SC::getConfig()->saveConfig("base", "update_soft", 1);
            }
    ...
    public function getItemInfoForUpdate( $key, $devKey, $currentBuild = 0 )
    {
        ...
        $requestUrl = SC::getRequest()->buildUrlQueryString($this->getStorageUrl(self::URI_GET_ITEM_INFO), $data);
    ...
    public function downloadPlatform()
    {
        $params = array(
            ...
            "site-url" => SC::getRouter()->getBaseUrl()
        );

        $data = array_merge($params, $this->triggerEventBeforeRequest($params));
        $requestUrl = SC::getRequest()->buildUrlQueryString($this->getStorageUrl(self::URI_DSCNLOAD_PLATFORM_ARCHIVE),
            $data);

        $paramsObj = new UTIL_HttpClientParams();
        $paramsObj->addParams($data);
        $response = UTIL_HttpClient::get($this->getStorageUrl(self::URI_DSCNLOAD_PLATFORM_ARCHIVE), $paramsObj);
        ...
        $archivePath = SC_DIR_PLUGINFILES . DS . $fileName;
    ...
    public function downloadItem( $key, $devKey, $licenseKey = null )
    {
        ...
        $paramsObj = new UTIL_HttpClientParams();
        $paramsObj->addParams($data);
        $response = UTIL_HttpClient::get($this->getStorageUrl(self::URI_DSCNLOAD_ITEM), $paramsObj);
    ...
    {
        $filePath = SC_DIR_ROOT . "sc_version.xml";
    ...
    public function getFtpConnection()
    {
        ...
        if ( !SC::getSession()->isKeySet("ftpAttrs") || !is_array(SC::getSession()->get("ftpAttrs")) )
        {
        ...
        else
        {
            ...
            try
            {
                $ftp = UTIL_Ftp::getConnection(SC::getSession()->get("ftpAttrs"));
            }
            catch ( Exception $ex )
            {
    ...
    {
        return SC_URL_HOME . "sc_updates/index.php";
    }
    ...
    {
        $event = SC::getEventManager()->trigger(new SC_Event('base.on_plugin_info_update', $params));
        $data = $event->getData();
        ...
        foreach ( $itemsToCheck as $item )
        {
            ...
            if ( isset($invalidItems[$type][$item->getKey()]) && $invalidItems[$type][$item->getKey()] == $item->getDeveloperKey() )
            {
                ...
                else if ( $this->isItemLicenseCheckPeriodExpired($item->getLicenseCheckTimestamp()) )
                {
                    ...
                    {
                        $defaultTheme = SC::getEventManager()->call("base.get_default_theme");
    ...
    {
        $event = new SC_Event(self::EVENT_ON_NOTIFY_ADMIN_ABOUT_INVALID_ITEMS, array(), $items);
        ...
        $params = array(
            ...
            "siteURL" => SC::getRouter()->getBaseUrl(),
            "adminUrl" => SC::getRouter()->urlForRoute("admin_plugins_installed")
        );

        $language = SC::getLanguage();
        $mail = SC::getMailer()->createMail();
        $mail->addRecipientEmail(SC::getConfig()->getValue("base", "site_email"));
    ...
    {
        $data["site-url"] = SC::getRouter()->getBaseUrl();
        $params = new UTIL_HttpClientParams();
        $params->addParams($data);
