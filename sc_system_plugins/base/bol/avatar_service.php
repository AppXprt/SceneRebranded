 ...
 * @package ow.sc_system_plugins.base.bol
 * @since 1.0
 */
class BOL_AvatarService
{
    ...
    public function deleteUserAvatar( $userId )
    {
        ...
        $event = new SC_Event('base.before_user_avatar_delete', array('avatarId' => $avatar->id ));
    ...
    public function cropAvatar( $userId, $path, $coords, $viewSize, array $editionalParams = array() )
    {
        ...
        $event = new SC_Event('base.after_avatar_update', array_merge($editionalParams, $params));
        SC::getEventManager()->trigger($event);
        ...
        {
            $toFilePath = SC::getPluginManager()->getPlugin('base')->getPluginFilesDir() . uniqid(md5( rand(0,9999999999) )).UTIL_File::getExtension($path);
    ...
    {
        $path = SC::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'avatars' . DS . 'tmp' . DS;
    ...
    public function renameAvatarOriginal( $userId, $oldHash, $newHash )
    {
        ...
        SC::getStorage()->renameFile($originalPath, $newPath);
    }
    ...
    {
        $event = new SC_Event("base.avatars.get_list", array(
            "userIds" => array($userId),
            "size" => $size,
            "checkModerationStatus" => $checkModerationStatus
        ));

        $eventAvatars = SC::getEventManager()->trigger($event)->getData();
    ...
    public function getDefaultAvatarUrl( $size = 1 )
    {
        ...
        {
            case 1:
                return SC::getThemeManager()->getSelectedTheme()->getStaticImagesUrl(SC::getApplication()->isMobile()) . 'no-avatar.png';

            case 2:
                return SC::getThemeManager()->getSelectedTheme()->getStaticImagesUrl(SC::getApplication()->isMobile()) . 'no-avatar-big.png';
        }
    ...
    private function getCustomDefaultAvatarUrl( $size = 1 )
    {
        ...
        $conf = json_decode(SC::getConfig()->getValue('base', 'default_avatar'), true);

        if ( isset($conf[$size]) )
        {
            $path = SC::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'avatars' . DS . $conf[$size];

            return SC::getStorage()->getFileUrl($path);
        }
    ...
    public function setCustomDefaultAvatar( $size, $file )
    {
        ...
        $dir = SC::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'avatars' . DS;
        ...
        if ( is_uploaded_file($file['tmp_name']) )
        {
            ...
            if ( $storage->copyFile($file['tmp_name'], $dir . $fileName) )
            {
                ...
                $conf[$size] = $fileName;
                SC::getConfig()->saveConfig('base', 'default_avatar', json_encode($conf));
    ...
    public function getAvatarUrlByAvatarDto( $avatar, $size = 1, $hash = null, $checkModerationStatus = true )
    {
        ...
        return SC::getStorage()->getFileUrl($dir . $avatarFile);
    }
    ...
    {
        $url = SC::getPluginManager()->getPlugin('base')->getUserFilesUrl() . 'avatars/tmp/';
    ...
    {
        return SC::getPluginManager()->getPlugin('base')->getUserFilesDir() . 'avatars' . DS;
    }

    public function getAvatarsPluginFilesDir()
    {
        return SC::getPluginManager()->getPlugin('base')->getPluginFilesDir() . 'avatars' . DS;
    }
    ...
    public function trackAvatarChangeActivity( $userId, $avatarId )
    {
        ...
        $event = new SC_Event('feed.action', array(
            ...
            'string' => SC::getLanguage()->text('base', 'avatar_feed_string'),
            /* 'content' => '<img src="' . $this->getAvatarUrl($userId) . '" />', */
            'view' => array(
                'iconClass' => 'sc_ic_picture'
            )
        ));
        SC::getEventManager()->trigger($event);
    }
    ...
    public function getAvatarChangeGalleryItem( $entityType, $entityId, $itemId )
    {
        ...
        $event = new SC_Event('base.avatar_change_get_item', $params);
    ...
    {
        $key = SC::getSession()->get(self::AVATAR_CHANGE_SESSION_KEY);
    ...
    public function setAvatarChangeSessionKey()
    {
        ...
        {
            $key = uniqid();
            SC::getSession()->set(self::AVATAR_CHANGE_SESSION_KEY, $key);
        }
    }
