...
final class BOL_CommentService
{
    ...
    const CONFIG_ALLSCED_TAGS = 'allowed_tags';
    const CONFIG_ALLSCED_ATTRS = 'allowed_attrs';
    ...
    private function __construct()
    {
        ...
        //$this->configs[self::CONFIG_ALLSCED_TAGS] = array('a', 'b', 'i', 'span', 'u', 'strong', 'br');
        //$this->configs[self::CONFIG_ALLSCED_ATTRS] = array('style', 'href');
    }
    ...
    public function addComment( $entityType, $entityId, $pluginKey, $userId, $message, $attachment = null )
    {
        ...
        //$message = UTIL_HtmlTag::stripTags($message, $this->configs[self::CONFIG_ALLSCED_TAGS], $this->configs[self::CONFIG_ALLSCED_ATTRS]);
    ...
    public function findBatchCommentsData( array $items )
    {
        ...
        if ( SC::getUser()->isAuthenticated() )
        {
            $userInfo = BOL_AvatarService::getInstance()->getDataForUserAvatars(array(SC::getUser()->getId()));
        }
        ...
        if ( SC::getUser()->isAuthenticated() )
        {
            $resultArray['_static']['currentUserInfo'] = $currentUserInfo[SC::getUser()->getId()];
        }

        $eventParams = array('actionList' => $creditsParams);
        $resultArray['_static']['credits'] = SC::getEventManager()->call('usercredits.batch_check_balance_for_action_list', $eventParams);
