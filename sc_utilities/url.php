...
class UTIL_Url
{
    ...
    */
    ...
    {
        $userFilesUrl = SC::getStorage()->getFileUrl(SC_DIR_USERFILES);
        $path = null;

        if ( stripos($uri, SC_URL_HOME) !== false )
        {
            $path = str_replace(SC_URL_HOME, SC_DIR_ROOT, $uri);
            $path = str_replace('/', DS, $path);
        }
        else if ( stripos($uri, $userFilesUrl) !== false )
        {
            $path = str_replace($userFilesUrl, SC_DIR_USERFILES, $uri);
            $path = str_replace('/', DS, $path);
        }
