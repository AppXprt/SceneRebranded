...
class UTIL_HtmlTag
{
        ...
        $mediaSrcValidate = true )
    {
        if ( self::$jevix === null )
        {
            require_once SC_DIR_LIB . 'jevix' . DS . 'jevix.class.php';
        ...
        if ( $attrList !== null )
        {
            foreach ( $attrList as $attr )
            {
                if ( strstr($attr, '.') )
                {
                    ...
                    if ( !isset($tagRules[$tag][Jevix::TR_PARAM_ALLSCED]) )
                    {
                        $tagRules[$tag][Jevix::TR_PARAM_ALLSCED] = array();
                    }

                    $tagRules[$tag][Jevix::TR_PARAM_ALLSCED][$param] = true;
                }
                else
                {
